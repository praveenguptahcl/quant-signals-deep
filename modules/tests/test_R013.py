...
NS = 1_000_000_000
STATE_LABELS = ['normal', 'elevated', 'abnormal', 'undefined', 'warming', 'missing', 'invalid']
...
# ============================ R013 ============================
# RB-list: R013 Abnormal volume / participation regime. Grok RB2 worked example:
# lookback 9 days (M shares): 1.1,0.9,1.0,1.2,0.8,1.0,1.1,0.95,1.05;
# μ=1.011m, σ=0.119m; today 1.45m -> z ≈ 3.69 -> abnormal.
R013_VOL = [1.10, 0.90, 1.00, 1.20, 0.80, 1.00, 1.10, 0.95, 1.05, 1.45]

def r013_tape():
    hdr = ["bar", "event_ts", "asof_ts", "vol"]
    rows = []
    for i, v in enumerate(R013_VOL, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS, "vol": v})
    return hdr, rows

def _vol_z(vols):
    if len(vols) < 4:
        return float("nan")
    base, today = vols[:-1], vols[-1]
    mu = _mean(base)
    sd = _stdev(base, ddof=1)
    return (today - mu) / sd if sd > 0 else float("nan")

def _vol_z_robust(vols):
    if len(vols) < 4:
        return float("nan")
    base, today = vols[:-1], vols[-1]
    s = sorted(base)
    med = s[len(s) // 2]
    mad = sorted(abs(v - med) for v in base)[len(s) // 2]
    scl = 1.4826 * mad
    return (today - med) / scl if scl > 0 else float("nan")

def _r013_state(z):
    if not math.isfinite(z):
        return "undefined"
    return "abnormal" if z > 3 else ("elevated" if z > 2 else "normal")

def primary_indicator_r013(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("vol" not in r or not r["vol"] >= 0 for r in rows):
    ...
    z = _vol_z([r["vol"] for r in rows])
    return {"value": z, "state": _r013_state(z), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r013(rows, cfg):
    rows = list(rows)
    return {"value": _vol_z_robust([r["vol"] for r in rows]),
            "state": "n/a", "computed_at": rows[-1]["event_ts"] if rows else 0}

F2_POISON_R013 = '''
F2_POISON_TAPE = [
    {"bar": i + 1, "event_ts": 1000 + i, "asof_ts": 1001 + i, "vol": 1.0}
    for i in range(8)
]  # zero variance -> z = 0/0 = NaN -> F2
'''


primary_indicator = primary_indicator_r013
second_estimator = second_estimator_r013
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if r["module_state"] != "OK":
        return RegimeState("R013", r["state"], r["value"], ESTIMATOR_VERSION,
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R013", "out_of_bounds", v, ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R013", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

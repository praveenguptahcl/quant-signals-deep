...
def recompute(trows):
    """Huang-Stoll fixture: op = S*(1-a-b), inv = S*b, adv = S*a."""
    out = []
    for r in trows:
        s, a, b = float(r[1]), float(r[2]), float(r[3])
        out.append([r[0], round(s * (1 - a - b), 6), round(s * b, 6), round(s * a, 6)])
    return out
...
def test_06_shares_sum():
    ...
    s = float(trows[0][1])
    e = erows[0]
    assert abs(float(e[1]) + float(e[2]) + float(e[3]) - s) < 1e-9
    a, b = float(trows[0][2]), float(trows[0][3])

"""Concrete sketch: T014 Bollinger Reversal + Squeeze Exit.

Run: python3 -m pytest modules/tests/test_T014.py -q
"""
import csv
import math
import os
...
SID = "T014"
TAPE = os.path.join("modules", "fixtures", "T014_tape.csv")
EXPECTED = os.path.join("modules", "fixtures", "T014_expected.csv")
TOL = 1e-9
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["3 consecutive band-rides [example]", "feed heartbeat missed > 2 s [example]", "clock skew > 50 ms [example]", "4 concurrent band reversals [example]"]
    stale_s = 90.0
    if stale_s >= 2.0:

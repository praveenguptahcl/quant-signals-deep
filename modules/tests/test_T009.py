"""Concrete sketch: T009 ETF-vs-Basket Arbitrage.

Run: python3 -m pytest modules/tests/test_T009.py -q
"""
import csv
import math
import os
...
SID = "T009"
TAPE = os.path.join("modules", "fixtures", "T009_tape.csv")
EXPECTED = os.path.join("modules", "fixtures", "T009_expected.csv")
TOL = 1e-9
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["PCF match < 99% [example]", "creation pipeline unavailable [example]", "clock skew > 50 ms [example]", "5 concurrent arb positions [example]"]
    stale_s = 90.0
    if stale_s >= 2.0:

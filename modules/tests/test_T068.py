"""T068 — Scheduled-Event Vol Expansion: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T068.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T068"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 20.0, 'z_short': -20.0, 'conf_scale': 1.0, 'edge_mult': 1.0, 'time_stop': 5, 'exit_flip': False, 'k': 0.5, 'sizing': ('fixed', 50), 'cooldown_bars': 5, 'bar_ns': 86400000000000, 'tif': 'DAY', 'venue': 'CBOE', 'side_class': 'mixed', 'adv_pct': 0.01, 'cost': {'spread_bps': 6.0, 'fee_bps': 2.0, 'borrow_bps': 0.0, 'impact_bps': 2.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S034")

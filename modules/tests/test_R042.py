"""R042 — Cross-sectional dispersion / stock-picking environment: acceptance tests (concrete sketch).
...
CFG = {"picker": 0.015, "index": 0.007, "N": 6, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _xs = [_parse_float(e['s%d' % _i]) for _i in range(1,7)]
    if any(not math.isfinite(_v) for _v in _xs):
        raise ValueError('non-finite name return')
    _n = len(_xs); _m = sum(_xs)/_n
    return math.sqrt(sum((_a-_m)**2 for _a in _xs)/(_n-1))
...
def _dual_aggregate(events, cfg):
    return statistics.stdev([float(events[-1]['s%d' % _i]) for _i in range(1,7)])

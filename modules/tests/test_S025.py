#!/usr/bin/env python3
"""Acceptance tests for S025 — Intraday time-series momentum (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Momentum fixture: lookback return vs bar 1 in bps; trigger at |r|>=10."""
    out = []
    base = float(trows[0][1])
    for r in trows:
        c = float(r[1])
        rb = round((c - base) / base * 1e4, 1)
        trig = 1 if rb >= 10.0 else (-1 if rb <= -10.0 else 0)
...
def test_06_triggers():
    # Bars 2,3 LONG; bar 4 flat; bar 1 baseline.
    eheader, erows = load(EXP)
    got = {e[0]: int(e[2]) for e in erows}
    assert got["2"] == 1 and got["3"] == 1 and got["4"] == 0 and got["1"] == 0


def test_02_signal_vector_shape():
    sig = signal_stub(1, 10.0, computed_at=1700000000000000000)
...
def test_03_no_signal_bar_fills():
    ...
    sig = signal_stub(1, 10.0, computed_at=computed_at)

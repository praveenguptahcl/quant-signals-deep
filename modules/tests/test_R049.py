...
def _row_value(e, cfg):
    _v = _parse_float(e['vix']); _f = _parse_float(e['vix3m'])
    if not (math.isfinite(_v) and math.isfinite(_f)) or _v <= 0:
        raise ValueError('bad VIX inputs')  # F1/F2
    return _f - _v
...
def _dual_aggregate(events, cfg):
    return float(events[-1]['vix3m']) - float(events[-1]['vix'])

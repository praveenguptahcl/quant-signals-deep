"""R048 — Put/call ratio regime: acceptance tests (concrete sketch).
...
CFG = {"fear": 1.2, "greed": 0.6, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _p = _parse_float(e['pcr'])
    if not math.isfinite(_p) or _p <= 0:
        raise ValueError('bad put/call print')  # F1/F2
    return _p

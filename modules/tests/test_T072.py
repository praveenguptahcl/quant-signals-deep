"""T072 — Social-Sentiment Ignition Rider: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T072.py -q` exits 0.
"""
import csv
import math
import os
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S097")
...
def test_5_invalid_input_yields_unknown():
    ...
    bad[3]["sig"] = float("nan")
    _, mstate = emit_intents(bad, CFG, SID, "S097")
    assert mstate == "UNKNOWN"
    bad2 = [dict(r) for r in rows]
    bad2[4]["close"] = -1.0
    _, mstate2 = emit_intents(bad2, CFG, SID, "S097")

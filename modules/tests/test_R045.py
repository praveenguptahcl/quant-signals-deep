"""R045 — Cross-sectional volatility regime: acceptance tests (concrete sketch).
...
CFG = {"wide": 0.50, "narrow": 0.25, "N": 8, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _xs = [_parse_float(e['v%d' % _i]) for _i in range(1,9)]
    if any(not math.isfinite(_v) for _v in _xs) or any(_v < 0 for _v in _xs):
        raise ValueError('non-finite vol')
...
def _dual_aggregate(events, cfg):
    return statistics.stdev([float(events[-1]['v%d' % _i]) for _i in range(1,9)])

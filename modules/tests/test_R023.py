...
NS = 1_000_000_000
STATE_LABELS = ['normal', 'stress', 'cascade', 'undefined', 'warming', 'missing', 'invalid']
...
R023_LIQ = [60, 70, 65, 80, 75, 90, 85, 95, 100, 90, 70, 85, 95, 110, 100,
            90, 80, 75, 85, 90, 95, 105, 100, 90, 85, 95, 100, 90, 80, 480]
...
def r023_tape():
    ...
    for i, v in enumerate(R023_LIQ, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS, "reported_liq": v * 1e6})
    return hdr, rows
...
def _r023_state(z):
    if not math.isfinite(z):
        return "undefined"
    return "cascade" if z > 6 else ("stress" if z >= 3 else "normal")

def primary_indicator_r023(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("reported_liq" not in r or not (r["reported_liq"] >= 0) for r in rows):
    ...
    hist = [r["reported_liq"] / 1e6 for r in rows[:-1]]
    m = _med(hist)
    mad = _mad(hist, m)
    scl = 1.4826 * mad
    x = rows[-1]["reported_liq"] / 1e6
    v = (x - m) / scl if scl > 0 else (float("inf") if x > m else (float("-inf") if x < m else 0.0))
    return {"value": v, "state": _r023_state(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r023(rows, cfg):
    rows = list(rows)
    if len(rows) < 30:
        return {"value": float("nan"), "state": "undefined", "computed_at": 0}
    hist = [r["reported_liq"] / 1e6 for r in rows[-26:-1]]  # 25-day baseline
    m = _med(hist)
    mad = _mad(hist, m)
    scl = 1.4826 * mad
    v = (rows[-1]["reported_liq"] / 1e6 - m) / scl if scl > 0 else float("nan")
    return {"value": v, "state": "n/a", "computed_at": rows[-1]["event_ts"]}

F2_POISON_R023 = '''
F2_POISON_TAPE = [
    {"bar": i + 1, "event_ts": 1000 + i, "asof_ts": 1001 + i, "reported_liq": 90e6}
    for i in range(29)
] + [{"bar": 30, "event_ts": 1029, "asof_ts": 1030, "reported_liq": 1e12}]
  # degenerate baseline (MAD = 0) + $1T print -> z = +inf > 100 bound -> F2
'''


primary_indicator = primary_indicator_r023
second_estimator = second_estimator_r023

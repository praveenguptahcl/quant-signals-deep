"""Concrete sketch: T025 Trade-Classification Trend Filter.
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["3 consecutive filter vetoes of winners [example]", "trade tape stale > 30 s [example]", "clock skew > 50 ms [example]", "4 concurrent filtered trends [example]"]
    stale_s = 90.0
    if stale_s >= 2.0:

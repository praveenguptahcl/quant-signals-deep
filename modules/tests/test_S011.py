#!/usr/bin/env python3
"""Acceptance tests for S011 — Amihud illiquidity (sketch-level, concrete)."""
import csv
import math
import pathlib

FIX = pathlib.Path(__file__).resolve().parent.parent / "fixtures"
TAPE = FIX / "S011_tape.csv"
EXP = FIX / "S011_expected.csv"
TOL = 1e-4
...
def recompute(trows):
    """Amihud fixture: ret from close-to-close; ILLIQ = |r|/DV * 1e6."""
    out, prev = [], None
    for r in trows:
        close, dv = float(r[1]), float(r[2])
        ret = 0.0 if prev is None else (close - prev) / prev
        prev = close
        out.append([r[0], round(ret, 6), round(abs(ret) / dv * 1e6, 6)])
    return out
...
def test_06_illiq_mean():
    # Mean ILLIQ x 1e6 over days 2..6 (day 1 has no return).
    eheader, erows = load(EXP)
    vals = [float(e[2]) for e in erows[1:]]
    mean = sum(vals) / len(vals)
    assert mean > 0
    assert abs(mean - 0.0005294) < 1e-6


def test_02_signal_vector_shape():
    sig = signal_stub(5e-7, 1e-6, computed_at=1700000000000000000)
...
def test_03_no_signal_bar_fills():
    ...
    sig = signal_stub(5e-7, 1e-6, computed_at=computed_at)

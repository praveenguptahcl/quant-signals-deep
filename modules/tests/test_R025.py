"""Acceptance tests for R025 - Yield-curve shape regime.
...
BOUNDS = (-1000.0, 1000.0)  # mathematical bounds of the indicator value (F2)
...
# ============================ R025 ============================
# RB-list: R025 Yield-curve shape regime. Grok RB3 worked example:
# y10 4.2%, y2 4.8% -> 2s10s = -60bp -> inverted.
R025_Y10 = [4.3, 4.25, 4.22, 4.2, 4.18, 4.2, 4.18, 4.18, 4.19, 4.2]
R025_Y2 = [4.5, 4.55, 4.6, 4.65, 4.7, 4.72, 4.75, 4.78, 4.79, 4.8]

def r025_tape():
    hdr = ["bar", "event_ts", "asof_ts", "y10", "y2"]
    rows = []
    for i, (y10, y2) in enumerate(zip(R025_Y10, R025_Y2), 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS, "y10": y10, "y2": y2})
    return hdr, rows

def _r025_state(s):
    if not math.isfinite(s):
        return "undefined"
    return "inverted" if s < -25 else ("flat" if s <= 25 else "normal")

def primary_indicator_r025(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("y10" not in r or "y2" not in r or not (0 <= r["y2"] <= 20 and 0 <= r["y10"] <= 20) for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    v = (rows[-1]["y10"] - rows[-1]["y2"]) * 100
    return {"value": v, "state": _r025_state(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r025(rows, cfg):
    # FRED T10Y2Y-style cross-check: 3-obs average spread
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "undefined", "computed_at": 0}
    v = _mean([(r["y10"] - r["y2"]) * 100 for r in rows[-3:]])
    return {"value": v, "state": "n/a", "computed_at": rows[-1]["event_ts"]}
...
primary_indicator = primary_indicator_r025
second_estimator = second_estimator_r025
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R025", "out_of_bounds", v, ESTIMATOR_VERSION,

...
def signal(state, bars, cfg):
    ...
    b = bars[-1]
    if any(r["gamma"] < 0 or not math.isfinite(r["gamma"]) for r in bars):
        return SignalVector("TEST", 0, 0.0, 0.0, b["event_ts"], 0, "UNKNOWN")
    tot = sum(r["gamma"] * r["net_oi"] * 10000.0 for r in bars)
    # negative dealer gamma -> amplifying regime -> fade momentum (direction -1 on up-moves)
    direction = -1 if tot < 0 else 0  # [example] convention
...
def test_invalid_input_yields_unknown():
    bad = {"strike": 100.0, "event_ts": 1, "asof_ts": 2, "gamma": -0.5, "net_oi": 100.0}
    s = signal({}, [bad], Config())

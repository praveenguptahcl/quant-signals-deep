"""Acceptance tests for T086 — OFI-Paced Participation Tracker.
...
Run: python3 -m pytest modules/tests/test_T086.py -q   (from repo root)
"""
import csv
import hashlib
import uuid
...
TAPE = FIX / "T086_tape.csv"
EXPECTED = FIX / "T086_expected.csv"

GATE_CFG = [('<=', 1.5), ('>=', 1.0), ('>=', 1.0)]   # [(op, threshold)] for g1, g2, g3
GATE_NAMES = ['rvol_ok', 'z_demeaned', 'parent_ok']
COST_GATE_K = 0.5                       # [default]
PER_TRADE_R = 6153        # $ risk per trade [example]
DAILY_LOSS_STOP_R = 2  # in units of R [default]
SYMBOL = 'EXE:XNAS'
PARENT_SIGNAL = 'S001'
...
def test_cost_gate_predicate():
    ...
    assert not (cost <= k * 2.4000000000000004)  # tiny edge -> gate blocks

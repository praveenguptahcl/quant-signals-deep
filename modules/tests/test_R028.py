...
CFG = {"stress_bps": 550.0, "watch_bps": 350.0, "acute_chg_bps": 100.0, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    v = _parse_float(e['oas'])
    if not math.isfinite(v) or v < 0:
        raise ValueError('bad OAS')  # F1/F2
    return v
...
def _dual_aggregate(events, cfg):
    if len(events) < 21:
        return float('nan')
    return float(events[-1]['oas']) - float(events[0]['oas'])


def _state_of(value, e, cfg):
    last = float(e['oas'])
    if last > cfg['stress_bps'] or value > cfg['acute_chg_bps']:
        return 'STRESSED'
    if last > cfg['watch_bps']:
        return 'WATCH'
    return 'CALM'
...
def test_f2_bounds_violation_unknown():
    ...
    assert got[20]["module_state"] == "UNKNOWN", got[20]
...
def test_spot_handcheck():
    ...
    tape = _load('R028_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[20]['value'] - 140.0) <= 1e-9, got[20]['value']
    assert got[20]['state'] == 'STRESSED'
    assert got[0]['state'] == 'UNKNOWN'  # <21 rows: insufficient lookback

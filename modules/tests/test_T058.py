"""T058 — Dispersion Trader: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T058.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T058"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 8.0, 'z_short': -8.0, 'conf_scale': 1.0, 'edge_mult': 2.5, 'time_stop': 30, 'exit_flip': False, 'k': 0.5, 'sizing': ('fixed', 40), 'cooldown_bars': 5, 'bar_ns': 86400000000000, 'tif': 'DAY', 'venue': 'CBOE', 'side_class': 'mixed', 'adv_pct': 0.01, 'cost': {'spread_bps': 5.0, 'fee_bps': 2.0, 'borrow_bps': 0.0, 'impact_bps': 3.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S075")

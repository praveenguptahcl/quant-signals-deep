...
CFG = {"W": 63, "confirm_days": 21, "pos": 0.20, "min_lag_bars": 1}
...
def _aggregate(events, vals, cfg):
    if len(events) < 2:
        return float('nan')
    _xs = [float(_e['spy']) for _e in events]
    _ys = [float(_e['tlt']) for _e in events]
    _n = len(_xs); _mx = sum(_xs)/_n; _my = sum(_ys)/_n
    _cov = sum((_a-_mx)*(_b-_my) for _a,_b in zip(_xs,_ys))
    _vx = sum((_a-_mx)**2 for _a in _xs); _vy = sum((_b-_my)**2 for _b in _ys)
...
def _dual_aggregate(events, cfg):
    ...
    _xs = [float(_e['spy']) for _e in events]
    _ys = [float(_e['tlt']) for _e in events]
    return statistics.correlation(_xs, _ys)
...
def test_spot_handcheck():
    ...
    tape = _load('R032_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[7]['value'] - 0.3312) <= 1e-3, got[7]['value']
    assert got[7]['state'] == 'INFLATION_FEAR'

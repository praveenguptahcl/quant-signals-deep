"""Acceptance tests for T083 — Retail-Flow Internalizer Fade.
...
Run: python3 -m pytest modules/tests/test_T083.py -q   (from repo root)
"""
import csv
import hashlib
import uuid
...
TAPE = FIX / "T083_tape.csv"
EXPECTED = FIX / "T083_expected.csv"

GATE_CFG = [('>=', 1.5), ('<=', 0.4), ('<=', 0.5)]   # [(op, threshold)] for g1, g2, g3
GATE_NAMES = ['z_abs', 'theta_ok', 'bounce_ok']
COST_GATE_K = 0.5                       # [default]
PER_TRADE_R = 125        # $ risk per trade [example]
...
def test_cost_gate_predicate():
    ...
    assert not (cost <= k * 0.38)  # tiny edge -> gate blocks

#!/usr/bin/env python3
"""Acceptance tests for S005 — Multi-level book pressure (static depth imbalance) (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Static depth imbalance BP_5 = (sum bids - sum asks) / (sum bids + sum asks)."""
    out = []
    for r in trows:
        bids = [float(x) for x in r[1:6]]
        asks = [float(x) for x in r[6:11]]
        out.append([r[0], round((sum(bids) - sum(asks)) / (sum(bids) + sum(asks)), 4)])
    return out
...
def test_06_depth_tilt_direction():

...
def recompute(rows):
    fair = 100.0 * (math.exp(0.032 * 90.0 / 365.0) - 1)
    out = []
    for x in rows:
        dev = (x["back"] - x["front"]) - fair
        d = 1 if dev > 0.10 else (-1 if dev < -0.10 else 0)
...
def test_fixture_recomputes_to_expected():
    ...
    for g, e in zip(got, exp):
        ...
        assert abs(got[0]["fair_spread"] - 100.0*(math.exp(0.032*90.0/365.0)-1)) < 1e-9

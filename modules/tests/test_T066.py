"""T066 — VWAP-Cross Institutional Follower: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T066.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T066"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 0.3, 'z_short': -0.3, 'conf_scale': 1.0, 'edge_mult': 14.0, 'time_stop': 60, 'exit_flip': True, 'k': 0.5, 'sizing': ('risk', 400.0, None), 'cooldown_bars': 10, 'bar_ns': 60000000000, 'tif': 'DAY', 'venue': 'XNAS', 'side_class': 'mixed', 'adv_pct': 0.02, 'cost': {'spread_bps': 1.5, 'fee_bps': 1.2, 'borrow_bps': 0.0, 'impact_bps': 0.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S024")

"""R040 — Index rebalance / inclusion flows: acceptance tests (concrete sketch).
...
CFG = {"large": 0.25, "moderate": 0.05, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _w = _parse_float(e['w_new']); _a = _parse_float(e['aum_bn'])
    _p = _parse_float(e['price']); _ad = _parse_float(e['adv'])
    if not all(math.isfinite(_v) for _v in [_w,_a,_p,_ad]) or _p <= 0 or _ad <= 0:
        raise ValueError('bad rebalance inputs')  # F1/F2
    return 0.0


def _aggregate(events, vals, cfg):
    _n = float(events[-1]['w_new'])*float(events[-1]['aum_bn'])*1e9/float(events[-1]['price'])
    return _n/float(events[-1]['adv'])


def _dual_aggregate(events, cfg):
    _flow = float(events[-1]['w_new'])*1e9*float(events[-1]['aum_bn'])
    _shares = _flow/float(events[-1]['price'])
    return _shares/float(events[-1]['adv'])

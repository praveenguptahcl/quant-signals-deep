"""Concrete sketch: T007 Cointegration Z-Score Pairs.

Run: python3 -m pytest modules/tests/test_T007.py -q
"""
import csv
import math
import os
...
SID = "T007"
TAPE = os.path.join("modules", "fixtures", "T007_tape.csv")
EXPECTED = os.path.join("modules", "fixtures", "T007_expected.csv")
TOL = 1e-9
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["leg halted", "ADF fails on recalibration [example]", "clock skew > 50 ms [example]", "8 concurrent pairs reached [example]"]
    stale_s = 120.0
    if stale_s >= 2.0:

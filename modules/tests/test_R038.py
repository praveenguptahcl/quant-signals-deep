...
CFG = {"thin": 0.70, "severe": 0.50, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _v = _parse_float(e['cumvol']); _b = _parse_float(e['baseline'])
    if not (math.isfinite(_v) and math.isfinite(_b)) or _b <= 0:
        raise ValueError('bad liquidity inputs')  # F1/F2
    return _v/_b
...
def _dual_aggregate(events, cfg):
    return float(events[-1]['cumvol']) / float(events[-1]['baseline'])
...
def test_spot_handcheck():
    ...
    tape = _load('R038_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[0]['value'] - 0.55) <= 1e-12 and got[0]['state'] == 'THIN'

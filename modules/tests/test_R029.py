...
CFG = {"pct_hi": 0.80, "pct_lo": 0.20, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    ...
    if not (math.isfinite(g) and math.isfinite(p)):
        raise ValueError('non-finite DXY inputs')  # F1/F2
    if not (0.0 <= p <= 1.0):
...
def _state_of(value, e, cfg):
    p = float(e['pctile'])
    if value > 0 and p >= cfg['pct_hi']:
        return 'STRONG_DOLLAR_UPTREND'
    if value < 0 and p <= cfg['pct_lo']:
        return 'WEAK_DOLLAR_DOWNTREND'
    return 'DOLLAR_UPTREND' if value > 0 else 'DOLLAR_DOWNTREND'
...
def test_spot_handcheck():
    ...
    assert got[0]['state'] == 'STRONG_DOLLAR_UPTREND'
    assert got[1]['state'] == 'WEAK_DOLLAR_DOWNTREND'

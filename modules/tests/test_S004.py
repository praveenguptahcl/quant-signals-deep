#!/usr/bin/env python3
"""Acceptance tests for S004 — Microprice (Stoikov) (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Closed-form microprice: (qa*bid + qb*ask) / (qa + qb); I = qb/(qb+qa)."""
    out = []
    for r in trows:
        ev, bid, qb, ask, qa = r[0], float(r[1]), float(r[2]), float(r[3]), float(r[4])
        micro = (qa * bid + qb * ask) / (qa + qb)
        out.append([ev, round(micro, 3), round((bid + ask) / 2, 3), round(qb / (qb + qa), 4)])
    return out

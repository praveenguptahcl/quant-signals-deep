"""Concrete sketch: T018 RVOL-Gated Gap-and-Go.

Run: python3 -m pytest modules/tests/test_T018.py -q
"""
import csv
import math
import os
...
SID = "T018"
TAPE = os.path.join("modules", "fixtures", "T018_tape.csv")
EXPECTED = os.path.join("modules", "fixtures", "T018_expected.csv")
TOL = 1e-9
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    borrow_bps = 0.0    # [default] go reference; shorts add C7
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["2 consecutive gap fills [example]", "feed heartbeat missed > 2 s [example]", "clock skew > 50 ms [example]", "4 concurrent gap rides [example]"]
    stale_s = 90.0
    if stale_s >= 2.0:

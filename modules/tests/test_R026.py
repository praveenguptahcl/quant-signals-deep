"""R026 — Inflation-surprise regime: acceptance tests (concrete sketch).
...
CFG = {"sigma_L": 0.2, "L": 12, "z_hot": 1.5, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    ...
    c = _parse_float(e['consensus'])
    if not (math.isfinite(a) and math.isfinite(c)):
        raise ValueError('non-finite CPI inputs')  # F1/F2
    return (a - c) / cfg['sigma_L']
...
def _dual_aggregate(events, cfg):
    a = float(events[-1]['actual']); c = float(events[-1]['consensus'])
    return (a - c) / cfg['sigma_L']
...
def test_spot_handcheck():
    ...
    tape = _load('R026_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[2]['value'] - 2.0) <= 1e-9, got[2]
    assert got[2]['state'] == 'HOT'
    assert abs(got[3]['value'] - (-2.0)) <= 1e-9, got[3]

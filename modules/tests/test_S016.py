#!/usr/bin/env python3
"""Acceptance tests for S016 — Hasbrouck information share (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Information-share fixture: leader = 1 if IS >= 0.6 else 0."""
    out = []
    for r in trows:
        isv = float(r[1])
        out.append([r[0], isv, 1 if isv >= 0.6 else 0])
    return out
...
def test_06_shares_sum():
    # Information shares sum to 1; exactly one leader at the 0.6 threshold.
    eheader, erows = load(EXP)
    assert abs(sum(float(e[1]) for e in erows) - 1.0) < 1e-9
    assert sum(int(e[2]) for e in erows) == 1

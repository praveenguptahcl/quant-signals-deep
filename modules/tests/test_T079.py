"""Acceptance tests for T079 — Post-Announcement Vol Fade.
...
Run: python3 -m pytest modules/tests/test_T079.py -q   (from repo root)
"""
import csv
import hashlib
import uuid
...
TAPE = FIX / "T079_tape.csv"
EXPECTED = FIX / "T079_expected.csv"
...
def test_cost_gate_predicate():
    ...
    assert cost > 0
    assert cost <= k * 432.0      # large edge -> gate passes
    assert not (cost <= k * 14.4)  # tiny edge -> gate blocks

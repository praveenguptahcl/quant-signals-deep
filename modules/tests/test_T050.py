...
Run: pytest modules/tests/test_T050.py -q   (from repo root)
"""
import csv
import math
...
def test_kill_switch_trip_and_rearm():
    ...
    assert t9 is None and ms9 == "OFF" and note9 == "kill-tripped"
    assert st["kill"].state == "TRIPPED"
    # still tripped on the next bar: no intents while TRIPPED
    t10, ms10, _ = process_bar(st, bars[10], CFG)
    assert t10 is None and ms10 == "OFF"

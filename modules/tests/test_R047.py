"""R047 — Sentiment / positioning regime: acceptance tests (concrete sketch).
...
CFG = {"extreme": 2.0, "calm": 0.5, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _z = _parse_float(e['sent_z'])
    if not math.isfinite(_z):
        raise ValueError('non-finite sentiment z')
    return _z
...
def _dual_aggregate(events, cfg):
    return float(events[-1]['sent_z'])


def _state_of(value, e, cfg):
    if value <= -cfg['extreme']:
        return 'PANIC'
    if value >= cfg['extreme']:
        return 'EUPHORIA'
    if abs(value) <= cfg['calm']:
        return 'CALM'
    return 'ELEVATED'


def _bounds_ok(value, cfg):
    return math.isfinite(value) and abs(value) <= 10.0
...
def test_spot_handcheck():
    ...
    tape = _load('R047_tape.csv')
    got = detect(None, tape, CFG)
    assert got[0]['state'] == 'PANIC', got[0]
    assert got[1]['state'] == 'CALM'
    assert got[2]['state'] == 'ELEVATED'

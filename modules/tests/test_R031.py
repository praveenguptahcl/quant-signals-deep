"""R031 — Cross-asset correlation regime: acceptance tests (concrete sketch).
...
CFG = {"W": 63, "fused": 0.50, "diverse": 0.30, "min_lag_bars": 1}
...
def _aggregate(events, vals, cfg):
    if len(events) < 2:
        return float('nan')
    _assets = ['A','B','C','D','E']
    _cols = [[float(_e[_a]) for _e in events] for _a in _assets]
    def _c(_x, _y):
    ...
    _ps = [_c(_cols[_i], _cols[_j]) for _i in range(5) for _j in range(_i+1, 5)]
...
def _dual_aggregate(events, cfg):
    ...
    _assets = ['A','B','C','D','E']
    _cols = [[float(_e[_a]) for _e in events] for _a in _assets]
    _ps = [statistics.correlation(_cols[_i], _cols[_j]) for _i in range(5) for _j in range(_i+1, 5)]
    return sum(_ps)/len(_ps)

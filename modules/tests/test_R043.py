"""R043 — Short interest / crowdedness: acceptance tests (concrete sketch).
...
CFG = {"squeeze": 0.25, "crowded": 0.12, "dtc_crit": 5.0, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _s = _parse_float(e['short_pct_float']); _d = _parse_float(e['days_to_cover'])
    if not (math.isfinite(_s) and math.isfinite(_d)):
        raise ValueError('non-finite SI inputs')
    return 0.0
...
def _state_of(value, e, cfg):
    _s = float(e['short_pct_float']); _d = float(e['days_to_cover'])
    if _s >= cfg['squeeze']:
        return 'SQUEEZE_WATCH'
    if _s >= cfg['crowded'] or _d >= cfg['dtc_crit']:
        return 'CROWDED'
    return 'NORMAL'
...
def test_spot_handcheck():
    ...
    tape = _load('R043_tape.csv')
    got = detect(None, tape, CFG)
    assert got[0]['state'] == 'CROWDED', got[0]
    assert got[1]['state'] == 'NORMAL'
    assert got[2]['state'] == 'SQUEEZE_WATCH'

...
def signal(state, bars, cfg):
    ...
    mu = sum(dr) / len(dr)
    sd = math.sqrt(sum((v - mu) ** 2 for v in dr) / len(dr))
    z = (dr[-1] - mu) / sd if sd > 0 else 0.0
    direction = 1 if z > cfg.z_star else (-1 if z < -cfg.z_star else 0)
...
def test_fixture_recomputes_to_expected():
    ...
    for g, e in zip(got, exp):
        ...
        assert abs(got[0]["rr25"] - (-4.5)) < 1e-9
        assert abs(got[4]["d_rr"] - 1.4) < 1e-9

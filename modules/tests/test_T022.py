"""Concrete sketch: T022 Queue-Imbalance Maker with Toxicity Cancel.
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["VPIN > 0.35 [example]", "inventory > max [example]", "L1 stale > 1 s [example]", "clock skew > 1 ms [example]"]
    stale_s = 1.0
    if stale_s >= 1.0:

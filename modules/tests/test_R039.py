...
CFG = {"pin_tol": 0.005, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    for _k in ['strike','call_oi','put_oi','spot']:
        _v = _parse_float(e[_k])
        if not math.isfinite(_v):
            raise ValueError('non-finite chain field')
    return 0.0


def _aggregate(events, vals, cfg):
    _ks=[float(_e['strike']) for _e in events]
    _cs=[float(_e['call_oi']) for _e in events]
    _ps=[float(_e['put_oi']) for _e in events]
    def _pi(_k):
        return sum(max(0.0,_k-_ki)*_pi_ for _ki,_pi_ in zip(_ks,_ps)) + sum(max(0.0,_ki-_k)*_ci for _ki,_ci in zip(_ks,_cs))
    return float(min(_ks, key=_pi))
...
def _dual_aggregate(events, cfg):
    ...
    _cs=[float(_e['call_oi']) for _e in events]
    _ps=[float(_e['put_oi']) for _e in events]
    _pis={_k: sum(max(0.0,_k-_ki)*_p for _ki,_p in zip(_ks,_ps)) + sum(max(0.0,_ki-_k)*_c for _ki,_c in zip(_ks,_cs)) for _k in _ks}
    return float(min(_pis, key=lambda _k: _pis[_k]))


def _state_of(value, e, cfg):
    _spot = float(e['spot'])
    _dist = abs(_spot - value)/_spot if _spot > 0 else float('inf')
    return 'PIN' if _dist <= cfg['pin_tol'] else 'WATCH'
...
def test_spot_handcheck():
    ...
    tape = _load('R039_tape.csv')
    got = detect(None, tape, CFG)
    assert got[4]['value'] == 100.0, got[4]['value']
    assert got[4]['state'] == 'PIN'

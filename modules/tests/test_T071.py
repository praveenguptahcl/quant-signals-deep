"""T071 — News-Novelty Reversal: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T071.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T071"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': True, 'z_long': 2.0, 'z_short': -2.0, 'conf_scale': 1.0, 'edge_mult': 2.5, 'time_stop': 3, 'exit_flip': False, 'k': 0.5, 'sizing': ('risk', 800.0, None), 'cooldown_bars': 2, 'bar_ns': 86400000000000, 'tif': 'DAY', 'venue': 'XNAS', 'side_class': 'mixed', 'adv_pct': 0.02, 'cost': {'spread_bps': 1.5, 'fee_bps': 1.2, 'borrow_bps': 0.0, 'impact_bps': 0.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S093")

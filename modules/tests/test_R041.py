"""R041 — Overnight gap vs intraday variance decomposition: acceptance tests (concrete sketch).
...
CFG = {"on_hi": 0.60, "on_lo": 0.40, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _o = _parse_float(e['r_on']); _i = _parse_float(e['r_id'])
    if not (math.isfinite(_o) and math.isfinite(_i)):
        raise ValueError('non-finite session return')
    return 0.0
...
def _aggregate(events, vals, cfg):
    if len(events) < 2:
        return float('nan')
    _os = [float(_e['r_on']) for _e in events]; _is = [float(_e['r_id']) for _e in events]
    _n = len(_os); _mo = sum(_os)/_n; _mi = sum(_is)/_n
    _vo = sum((_a-_mo)**2 for _a in _os)/(_n-1)
    _vi = sum((_a-_mi)**2 for _a in _is)/(_n-1)
    _cv = sum((_a-_mo)*(_b-_mi) for _a,_b in zip(_os,_is))/(_n-1)
    _vc = _vo + _vi + 2*_cv
    return _vo/_vc if _vc > 0 else float('nan')
...
def _dual_aggregate(events, cfg):
    if len(events) < 2:
        return float('nan')
    _os = [float(_e['r_on']) for _e in events]; _is = [float(_e['r_id']) for _e in events]
    _vo = statistics.variance(_os); _vi = statistics.variance(_is); _cv = statistics.covariance(_os, _is)
    _vc = _vo + _vi + 2*_cv
    return _vo/_vc if _vc > 0 else float('nan')

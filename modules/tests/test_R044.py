"""R044 — Institutional ownership concentration: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_R044.py -q` exits 0.
"""
import csv
import math
import os
...
CFG = {"conc": 0.020, "diffuse": 0.012, "tail_names": 490, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _ws = [_parse_float(e['w%d' % _i]) for _i in range(1,11)]
    if any(not math.isfinite(_w) for _w in _ws) or any(_w < 0 or _w >= 1 for _w in _ws) or sum(_ws) >= 1:
        raise ValueError('bad holder weights')
    return 0.0


def _aggregate(events, vals, cfg):
    _ws = [float(events[-1]['w%d' % _i]) for _i in range(1,11)]
    _s = sum(_ws); _tn = float(cfg['tail_names'])
    _t = (1.0-_s)/_tn
    return sum(_w*_w for _w in _ws) + _tn*_t*_t


def _dual_aggregate(events, cfg):
    _ws = [float(_x) for _x in [events[-1]['w%d' % _i] for _i in range(1,11)]]
    _s = 0.0
    for _w in _ws:
        _s += _w
    _t = (1.0-_s)/float(cfg['tail_names'])
    return sum(map(lambda _w: _w**2, _ws)) + float(cfg['tail_names'])*_t**2


def _state_of(value, e, cfg):
    if value >= cfg['conc']:
        return 'CONCENTRATED'
    if value < cfg['diffuse']:
        return 'DIFFUSE'
    return 'MODERATE'
...
def test_spot_handcheck():
    ...
    tape = _load('R044_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[0]['value'] - 0.0224399) <= 1e-6, got[0]['value']
    assert got[0]['state'] == 'CONCENTRATED'
    assert abs(got[1]['value'] - 0.0175894) <= 1e-6, got[1]['value']
    assert got[1]['state'] == 'MODERATE'

"""R034 — Energy / commodity shock regime: acceptance tests (concrete sketch).
...
CFG = {"H": 20, "z_shock": 2.0, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _pn = _parse_float(e['P_now']); _pt = _parse_float(e['P_then'])
    _mu = _parse_float(e['mu_R']); _sg = _parse_float(e['sigma_R'])
    if not all(math.isfinite(_v) for _v in [_pn,_pt,_mu,_sg]) or _pn <= 0 or _pt <= 0 or _sg <= 0:
        raise ValueError('bad shock inputs')  # F1/F2
    return (math.log(_pn/_pt) - _mu) / _sg
...
def _dual_aggregate(events, cfg):
    _pn = float(events[-1]['P_now']); _pt = float(events[-1]['P_then'])
    _mu = float(events[-1]['mu_R']); _sg = float(events[-1]['sigma_R'])
    return (math.log(_pn) - math.log(_pt) - _mu) / _sg


def _state_of(value, e, cfg):
    if abs(value) > cfg['z_shock']:
        return 'SHOCK_UP' if value > 0 else 'SHOCK_DOWN'
    return 'NORMAL'

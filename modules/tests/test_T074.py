"""T074 — Short-Interest Squeeze Rider: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T074.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T074"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 20.0, 'z_short': 99.0, 'conf_scale': 1.0, 'edge_mult': 0.8, 'time_stop': 7, 'exit_flip': False, 'k': 0.5, 'sizing': ('risk', 500.0, None), 'cooldown_bars': 10, 'bar_ns': 86400000000000, 'tif': 'DAY', 'venue': 'XNAS', 'side_class': 'mixed', 'adv_pct': 0.05, 'cost': {'spread_bps': 4.0, 'fee_bps': 0.4, 'borrow_bps': 0.0, 'impact_bps': 2.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S098")
...
def test_5_invalid_input_yields_unknown():
    ...
    bad[3]["sig"] = float("nan")
    _, mstate = emit_intents(bad, CFG, SID, "S098")
    assert mstate == "UNKNOWN"
    bad2 = [dict(r) for r in rows]
    bad2[4]["close"] = -1.0
    _, mstate2 = emit_intents(bad2, CFG, SID, "S098")
    assert mstate2 == "UNKNOWN"
    bad3 = [dict(r) for r in rows]
    bad3[5]["event_ts"] = bad3[4]["event_ts"]  # non-monotonic: not a later event
    _, mstate3 = emit_intents(bad3, CFG, SID, "S098")
    assert mstate3 == "UNKNOWN"

def test_6_handcheck_literals():
    rows, (tickets, mstate) = _replay()
    t = tickets[0]
    assert t["ticket_id"] == "T-T074-2-0"
    assert int(t["qty"]) == 666
    assert _close(float(t["cost_bps"]), 6.4)
    assert _close(float(t["cost_usd"]), 21.316859)
    assert _close(float(t["edge_bps"]), 20.0)

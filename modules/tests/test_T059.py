"""T059 — News-Sentiment First-Minute Momentum: acceptance tests (concrete sketch).
...
SID = "T059"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 0.6, 'z_short': -0.6, 'conf_scale': 1.0, 'edge_mult': 12.0, 'time_stop': 3, 'exit_flip': False, 'k': 0.5, 'sizing': ('fixed', 1000), 'cooldown_bars': 10, 'bar_ns': 1000000000, 'tif': 'IOC', 'venue': 'XNAS', 'side_class': 'taker', 'adv_pct': 0.05, 'cost': {'spread_bps': 2.5, 'fee_bps': 0.4, 'borrow_bps': 0.0, 'impact_bps': 1.0}}
K = 0.5
...
def test_6_handcheck_literals():
    rows, (tickets, mstate) = _replay()
    t = tickets[0]
    assert t["ticket_id"] == "T-T059-2-0"
    assert int(t["qty"]) == 1000
    assert _close(float(t["cost_bps"]), 3.9)
    assert _close(float(t["cost_usd"]), 97.522308)
    assert _close(float(t["edge_bps"]), 8.64)

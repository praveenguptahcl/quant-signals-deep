...
NS = 1_000_000_000
STATE_LABELS = ['mr', 'indeterminate', 'momentum', 'undefined', 'warming', 'missing', 'invalid']
BOUNDS = (0.0, 4.0)  # mathematical bounds of the indicator value (F2)
...
# ============================ R009 ============================
R009_RETS = [0.010,-0.008,0.006,-0.005,0.007,-0.006,0.004,-0.005,0.008,-0.007,
             0.005,-0.004,0.009,-0.008,0.006,-0.005,0.007,-0.006,0.003,-0.004,
             0.008,-0.006,0.005,-0.007,0.006,-0.005,0.004,-0.003,0.005,-0.004]
...
def _vr2(rets):
    """Lo-MacKinlay VR(2), overlapping estimator."""
    n = len(rets)
    if n < 8:
        return float("nan")
    mu = _mean(rets)
...
def _r009_state_vr(v):
    if not math.isfinite(v):
        return "undefined"
    return "mr" if v < 0.85 else ("momentum" if v > 1.15 else "indeterminate")

def _r009_state_ac(a):
    if not math.isfinite(a):
        return "undefined"
    return "mr" if a < -0.05 else ("momentum" if a > 0.05 else "indeterminate")

def primary_indicator_r009(rows, cfg):
    rows = list(rows)
    if not rows:
    ...
    v = _vr2(rets)
    return {"value": v, "state": _r009_state_vr(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r009(rows, cfg):
    rows = list(rows)
    if len(rows) < 12:
        return {"value": float("nan"), "state": "warming", "computed_at": rows[-1]["event_ts"] if rows else 0}
    rets = [math.log(rows[i]["c"] / rows[i - 1]["c"]) for i in range(1, len(rows))]
    a = _ac1(rets)
    return {"value": a, "state": _r009_state_ac(a), "computed_at": rows[-1]["event_ts"]}
...
primary_indicator = primary_indicator_r009
second_estimator = second_estimator_r009
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R009", "out_of_bounds", v, ESTIMATOR_VERSION,

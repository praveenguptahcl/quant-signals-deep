...
NS = 1_000_000_000
STATE_LABELS = ['hike_biased', 'on_hold', 'cut_biased', 'undefined', 'warming', 'missing', 'invalid']
...
# ============================ R024 ============================
# RB-list: R024 Interest-rate level/direction regime. Grok RB3: policy-path from
# Fed-funds futures: P(+25bp) = (implied EFFR_eom - starting EFFR)/0.25;
# worked example 68% -> hike_biased.
R024_EFFR_START = 4.33
R024_IMPL = [4.35, 4.38, 4.40, 4.42, 4.44, 4.46, 4.47, 4.48, 4.49, 4.50]

def r024_tape():
    hdr = ["bar", "event_ts", "asof_ts", "effr_start", "effr_implied"]
    rows = []
    for i, ei in enumerate(R024_IMPL, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS,
                     "effr_start": R024_EFFR_START, "effr_implied": ei})
    return hdr, rows
...
def _r024_state(p):
    if not math.isfinite(p):
        return "undefined"
    return "hike_biased" if p > 0.6 else ("cut_biased" if p < -0.2 else "on_hold")

def primary_indicator_r024(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("effr_implied" not in r or "effr_start" not in r or not (0 <= r["effr_implied"] <= 20 and 0 <= r["effr_start"] <= 20)
           for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    p = _r024_prob(rows[-1])
    return {"value": p, "state": _r024_state(p), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r024(rows, cfg):
    rows = list(rows)
    r = rows[-1] if rows else None
    if r is None:
        return {"value": float("nan"), "state": "undefined", "computed_at": 0}
    # independent: probability from the 3-obs average implied path
    m = _mean([x["effr_implied"] for x in rows[-3:]])
    p = (m - r["effr_start"]) / 0.25
    return {"value": p, "state": "n/a", "computed_at": r["event_ts"]}

F2_POISON_R024 = '''
F2_POISON_TAPE = [
    {"bar": 1, "event_ts": 1000, "asof_ts": 1001, "effr_start": 4.33, "effr_implied": 4.75},
]  # p = (4.75-4.33)/0.25 = 1.68 > 1.5 bound -> F2
'''


primary_indicator = primary_indicator_r024
second_estimator = second_estimator_r024
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if r["module_state"] != "OK":
        return RegimeState("R024", r["state"], r["value"], ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R024", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

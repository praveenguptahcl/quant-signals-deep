"""T052 — AR/ARMA Innovation Trader: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T052.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T052"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': True, 'z_long': 2.0, 'z_short': -2.0, 'conf_scale': 4.0, 'edge_mult': 1.5, 'time_stop': 5, 'exit_flip': True, 'k': 0.5, 'sizing': ('fixed', 800), 'cooldown_bars': 1, 'bar_ns': 60000000000, 'tif': 'IOC', 'venue': 'XNAS', 'side_class': 'taker', 'adv_pct': 0.02, 'cost': {'spread_bps': 0.8, 'fee_bps': 0.28, 'borrow_bps': 0.0, 'impact_bps': 0.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S078")

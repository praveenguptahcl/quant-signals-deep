"""R027 — Growth-surprise regime: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_R027.py -q` exits 0.
"""
import csv
import math
import os
...
CFG = {"lam": 0.02222222222222222, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    z = _parse_float(e['z_i'])
    if not math.isfinite(z):
        raise ValueError('non-finite z_i')  # F1/F2
    return z
...
def _aggregate(events, vals, cfg):
    tot = 0.0
    for e in events:
        tot += float(e['weight']) * math.exp(-cfg['lam'] * float(e['days_ago'])) * float(e['z_i'])
    return tot


def _dual_aggregate(events, cfg):
    terms = [float(e['weight']) * math.exp(-cfg['lam'] * float(e['days_ago'])) * float(e['z_i']) for e in events]
...
def test_spot_handcheck():
    ...
    tape = _load('R027_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[2]['value'] - 0.5634) <= 1e-3, got[2]['value']
    assert got[2]['state'] == 'UPSIDE'

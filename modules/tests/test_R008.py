...
NS = 1_000_000_000
STATE_LABELS = ['squeeze', 'normal', 'expansion', 'undefined', 'warming', 'missing', 'invalid']
...
# ============================ R008 ============================
R008_C = [100.0,100.4,99.8,100.2,100.6,100.1,99.9,100.3,100.5,100.2,
          99.7,100.1,100.4,100.0,99.8,100.2,100.3,100.1,99.9,100.2,
          100.0,100.1,99.9,100.2,100.1]

def r008_tape():
    hdr = ["bar", "event_ts", "asof_ts", "c"]
    rows = []
    for i, c in enumerate(R008_C, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS, "c": c})
    return hdr, rows

def _bb_bw(cs, n=20, k=2.0):
    if len(cs) < n:
        return float("nan")
    w = cs[-n:]
    mu = _mean(w)
    sd = _stdev(w, ddof=1)
    return (2 * k * sd) / mu if mu > 0 else float("nan")

def _keltner_bw(cs, n=20, m=2.0):
    if len(cs) < n + 1:
        return float("nan")
    ema = _mean(cs[:n])
    alpha = 2 / (n + 1)
    ...
    atr = _mean(trs[-n:])
    return (2 * m * atr) / ema if ema > 0 else float("nan")

def _r008_state(p):
    if not math.isfinite(p):
        return "undefined"
    return "squeeze" if p < 0.10 else ("expansion" if p > 0.90 else "normal")

def primary_indicator_r008(rows, cfg):
    rows = list(rows)
    if not rows:
    ...
    hist = [_bb_bw(cs[:j], 20, 2.0) for j in range(20, len(cs) + 1)]
    bw = hist[-1]
    p = _pct_rank(bw, hist[:-1])
    return {"value": bw, "state": _r008_state(p), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic", "pct": p}

def second_estimator_r008(rows, cfg):
    rows = list(rows)
    cs = [r["c"] for r in rows]
    if len(cs) < 24:
        return {"value": float("nan"), "state": "warming", "computed_at": rows[-1]["event_ts"] if rows else 0}
    hist = [_keltner_bw(cs[:j], 20, 2.0) for j in range(20, len(cs) + 1)]
    kw = hist[-1]
    p = _pct_rank(kw, hist[:-1])
    return {"value": kw, "state": _r008_state(p), "computed_at": rows[-1]["event_ts"]}

F2_POISON_R008 = '''
F2_POISON_TAPE = (
    [{"bar": i + 1, "event_ts": 1000 + i, "asof_ts": 1001 + i, "c": 100.0} for i in range(23)]
    + [{"bar": 24, "event_ts": 1023, "asof_ts": 1024, "c": 10000.0}]
)  # one huge outlier -> BW >> 5 -> F2
'''


primary_indicator = primary_indicator_r008
second_estimator = second_estimator_r008
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R008", "out_of_bounds", v, ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R008", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

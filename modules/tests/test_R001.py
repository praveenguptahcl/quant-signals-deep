...
BOUNDS = (0.0, 5.0)  # mathematical bounds of the indicator value (F2)
...
def r001_tape():
    ohlc = [
        (100.0, 101.2, 99.1, 100.4), (100.4, 101.5, 99.6, 100.9),
        (100.9, 101.8, 100.0, 101.2), (101.2, 102.1, 100.3, 101.6),
        (101.6, 102.4, 100.8, 102.0), (102.0, 103.8, 100.2, 101.5),
        (101.5, 103.5, 99.8, 102.8), (102.8, 104.9, 100.9, 103.6),
        (103.6, 105.8, 101.7, 104.2), (104.2, 106.5, 102.0, 105.1),
    ]
    hdr = ["bar", "event_ts", "asof_ts", "o", "h", "l", "c"]
    rows = []
    for i, (o, h, l, c) in enumerate(ohlc, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 60 * NS,
                     "o": o, "h": h, "l": l, "c": c})
    return hdr, rows

def primary_indicator_r001(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN",
                "computed_at": 0, "vintage": "synthetic"}
    for r in rows:
        if not all(k in r for k in ("o", "h", "l", "c")) or not (r["h"] > r["l"] > 0 and r["o"] > 0 and r["c"] > 0):
            return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                    "computed_at": r.get("event_ts", 0), "vintage": "synthetic"}
    c2 = 2 * math.log(2) - 1  # 0.386294... [documented] Garman-Klass constant
    gk = [0.5 * math.log(r["h"] / r["l"]) ** 2 - c2 * math.log(r["c"] / r["o"]) ** 2 for r in rows]
    W = 5  # [default]
    if len(rows) < W:
        return {"value": float("nan"), "state": "warming", "module_state": "DEGRADED",
                "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}
    hist = []
    for j in range(W, len(rows) + 1):
        hist.append(math.sqrt(_mean(gk[j - W:j]) * 252))  # annualized [documented] 252
    rv = hist[-1]
    p = _pct_rank(rv, hist)
    state = "complacent" if p < 0.2 else ("normal" if p < 0.8 else ("elevated" if p <= 0.95 else "extreme"))
    return {"value": rv, "state": state, "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic", "percentile": p}

def second_estimator_r001(rows, cfg):
    rows = list(rows)
    pk = [math.log(r["h"] / r["l"]) ** 2 / (4 * math.log(2)) for r in rows]  # Parkinson 1980 [documented]
    W = 5
    rv = math.sqrt(_mean(pk[-W:]) * 252) if len(rows) >= W else float("nan")
    return {"value": rv, "state": "n/a", "computed_at": rows[-1]["event_ts"] if rows else 0}
...
primary_indicator = primary_indicator_r001
second_estimator = second_estimator_r001

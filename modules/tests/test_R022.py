...
NS = 1_000_000_000
STATE_LABELS = ['normal', 'elevated', 'squeeze_candidate', 'hard_to_borrow', 'undefined', 'warming', 'missing', 'invalid']
BOUNDS = (0.0, 500.0)  # mathematical bounds of the indicator value (F2)
...
# ============================ R022 ============================
# RB-list: R022 Hard-to-borrow / short-squeeze regime. Grok RB3 worked example:
# borrow fee 12% ann, utilization 93%, SI 12m, ADV 2m -> DTC 6.0 -> squeeze_candidate/HTB.
R022_SECS = [  # (fee_ann, util, si, adv)
    (0.08, 0.80, 8e6, 2e6), (0.10, 0.85, 9e6, 2e6), (0.11, 0.90, 10e6, 2e6),
    (0.12, 0.93, 11e6, 2e6), (0.12, 0.93, 12e6, 2e6),
]

def r022_tape():
    hdr = ["bar", "event_ts", "asof_ts", "fee_ann", "util", "si", "adv"]
    rows = []
    for i, (f, u, si, ad) in enumerate(R022_SECS, 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS,
                     "fee_ann": f, "util": u, "si": si, "adv": ad})
    return hdr, rows

def _dtc(r):
    return r["si"] / r["adv"]

def _r022_state(fee, util, dtc):
    if fee > 0.15 or util > 0.97 or dtc > 10:
        return "hard_to_borrow"
    if fee > 0.10 or util > 0.90 or dtc > 6:
        return "squeeze_candidate"
    if fee > 0.05 or util > 0.75 or dtc > 4:
        return "elevated"
    return "normal"

def primary_indicator_r022(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("fee_ann" not in r or "util" not in r or "si" not in r or "adv" not in r
           or not (r["util"] > 0 and r["adv"] > 0 and r["fee_ann"] >= 0) for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    r = rows[-1]
    v = _dtc(r)  # primary scalar: days-to-cover
    return {"value": v, "state": _r022_state(r["fee_ann"], r["util"], v), "module_state": "OK",
            "computed_at": r["event_ts"], "vintage": "synthetic"}

def second_estimator_r022(rows, cfg):
    rows = list(rows)
    r = rows[-1] if rows else None
    if r is None:
        return {"value": float("nan"), "state": "undefined", "computed_at": 0}
    v = _dtc(r)  # independent: median-DTC over the last up-to-3 observations
    ds = sorted(_dtc(x) for x in rows[-3:])
    m = ds[len(ds) // 2]
    return {"value": m, "state": "n/a", "computed_at": r["event_ts"]}
...
primary_indicator = primary_indicator_r022
second_estimator = second_estimator_r022
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R022", "out_of_bounds", v, ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R022", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

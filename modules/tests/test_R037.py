"""R037 — Month/quarter-end rebalancing flows: acceptance tests (concrete sketch).
...
CFG = {"w_star": 0.60, "flow_band": 0.01, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    _re = _parse_float(e['R_e']); _rb = _parse_float(e['R_b'])
    if not (math.isfinite(_re) and math.isfinite(_rb)):
...
def _aggregate(events, vals, cfg):
    _w = cfg['w_star']
    _re = float(events[-1]['R_e']); _rb = float(events[-1]['R_b'])
    _we = _w*(1.0+_re)/(_w*(1.0+_re)+(1.0-_w)*(1.0+_rb))
    return _we - _w
...
def _dual_aggregate(events, cfg):
    _w = cfg['w_star']
    _re = float(events[-1]['R_e']); _rb = float(events[-1]['R_b'])
    _num = _w + _w*_re; _den = _num + (1.0-_w)*(1.0+_rb)
    return _num/_den - _w


def _state_of(value, e, cfg):
    if value > cfg['flow_band']:
        return 'SELL_EQUITY'
    if value < -cfg['flow_band']:
        return 'BUY_EQUITY'
    return 'NEUTRAL'
...
def _bounds_ok(value, cfg):
    return math.isfinite(value) and abs(value) < 1.0
...
def test_spot_handcheck():
    ...
    tape = _load('R037_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[0]['value'] - 0.022642) <= 1e-6, got[0]['value']
    assert got[0]['state'] == 'SELL_EQUITY'
    assert abs(got[1]['value'] - (-0.017178)) <= 1e-6, got[1]['value']
    assert got[1]['state'] == 'BUY_EQUITY'

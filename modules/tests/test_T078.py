"""Acceptance tests for T078 — Earnings-Drift Intraday Leg.
...
Run: python3 -m pytest modules/tests/test_T078.py -q   (from repo root)
"""
import csv
import hashlib
import uuid
...
TAPE = FIX / "T078_tape.csv"
EXPECTED = FIX / "T078_expected.csv"

GATE_CFG = [('>=', 1.5), ('>=', 0.5), ('>=', 1.0)]   # [(op, threshold)] for g1, g2, g3
GATE_NAMES = ['sue_abs', 'leg_ratio', 'news_agree']
...
def test_cost_gate_predicate():
    ...
    assert cost > 0
    assert cost <= k * 114.0      # large edge -> gate passes
    assert not (cost <= k * 3.8000000000000003)  # tiny edge -> gate blocks

...
# ============================ R012 ============================
R012_ABSRET = [0.008, 0.012, 0.006, 0.015, 0.009, 0.011, 0.007, 0.013, 0.010, 0.014]
R012_VOLD = [5.0e6, 4.2e6, 6.1e6, 3.8e6, 5.5e6, 4.9e6, 5.8e6, 4.0e6, 5.2e6, 4.5e6]
R012_O = [100.0, 100.8, 101.2, 100.5, 101.0, 100.7, 101.1, 100.9, 101.3, 100.6]
R012_C = [100.8, 99.9, 101.8, 99.5, 101.9, 99.8, 101.8, 99.6, 102.3, 99.2]
HIST_AMIHUD = [0.15, 0.18, 0.20, 0.22, 0.25, 0.28, 0.30, 0.33, 0.35, 0.38,
               0.40, 0.45, 0.50, 0.55, 0.60, 0.70, 0.80, 0.90, 1.00, 1.20]  # [example] synthetic trailing history

def r012_tape():
    hdr = ["bar", "event_ts", "asof_ts", "absret", "vold", "o", "c"]
    rows = []
    for i, (ar, vd, o, c) in enumerate(zip(R012_ABSRET, R012_VOLD, R012_O, R012_C), 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS,
                     "absret": ar, "vold": vd, "o": o, "c": c})
    return hdr, rows

def _amihud(absret, vold, scale=1e6):
    vals = [a / v * scale for a, v in zip(absret, vold) if v > 0]
    return _mean(vals) if vals else float("nan")

def _r012_state(v):
    if not math.isfinite(v):
        return "undefined"
    p = _pct_rank(v, HIST_AMIHUD)
    return "normal" if p < 0.60 else ("illiquid" if p < 0.90 else "extreme")

def primary_indicator_r012(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("absret" not in r or "vold" not in r or r["absret"] < 0 or r["vold"] <= 0 for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    v = _amihud([r["absret"] for r in rows], [r["vold"] for r in rows])
    p = _pct_rank(v, HIST_AMIHUD)
    return {"value": v, "state": _r012_state(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic", "pct": p}

def second_estimator_r012(rows, cfg):
    rows = list(rows)
    oc = [abs(math.log(r["c"] / r["o"])) for r in rows if r["o"] > 0 and r["c"] > 0]
    v = _amihud(oc, [r["vold"] for r in rows])
...
F2_POISON_TAPE = [
    ...
    for i in range(10)
]  # ILLIQ = 200/100*1e6 = 2e6 > 1e6 bound -> F2
'''


primary_indicator = primary_indicator_r012
second_estimator = second_estimator_r012

"""T055 — DeepLOB + Feature-Stack Classifier: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T055.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T055"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 0.58, 'z_short': -0.58, 'conf_scale': 1.0, 'edge_mult': 2.0, 'time_stop': 10, 'exit_flip': True, 'k': 0.5, 'sizing': ('conf', 100, 0.58), 'cooldown_bars': 5, 'bar_ns': 1000000000, 'tif': 'IOC', 'venue': 'XNAS', 'side_class': 'taker', 'adv_pct': 0.05, 'cost': {'spread_bps': 0.24, 'fee_bps': 0.0, 'borrow_bps': 0.0, 'impact_bps': 0.0}, 'lot': 1, 'min_lot': 1}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S082")
...
def test_5_invalid_input_yields_unknown():
    ...
    bad[3]["sig"] = float("nan")
    _, mstate = emit_intents(bad, CFG, SID, "S082")
    assert mstate == "UNKNOWN"
    bad2 = [dict(r) for r in rows]
    bad2[4]["close"] = -1.0
    _, mstate2 = emit_intents(bad2, CFG, SID, "S082")
    assert mstate2 == "UNKNOWN"
    bad3 = [dict(r) for r in rows]
    bad3[5]["event_ts"] = bad3[4]["event_ts"]  # non-monotonic: not a later event
    _, mstate3 = emit_intents(bad3, CFG, SID, "S082")
    assert mstate3 == "UNKNOWN"

def test_6_handcheck_literals():
    rows, (tickets, mstate) = _replay()
    t = tickets[0]
    assert t["ticket_id"] == "T-T055-3-0"
    assert int(t["qty"]) == 19
    assert _close(float(t["cost_bps"]), 0.24)
    assert _close(float(t["cost_usd"]), 0.113986)
    assert _close(float(t["edge_bps"]), 1.32)

#!/usr/bin/env python3
"""Acceptance tests for S023 — Gap-and-go (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Gap fixture: open=101.50; holds if low >= 101.50; confirmed on bar 4."""
    open_px = 101.50
    out = []
    for r in trows:
        bar, low = int(r[0]), float(r[1])
        holds = 1 if low >= open_px else 0
        confirmed = 1 if (bar == 4 and holds) else 0
        out.append([bar, holds, confirmed])
    return out
...
def test_06_gap_confirmed():
    # Bar 4 confirms the LONG; bar 5 breaks the hold (low 100.80 < 101.50).
    eheader, erows = load(EXP)
    got = {e[0]: (int(e[1]), int(e[2])) for e in erows}
    assert got["4"] == (1, 1) and got["5"] == (0, 0) and got["1"] == (0, 0)

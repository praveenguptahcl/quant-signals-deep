"""T062 — Streak Runner with Exhaustion Flip: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T062.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T062"
TOL = 1e-6
CFG = {'mode': 'streak', 'fade': False, 'z_long': 3.0, 'z_short': -3.0, 'conf_scale': 1.0, 'edge_mult': 2.5, 'time_stop': 30, 'exit_flip': True, 'k': 0.5, 'sizing': ('risk', 300.0, None), 'cooldown_bars': 5, 'bar_ns': 60000000000, 'tif': 'DAY', 'venue': 'XNAS', 'side_class': 'taker', 'adv_pct': 0.05, 'cost': {'spread_bps': 2.5, 'fee_bps': 0.4, 'borrow_bps': 0.0, 'impact_bps': 0.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S031")

#!/usr/bin/env python3
"""Acceptance tests for S020 — Retail and odd-lot imbalance (sketch-level, concrete)."""
import csv
import math
import pathlib
...
def recompute(trows):
    """Retail fixture: per-print signed volume, cumulative net and total."""
    out = []
    cnet = ctot = 0.0
    for r in trows:
        sv = float(r[1])
...
def test_06_retail_imbalance():
    # RI = 210/370 = 0.5676 (retail-bullish); proxy share assumed adequate in fixture.
    eheader, erows = load(EXP)
    net, tot = float(erows[-1][2]), float(erows[-1][3])
    ri = net / tot
    assert abs(ri - 0.5676) < 1e-4
    assert ri > 0


def test_02_signal_vector_shape():
    sig = signal_stub(2.5, 2.0, computed_at=1700000000000000000)
...
def test_03_no_signal_bar_fills():
    ...
    sig = signal_stub(2.5, 2.0, computed_at=computed_at)

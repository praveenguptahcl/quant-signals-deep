"""T060 — Identified-News Drift Portfolio: acceptance tests (concrete sketch).

Real imports, fixture load, real assertions. Not a production harness.
Definition of done: `python3 -m pytest modules/tests/test_T060.py -q` exits 0.
"""
import csv
import math
import os
...
SID = "T060"
TOL = 1e-6
CFG = {'mode': 'z', 'fade': False, 'z_long': 0.7, 'z_short': -0.7, 'conf_scale': 1.0, 'edge_mult': 8.0, 'time_stop': 3, 'exit_flip': False, 'k': 0.5, 'sizing': ('risk', 1000.0, None), 'cooldown_bars': 1, 'bar_ns': 86400000000000, 'tif': 'DAY', 'venue': 'XNAS', 'side_class': 'mixed', 'adv_pct': 0.02, 'cost': {'spread_bps': 1.5, 'fee_bps': 1.2, 'borrow_bps': 0.0, 'impact_bps': 0.0}}
K = 0.5
...
def _replay():
    ...
    return rows, emit_intents(rows, CFG, SID, "S092")

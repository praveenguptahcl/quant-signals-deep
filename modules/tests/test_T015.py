"""Concrete sketch: T015 First-Half-Hour to Close Continuation.
...
def test_kill_switch_trips_and_rearms():
    ...
    state = "ARMED"
    kill_conditions = ["15:00 bar stale > 3 min [example]", "clock skew > 50 ms [example]", "HMM inputs stale [example]"]
    stale_s = 300.0
    if stale_s >= 180.0:

"""Acceptance tests for R021 - Retail-flow dominance regime.
...
NS = 1_000_000_000
STATE_LABELS = ['retail_dominated', 'mixed', 'baseline', 'undefined', 'warming', 'missing', 'invalid']
...
# ============================ R021 ============================
# RB-list: R021 Retail-flow dominance regime. Grok RB3 worked example:
# day-10 odd-lot 210 / all 380 = 0.5526 (baseline/off, NOT retail-dominated).
R021_ROWS = [  # (day, n_odd, n_all)
    (1, 140, 400), (2, 150, 420), (3, 145, 410), (4, 155, 430), (5, 148, 415),
    (6, 160, 440), (7, 152, 425), (8, 158, 432), (9, 150, 420), (10, 210, 380),
]

def r021_tape():
    hdr = ["bar", "event_ts", "asof_ts", "n_odd", "n_all"]
    rows = []
    for d, o, a in R021_ROWS:
        ts = TS0 + (d - 1) * DAY
        rows.append({"bar": d, "event_ts": ts, "asof_ts": ts + DAY * NS, "n_odd": o, "n_all": a})
    return hdr, rows

def _r021_state(frac):
    if not math.isfinite(frac):
        return "undefined"
    return "retail_dominated" if frac > 0.65 else ("baseline" if frac < 0.5 else "mixed")

def primary_indicator_r021(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("n_odd" not in r or "n_all" not in r or not r["n_all"] > 0 or r["n_odd"] < 0 or r["n_odd"] > r["n_all"]
           for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    r = rows[-1]
    v = r["n_odd"] / r["n_all"]
    return {"value": v, "state": _r021_state(v), "module_state": "OK",
            "computed_at": r["event_ts"], "vintage": "synthetic"}

def second_estimator_r021(rows, cfg):
    # count-share vs a volume-weighted complement: same-day volume share proxy
    rows = list(rows)
    r = rows[-1] if rows else None
    if r is None:
        return {"value": float("nan"), "state": "undefined", "computed_at": 0}
    v = r["n_odd"] / r["n_all"]
    return {"value": v, "state": "n/a", "computed_at": r["event_ts"]}

F2_POISON_R021 = '''
F2_POISON_TAPE = [
    {"bar": 1, "event_ts": 1000, "asof_ts": 1001, "n_odd": 1000, "n_all": 500},
]  # n_odd > n_all -> F1 invalid here
'''


primary_indicator = primary_indicator_r021
second_estimator = second_estimator_r021
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if r["module_state"] != "OK":
        return RegimeState("R021", r["state"], r["value"], ESTIMATOR_VERSION,
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R021", "out_of_bounds", v, ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R021", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

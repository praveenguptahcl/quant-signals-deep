"""R046 — Liquidity commonality: acceptance tests (concrete sketch).
...
CFG = {"common": 0.40, "frag": 0.15, "W": 63, "min_lag_bars": 1}
...
def _row_value(e, cfg):
    for _i in range(1,7):
        _v = _parse_float(e['a%d' % _i])
        if not math.isfinite(_v):
            raise ValueError('non-finite Amihud change')
    return 0.0
...
def _aggregate(events, vals, cfg):
    if len(events) < 2:
        return float('nan')
    _cols = [[float(_e['a%d' % (_i+1)]) for _e in events] for _i in range(6)]
    def _c(_x, _y):
        _n = len(_x); _mx = sum(_x)/_n; _my = sum(_y)/_n
        _cov = sum((_a-_mx)*(_b-_my) for _a,_b in zip(_x,_y))
        _vx = sum((_a-_mx)**2 for _a in _x); _vy = sum((_b-_my)**2 for _b in _y)
        return _cov / math.sqrt(_vx*_vy)
    _ps = [_c(_cols[_i], _cols[_j]) for _i in range(6) for _j in range(_i+1, 6)]
    return sum(_ps)/len(_ps)
...
def _dual_aggregate(events, cfg):
    if len(events) < 2:
        return float('nan')
    _cols = [[float(_e['a%d' % (_i+1)]) for _e in events] for _i in range(6)]
    _ps = [statistics.correlation(_cols[_i], _cols[_j]) for _i in range(6) for _j in range(_i+1, 6)]
    return sum(_ps)/len(_ps)
...
def test_spot_handcheck():
    ...
    tape = _load('R046_tape.csv')
    got = detect(None, tape, CFG)
    assert abs(got[4]['value'] - 0.9931) <= 1e-3, got[4]['value']
    assert got[4]['state'] == 'COMMON'

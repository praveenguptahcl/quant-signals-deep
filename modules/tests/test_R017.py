"""Acceptance tests for R017 - Funding-stress regime.
...
NS = 1_000_000_000
STATE_LABELS = ['normal', 'tight', 'stressed', 'undefined', 'warming', 'missing', 'invalid']
BOUNDS = (-100.0, 500.0)  # mathematical bounds of the indicator value (F2)
...
# ============================ R017 ============================
# RB-list: R017 Funding-stress regime = SOFR-OIS spread. Bands [documented]:
# <10 bps normal, 10-25 tight, >25 stressed. Grok RB2 example: SOFR 5.32,
# IORB 5.15 -> 17 bp (elevated/stress per illustrative bands).
R017_SOFR = [5.32, 5.35, 5.38, 5.42, 5.45, 5.48, 5.52, 5.55, 5.58, 5.60]
R017_OIS = [5.15] * 10
R017_IOER = [5.14] * 10

def r017_tape():
    hdr = ["bar", "event_ts", "asof_ts", "sofr", "ois", "ioer"]
    rows = []
    for i, (sf, oi, io) in enumerate(zip(R017_SOFR, R017_OIS, R017_IOER), 1):
        ts = TS0 + (i - 1) * DAY
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 3600 * NS,
                     "sofr": sf, "ois": oi, "ioer": io})
    return hdr, rows

def _r017_state(v):
    if not math.isfinite(v):
        return "undefined"
    return "normal" if v < 10 else ("tight" if v <= 25 else "stressed")

def primary_indicator_r017(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("sofr" not in r or "ois" not in r or not (0 <= r["ois"] <= 50 and 0 <= r["sofr"] <= 50) for r in rows):
        return {"value": float("nan"), "state": "invalid", "module_state": "UNKNOWN",
                "computed_at": rows[-1].get("event_ts", 0), "vintage": "synthetic"}
    v = _mean([(r["sofr"] - r["ois"]) * 100 for r in rows])
    return {"value": v, "state": _r017_state(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r017(rows, cfg):
    rows = list(rows)
    v = _mean([(r["sofr"] - r["ioer"]) * 100 for r in rows]) if rows else float("nan")
    return {"value": v, "state": "n/a", "computed_at": rows[-1]["event_ts"] if rows else 0}

F2_POISON_R017 = '''
F2_POISON_TAPE = [
    {"bar": 1, "event_ts": 1000, "asof_ts": 1001, "sofr": 15.15, "ois": 5.15, "ioer": 5.15}
]  # SOFR-OIS = 1000 bps > 500 bound -> F2
'''


primary_indicator = primary_indicator_r017
second_estimator = second_estimator_r017
...
def detect(rows, cfg, now_ns=None, select_periods=False, preregistered=False):
    ...
    if r["module_state"] != "OK":
        return RegimeState("R017", r["state"], r["value"], ESTIMATOR_VERSION,
    ...
    if not (math.isfinite(v) and lo <= v <= hi):
        return RegimeState("R017", "out_of_bounds", v, ESTIMATOR_VERSION,
    ...
    if not agree:
        return RegimeState("R017", r["state"], v, ESTIMATOR_VERSION,
                           r.get("vintage", "synthetic"), r["computed_at"], "UNKNOWN")

...
NS = 1_000_000_000
STATE_LABELS = ['clean', 'elevated', 'extreme', 'undefined', 'warming', 'missing', 'invalid']
...
def r014_tape():
    hdr = ["bar", "event_ts", "asof_ts", "dp", "buy_frac", "v", "sig_dp"]
    rows = []
    for i, (dp, bf) in enumerate(zip(R014_DP, R014_BUYF), 1):
        ts = TS0 + (i - 1) * 60
        rows.append({"bar": i, "event_ts": ts, "asof_ts": ts + 60 * NS,
                     "dp": dp, "buy_frac": bf, "v": R014_V, "sig_dp": R014_SIG})
    return hdr, rows
...
def _r014_state(v):
    if not math.isfinite(v):
        return "undefined"
    return "extreme" if v > 0.7 else ("elevated" if v >= 0.3 else "clean")

def primary_indicator_r014(rows, cfg):
    rows = list(rows)
    if not rows:
        return {"value": float("nan"), "state": "missing", "module_state": "UNKNOWN", "computed_at": 0, "vintage": "synthetic"}
    if any("dp" not in r or "buy_frac" not in r or "v" not in r or "sig_dp" not in r for r in rows):
    ...
    v = _vpin_bvc(rows)
    return {"value": v, "state": _r014_state(v), "module_state": "OK",
            "computed_at": rows[-1]["event_ts"], "vintage": "synthetic"}

def second_estimator_r014(rows, cfg):
    rows = list(rows)
    return {"value": _vpin_tick(rows), "state": "n/a", "computed_at": rows[-1]["event_ts"] if rows else 0}
...
primary_indicator = primary_indicator_r014
second_estimator = second_estimator_r014

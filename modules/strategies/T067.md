...
| Row | Value |
|---|---|
| Module | T067 — Anchored-VWAP Event Trader, v1.0.0 |
...
| Data cost | Research `~$50–500 + news`/mo [unverified]; production `~$2,000–10,000`/mo [unverified] — bands indicative, verify before budgeting |
| latency_class | bar / 1-minute |
| risk_class | medium |
| Capacity | moderate [example] — event-driven, 1-min bars |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced + news feed — cheapest data source candidate [internal-est]; latency_class = bar / 1-minute; required_fields = 1-min OHLCV; news timestamps; price_band = `~$50–500 + news`/mo [unverified] (indicative — verify before budgeting); build_or_buy = **build the AVWAP on bought bars + news; the anchor rulebook is the product**.
...
risk_contract:
  ...
  - "news feed stale -> veto new anchors"
  - "daily loss <= -$1,200 -> TRIPPED"
  - "three failed reclaims -> halve size"
...
```
ENTER_LONG  := (novel news) AND (close > AVWAP for >= `2` bars [example])
               AND (expected_cost_bps(...) <= cost_gate_k * edge_bps)
ENTER_SHORT := mirror below AVWAP on negative news
FLAT        := otherwise
```
...
|---|---|---|---|---|
| Reclaim bars | `reclaim_bars` | `2` | [example] | high |
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        s = AVWAP deviation (price - AVWAP), signed                                            # data <= t only
        ...
        if state.position is None and state.module_state == OK:
            if ((novel news) and (beyond AVWAP >= 2 bars)) and cost_ok:
...
- Tape: `modules/fixtures/T067_tape.csv` — `# TYPE: validation-run`;
  `12` bars [example], all numbers synthetic, seed `10067` [example];
...
- Acceptance tests (`modules/tests/test_T067.py`, `6` tests):
...
**Where the example is optimistic:** the fixture encodes AVWAP deviations as signal events with the news anchor folded into the entry threshold; production faces anchor ambiguity and post-news slippage
...
**Data rights row** (Appendix j v1.0.0): US equities 1-min bars + news via Polygon.io
...
Feasible on the M5 Max (Tier M, `20–60` h ≈ `$3k–15k` eng + data per notes/cost-model.md §5). Bottleneck is the anchor rulebook, not compute.
...
> Build module **T067 — Anchored-VWAP Event Trader** per `notes/module-template.md` v1.0.0
> and this file's §T0 contract.
> Cheapest path (§1.5): prototype `20–60` h [internal-est]; cheapest data
> candidate Polygon.io Stocks Advanced + news feed [internal-est]; compute pattern `per_bar`; skip

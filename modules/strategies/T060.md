...
sources:
  ...
  - "Boudoukh, J., Feldman, R., Kogan, S. & Richardson, M. (2013). Which News Moves Stock Prices? A Textual Analysis. NBER Working Paper No. 18725 (26 Jan 2013)"
unverified_sources: []
consumes:
...
changelog:
  ...
  - "1.0.1 2026-09-10: deep review — Boudoukh citation pinned to NBER WP 18725 (14 categories / 56 subcategories [documented]); MRN data model (MRN_STORY / MRN_TRNA / MRN_TRSI, FRAGMENT field [documented]); Config gained edge_mult/tif/venue/side_class/adv_pct/urgency/bar_ns; entry rule gained gate==1 guard; sizing became the 5-arg shares(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost); fill timing unified to open(t+1); regime-gate machine records; 2 new acceptance tests (gate block, sizing ADV cap); GROK-NEEDED questions added"
---
...
2. **Cheapest-source row:** min_tier = Refinitiv MRN (or equivalent) — cheapest data source candidate [internal-est]; latency_class = bar / daily; required_fields = story timestamp + language + entity tags, TRNA sentiment / novelty / relevance scores [documented], daily bars; price_band = `~$2,000–10,000`/mo [unverified] (indicative — verify before budgeting); build_or_buy = **buy the news feed; build the portfolio — drift capture is position management, not a model**. MRN data model: news arrives as UTF-8 JSON in the `FRAGMENT` field, gzip-compressed and fragmented across update messages [documented]; services `MRN_STORY` (real-time news), `MRN_TRNA` (news analytics), `MRN_TRSI` [documented] over Refinitiv Real-Time (RTDS / WebSocket API) [documented] — the prototype only needs the daily analytics slice, not the streaming stack.
...
| `edge_mult` | float | `8.0` | `1.0`–`20.0` | default |
| `tif` | str | `"DAY"` | {`DAY`, `MOO`, `MOC`} | default |
| `venue` | str | `"XNAS"` | {`XNAS`, `XNYS`} | default |
| `side_class` | str | `"mixed"` | {`taker`, `maker`, `mixed`} | default |
| `adv_pct` | float | `0.02` | `0.0`–`0.10` | default |
| `urgency` | str | `"normal"` | {`low`, `normal`, `high`} | default |
| `bar_ns` | int | `86400000000000` | — | fixed |
...
| TRNA sentiment / novelty / relevance scores (per-story JSON in the `FRAGMENT` field) | `sig` (story-defined score, Appendix b `value`) [example] |
| identified-story confirmation | `gate` ∈ {0, 1} (confirmation gate) [example] |
...
risk_contract:
  ...
  max_adverse_per_trade: `4`% [example]  # adverse excursion per trade cap; CI gate in §T2
  daily_loss_stop: `-5000` USD [example]          # [example]
  max_gross: `10`M USD [example]                # [example]
  kill_conditions:                        # any one trips -> TRIPPED
  - "news feed stale > 1 day [example] -> TRIPPED"
  - "daily loss <= -$5,000 [example] -> TRIPPED"
  - "book concentration > 20% in one story [example] -> TRIPPED"
...
def shares(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost):
    """Dollar-risk sizing per name; cost is checked by the cost gate, not the sizer [default]."""
    base = int(risk_budget_R // max(stop_distance, 1e-9))   # R_usd=1000 [default], stop_distance from stop_dist field [default]
    cap = int(ADV_cap) if ADV_cap else base                # ADV participation cap [example]
    # vol_estimate recorded for the decision log; the stop distance already
    # encodes the per-name risk budget in the reference build [default].
    # cost (bps, from expected_cost_bps) is recorded at sizing time; the
    # cost-gate predicate expected_cost_bps(...) <= k * edge_bps vetoes the
    # ticket, not the sizer [default].
...
| Max adverse per trade | `4`% [example] — CI gate: adverse excursion beyond stop vetoes re-entry |
...
| Aspect | Assumption |
|---|---|
| Latency | Daily; entries at/after the open of the next bar after identification [example] |
| Entries | diversified across stories; `≤30` names [example] |
| Exits | `3`-day hold or 4% adverse; exit intents are DAY-marketable [example] |
| TIF | `DAY` [default]; auction-aware MOO/MOC only in AUCTION state |
| Partial fills | Per Appendix c v1.0.0 FIFO |
| Adverse fill | Crowded exits on the same stories widen the exit spread [example] |

### Robustness

Identified-news filter: Boudoukh et al. (NBER WP 18725) show identified news days have >2× the volatility of no-news days while unidentified-news days look like no-news days [documented] — the operational identified taxonomy is their `14` categories / `56` subcategories [documented], approximated in the reference build by the TRNA story-identification scores [example]; extreme sentiment `|sent| ≥ 0.70` [example]; `3`-day hold [example] inside the `2`–`5`-day planning window [example]; no-news drift explicitly excluded (Chan 2003).
...
| Edge multiplier | `edge_mult` | `8.0` | [default] | high |
| Confirmation gate | `gate` | `1` | [example] | high |
| TIF | `tif` | `DAY` | [default] | low |
| Venue | `venue` | `XNAS` | [default] | low |
| Side class | `side_class` | `mixed` | [default] | medium |
...
[example]: enter in the direction of $s_t$ at the open of bar $t+1$ (per the
timing box — never on the signal bar), hold $3$ days [example] (inside the
$2$–$5$-day planning window [example]). Chan (2003): stocks with public news
exhibit post-news drift; no-news movers reverse — hence the identified-only
filter. Book of $\le 30$ names [example], dollar-risk sized at
$\$1{,}000$ per name [example].
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        ...
        assert t.event_ts > prev_event_ts else return UNKNOWN       # F2 monotonicity
        edge_bps = abs(s) * cfg.edge_mult                           # [default] — modeled per-trade edge in bps
        notional = shares(cfg.R_usd, stop_dist_t, vol_est_t, ADV_cap_t, cost_bps_t) * close_t
        cost = expected_cost_bps(notional, cfg.adv_pct, cfg.venue, cfg.side_class, cfg.urgency)
        cost_ok = expected_cost_bps(...) <= k * edge_bps
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
        if state.position is None and state.module_state == OK:
            if ((|s| >= cfg.sent_min) and identified and (r.gate == 1)
                    and (positions < cfg.max_pos)) and cost_ok:
            ...
            if ((bars_held >= cfg.hold_days) or (adverse > cfg.stop_pct)):
...
- Acceptance tests (`modules/tests/test_T060.py`, `8` tests):
  ...
  (qty / bps / $) match the generation run; `7.` `gate == 0` bars emit no
  tickets (confirmation-gate guard); `8.` 5-arg `shares()` sizing honors the
  ADV cap and handles `cost=None`.
...
| Boudoukh et al. (2013), NBER WP 18725 | Dow Jones Newswire, S&P 500, 2000–2009 | identified news days: volatility >2× no-news days; variance ratio `120`% higher [documented] | `BEFORE-COST` | supports the identified-only filter |
...
```yaml
regime_gates:
  - regime_id: PROV_EARNINGS_CLUSTER        # [unverified] — pending R001–R050 pass
    direction: degrades
    mechanism: "many names trigger on the same story; book concentrates > 20% in one story [example]"
    condition: "entries_on_same_story >= 6 [default]"
    action: halve
    min_lag: 1                               # bars [default]
    unknown_behavior: "halt new entries on the story until classified"
  - regime_id: PROV_DRIFT_COMPRESSION       # [unverified] — pending R001–R050 pass
    direction: degrades
    mechanism: "crowded exits widen exit spreads; the 2–5 day [example] window compresses"
    condition: "median adv_pct of held book > 0.05 [example]"
    action: halve
    min_lag: 5                               # bars [default]
    unknown_behavior: "flatten to OFF; do not re-enter unclassified"
  - regime_id: PROV_NEWS_DROUGHT            # [unverified] — pending R001–R050 pass
    direction: degrades
    mechanism: "no identified stories arrive; stale positions age out with no replacements"
    condition: "no entries for 10 bars [example]"
    action: pause_entry
    min_lag: 1                               # bars [default]
    unknown_behavior: "continue normal; never chase unidentified movers"
```
...
3. Boudoukh, J., Feldman, R., Kogan, S. & Richardson, M. (2013) [documented]. "Which News Moves Stock Prices? A Textual Analysis." NBER Working Paper No. 18725, posted 26 Jan 2013 [documented] — status: working paper (no journal version as of the last verified record; do not cite as published). Method: dictionary-based sentiment + phrase-level patterns + an event-identification platform classifying stories into `14` categories and `56` subcategories [documented]; over the 2000–2009 sample for all S&P 500 companies, the Dow Jones Newswire produced over `1.9` million stories but only about half were identified as relevant events [documented]. Findings: identified-news days show stock-level volatility over twice that of no-news days; variance ratios are `120`% higher on identified-news vs no-news days vs `20`% for unidentified [documented]; market-model R² `16`% vs `33`% on news vs no-news days [documented]; conditional on extreme moves, prices continue after identified news and reverse after no-news [documented].
...
1. **MRN/TRNA pricing.** What are the current list-price tiers (monthly bands) for Refinitiv/LSEG Machine Readable News (`MRN_STORY`) and Refinitiv News Analytics (`MRN_TRNA`) for a single-seat research account vs a production feed? The module's `~$2,000–10,000`/mo band [unverified] needs a real number or a quote-based confirmation.
2. **Fee schedule pin.** For the XNAS venue fee tier used by the reference build, what is the exact effective fee-schedule document title + version/effective date that should replace the `2026-09-01` [unverified] `fee_schedule_as_of` placeholder? (Retail/professional broker commission schedule is acceptable as the cheapest-path pin.)
3. **Borrow/locate cost for shorts.** What is the current typical locate fee in bps/day (and minimum locate hold time) for hard-to-borrow US large-cap shorts that would hit this drift book's short sleeve? Needed to replace the `0.0` [default] `borrow_bps_per_day` when the short sleeve is enabled.
4. **Drift-window compression.** Is there any post-2013 published estimate (paper, NBER WP, or practitioner study) of how the identified-news drift window has changed over time — specifically whether the multi-day continuation documented by Chan (2003) and Boudoukh et al. (NBER WP 18725) has compressed, and by how much? This is currently a Grok chatbot observation (unverified lead), not evidence.
5. **TRNA field schema.** What is the current documented field set of the `MRN_TRNA` news-analytics payload (sentiment, novelty, relevance, volume, and any confidence scores), and which field best maps to the module's `sig` (signed story sentiment)? Needed to make the §T0.3 vendor-column mapping exact rather than illustrative.
...
> the `gate == 1` confirmation guard, and

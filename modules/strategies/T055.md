...
sources:
  ...
  - "Wang (2025). Better Inputs Matter More Than Stacking Another Hidden Layer. arXiv:2506.05764"
unverified_sources: []
consumes:
...
| Row | Value |
|---|---|
| Module | T055 — DeepLOB + Feature-Stack Classifier, v1.0.0 |
...
| Regime gates | Provisional: provisional: wide-spread / thin-book regimes veto (S001 depth gate) |
...
2. **Cheapest-source row:** min_tier = Databento Standard (L2) — cheapest data source candidate [internal-est]; latency_class = microstructure / sub-second; required_fields = L2 MBP-10 snapshots, exchange timestamps; price_band = `~$200 + usage`/mo [unverified] (indicative — verify before budgeting); build_or_buy = **build the classifier harness on bought L2; the network architecture is the commodity — your labels, calibration, and cost gate are the product**.
3. **Resource budget:** `4` cores, `16` GB RAM + GPU for training [internal-est] [internal-est]; compute pattern `per_event`.
...
```
ENTER_LONG  := (c >= c_min) AND (p0 < p0_max) AND (argmax = UP)
...
```
EXIT := (mid moves +/- target_ticks / stop_ticks)   # 2 ticks [example]
        OR (events_held >= time_stop_events)                   # 10 events [example]
        OR (fresh prediction c < 0.50 against the position)    # confidence collapse [example]
...
```python
def size_shares(c, c_min=0.58, q_base=100, touch_depth=10_000):  # defaults [default]
    # Uncertainty-aware: linear from 0 at the gate to full size at c=1. [example]
    n = int(q_base * max(0.0, c - c_min) / max(1.0 - c_min, 1e-9))
    return int(min(n, 0.01 * touch_depth))  # 1% of visible touch depth [example]
```
...
venue/tier/source: XNAS / Databento MBP-10 [example]. `borrow_bps_per_day`: `0.0`
[default] — intraday-only, sub-second holds.
...
|---|---|---|---|---|
| Confidence gate | `c_min` | `0.58` | [example] | high |
| Flat-class veto | `p0_max` | `0.40` | [example] | medium |
| Round-trip cost | `rt_cost_ticks` | `0.6` ticks | [example] | high |
| Base size | `q_base` | `100` sh | [example] | medium |
| Target / stop | `target_ticks` | `±2` ticks | [example] | high |
| Time stop | `time_stop_events` | `10` events | [example] | low |
...
$c_t \ge 0.58$ [example], $p^0_t < 0.40$ [example] (flat-class veto), and the
expected move covers the 0.6-tick round-trip cost [example]. Direction =
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        ...
        if state.position is None and state.module_state == OK:
            if ((c >= 0.58) and (p0 < 0.40) and (expected_move >= rt_cost)) and cost_ok:
            ...
            if ((target/stop hit) or (events_held >= 10) or (confidence collapse)):
...
Heavy for a laptop (Tier H, `60–200` h ≈ `$9k–30k` eng + `~$200`/mo + usage data per notes/cost-model.md §5): L2 ingest at `10–50`k events/s busy [internal-est] needs Rust (Python `100–500`k events/s [internal-est] is borderline); GPU for training (M5 GPU `1–50` ms/sequence [internal-est] per cost-model §2). Bottleneck is inference latency, not training.
...
| Wang (2025), arXiv:2506.05764 | Crypto LOB data | simpler models match or beat deep nets on the paper's data [documented] | `BEFORE-COST` (classification) | model-complexity caution: the GBM may be doing the real work |
...
4. Wang (2025) [documented]. "Better Inputs Matter More Than Stacking Another Hidden Layer." arXiv:2506.05764. https://arxiv.org/abs/2506.05764 — simpler models match/exceed deep nets on LOB data; keep the GBM challenger.

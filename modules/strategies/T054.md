...
changelog:
  ...
  - "1.1.0 2026-09-11: deep review wave 2 — hysteresis implemented in the
    harness and normative pseudocode (was prose-only); sizing function
    standardized to shares=f(risk_budget_R, stop_distance, vol_estimate,
    ADV_cap, cost); machine-readable regime-gate records added; threshold
    calibration recipe added; GROK-NEEDED section added; fixtures regenerated
    (5 tickets)"
---
...
| Row | Value |
|---|---|
| Module | T054 — Hurst/Variance-Ratio Regime Toggle, v1.1.0 |
...
| Regime gates | The toggle IS the gate: machine records in §T2 (R-PROV-001..003, provisional); H `≥0.55` [default] momentum armed, `≤0.45` [default] reversion armed, else dark; non-finite H → `UNKNOWN`, no sleeve armed (restrictive default) |
...
R modules are provisional (R001–R050 annotation pass pending), so these use
provisional IDs; every record has a restrictive `unknown_behavior` (dark).

```yaml
regime_gates:
  - regime_id: R-PROV-001  # provisional: intraday trend-persistence
    direction: amplifies
    mechanism: "H >= H_trend ([default] `0.55`) on `hyst_bars` consecutive 5-min readings ([example] `2`) arms the momentum sleeve"
    condition: "H >= H_trend AND arm.streak >= hyst_bars"
    action: enable_entry   # sleeve armed; the cost gate still applies per ticket
    min_lag: 5             # [example] minutes: one H re-estimation window
    unknown_behavior: dark # [default] non-finite/stale H -> UNKNOWN, no sleeve armed
  - regime_id: R-PROV-002  # provisional: intraday mean-reversion
    direction: amplifies
    mechanism: "H <= H_rev ([default] `0.45`) on `hyst_bars` consecutive readings ([example] `2`) arms the reversion sleeve"
    condition: "H <= H_rev AND arm.streak >= hyst_bars"
    action: enable_entry
    min_lag: 5             # [example] minutes
    unknown_behavior: dark # [default]
  - regime_id: R-PROV-003  # provisional: random-walk dead zone
    direction: degrades
    mechanism: "`H_rev < H < H_trend` [example]: no sleeve has an edge; holding burns carry/costs"
    condition: "H_rev < H < H_trend"
    action: pause_entry    # also: exit any open position (dead-zone exit, §T3)
    min_lag: 5             # [example] minutes
    unknown_behavior: dark # [default]
```
...
```python
def size_shares(risk_budget_R=500.0, stop_distance=1.0,   # defaults [default]
                vol_estimate=None, ADV_cap=None, cost=None):
    """shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost).

    Dollar-risk N = floor(R/stop_distance), capped at ADV_cap; `cost`
    enters through the normative cost-gate predicate (no ticket when it
    fails), not through size. [example] reference implementation — mirrors
    the test harness; the module passes ADV_cap = sleeve_cap // price.
    """
...
**Calibration recipe (`H_trend`, `H_rev`, `hyst_bars`):** grid-search `H_trend`
∈ `0.52`–`0.65`, `H_rev` ∈ `0.35`–`0.48`, `hyst_bars` ∈ `1`–`5` [default] on
walk-forward splits with a `1`-day embargo [default]; rank candidate thresholds
by the Deflated Sharpe Ratio (Bailey & López de Prado 2014); require ≥ `200`
OOS sleeve trades [example] and ≥ `2` full regime cycles [example] per split;
threshold sets with DSR < `1.0` [default] are rejected toward `0.5` (no toggle).
Re-calibrate quarterly [default], or sooner when the C11 chatter cap trips.
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    arm = {side: None, streak: 0}                                  # hysteresis state
    for bar t in signals (newest last):
        ...
        cost_ok = expected_cost_bps(...) <= cost_gate_k * edge_bps
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
        # hysteresis (load-bearing): arming needs hyst_bars consecutive readings
        if H >= H_trend:      arm = {side: MOM, streak: arm.streak + 1 if arm.side == MOM else 1}
        elif H <= H_rev:      arm = {side: REV, streak: arm.streak + 1 if arm.side == REV else 1}
        else:                 arm = {side: None, streak: 0}         # dead zone resets the streak
        if state.position is None and state.module_state == OK:
            armed = (arm.side in {MOM, REV}) and (arm.streak >= hyst_bars)  # 2 readings [example]
            if armed and cost_ok:
                ticket = OrderTicket(symbol, side, qty, limit, tif,
                                     ticket_id=uuid(), parent_signal="S090@t",
                                     intent_ts=t.event_ts, state=NEW)
                qty = size_shares(R_usd, stop_dist, vol_est, sleeve_cap // price, cost_bps)
            ...
            if ((H_rev < H < H_trend) or bars_held >= time_stop_bars):   # dead zone exits immediately
                emit exit intent (cancel-replace rules, Appendix c) # C1
                arm = {side: None, streak: 0}
...
- Acceptance tests (`modules/tests/test_T054.py`, `7` tests):
  ...
  counter-case (`cost_gate_k = 0.5` [default]); `3.` no-signal-bar fills
  ...
  (qty / bps / $) match the generation run; `7.` hysteresis: one qualifying
  reading emits nothing, two consecutive readings arm (deep-review wave 2).
- `test_cmd`: `python3 -m pytest modules/tests/test_T054.py -q` → exits `0`.

**Cost-function calls on the fixture** (inputs → bps → $, from the harness run; all hysteresis-armed [example]):

| ticket | notional ($) | adv_pct | side | edge_bps | cost_bps | cost_$ | gate | event |
|---|---|---|---|---|---|---|---|---|
| T-T054-2-0 | 100022.88 | 0.0200 | taker | 8.8000 | 2.9000 | 29.0066 | PASS | entry (MOM armed on 2nd reading, sig=0.61) |
| T-T054-4-X | 100013.24 | 0.0200 | taker | 0.0000 | 2.9000 | 29.0038 | N/A | exit (dead zone, sig=0.50) |
| T-T054-7-0 | 99977.52 | 0.0200 | taker | 8.0000 | 2.9000 | 28.9935 | PASS | entry (REV armed on 2nd reading, sig=0.40, SHORT) |
| T-T054-9-X | 100015.84 | 0.0200 | taker | 0.0000 | 2.9000 | 29.0046 | N/A | exit (dead zone, sig=0.49) |
| T-T054-11-0 | 99978.12 | 0.0200 | taker | 8.0000 | 2.9000 | 28.9937 | PASS | entry (MOM armed on 2nd reading, sig=0.60) |

**Where the example is optimistic:** the fixture demonstrates the toggle's sleeve-arm intents; production sleeve entries (momentum sign sum, Bollinger tag-and-reenter) fan out per sleeve logic. Estimation chatter is suppressed by hysteresis in this run — the sketch is flattered by its smooth synthetic H path; real 1-min H is noisier and will arm less often.
...
- Third-party corroboration (not vendor-confirmed): a GitHub-hosted market-data source sheet lists Massive (formerly Polygon.io) Stocks Advanced at $199/month for real-time equities data. Tag stays `[unverified]` until the vendor pricing page is checked directly.

## GROK-NEEDED (exact questions web search could not answer)

1. Massive (formerly Polygon.io) Stocks Advanced, 2026: what is the current monthly price for real-time US equities 1-min aggregates via REST + WebSocket, and does real-time trading use require an exchange display/entitlement add-on on top of the plan? Quote the current pricing-page URL and as-of date.
2. Nasdaq (XNAS) equities, 2026: what is the current per-share charge for removing liquidity (taker) for a standard non-member stat-arb flow, and which SEC filing / rule release number sets it? As-of date required.
3. Is there any published calibration of Hurst-exponent thresholds (e.g. H ≥ 0.55 / H ≤ 0.45) specifically for 1-minute US equity bars — published values, or a study justifying a dead-zone width on intraday bars? If none exists, say so explicitly; thresholds stay `[example]` grid-search calibrated.

...
stage: 195
seed: 195
rng: "numpy.random.default_rng(195)"
provisional_regimes: ["R008", "R004"]
dependency_pins:
  python: ">=3.11"
  numpy: ">=1.26"
  pandas: ">=2.2"
  pytest: ">=8.0"
parents: [S075, S074, S068]
...
- **Seed / RNG:** `seed=195`, `numpy.random.default_rng(195)` [measured]
- **Provisional regimes:** `R008`, `R004`
- **Parent signal(s):** `S075` (dispersion spread); `S074` (regime switch); `S068` (carry regime gate)
- **Universe:** 8 large-cap names [example] + SPX 30-day straddles; 30-day expiry [example]; each vintage held 5–20 sessions [example].
...
Construction: long 8 single-name 30d straddles (delta-hedged daily), short SPX 30d straddles in index-beta-weighted notional.
```
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    if enter:
        ...
        for leg in legs:
            ...
            tickets.append(t)
    assert all(fill > signal for fill, signal in fills)              # t->t+1
...
Fixture tape (`T095_tape.csv`; `# TYPE:` header is part of the file):
...
on the gate-pass row (bar 0 [measured]): notional = 450.0 × 8 = $3,600.00 [example]:
...
- Data: CBOE / OPRA [example]; Research OPRA + index options via Databento ~$200/mo + usage [unverified]; production OPRA institutional pricing [unverified]
...
as such. Research OPRA + index options via Databento ~$200/mo + usage [unverified]; production OPRA institutional pricing [unverified] Confirm each feed's license before
...
- **Cheapest row:** min_tier = OPRA history via Databento (Tier 2) [unverified]; latency_class = daily bar-clock; required_fields = options quotes/OI for GEX model, index options, VIX curve; price_band = ~$200/mo + usage [unverified]; build_or_buy = build the dispersion+GEX harness, buy OPRA
...
|---|---|---|---|---|
| `R008` | inverts | VIX backwardation = stress; flatten entirely | vix backwardation [default] | veto |
| `R004` | degrades | correlation spikes kill the dispersion harvest | realized corr > implied [default] | halve |
...
- All regime thresholds (20-day median spread, GEX > 0, contango definition, 50% cut, 20-session vintage) are the chapter's own `example` design values (2026-09-10), not published recommendations. [unverified]

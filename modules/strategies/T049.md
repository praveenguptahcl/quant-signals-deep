...
| Row | Value |
|---|---|
| Module | T049 — Unusual Options Activity Follower, v1.0.0 |
| Edge verdict | `plausible` — option volume predicts stock returns (Pan–Poteshman 2006; Johnson–So 2012); informed-trader mechanism (Easley et al. 1998) |
| After-cost | `narrow` — public prints are the stale shadow of the informed flow — and predictability decays (Cremers–Weinbaum) |
...
| risk_class | market |
| Capacity | underlying-liquidity constrained [unverified] |
...
2. **Cheapest-source row:** min_tier = OPRA; latency_class = minutes;
   required_fields = option prints (price/size/side), stock volume, open interest; price_band = `$0–500`/yr (indicative —
...
|---|---|---|
| option prints | Event | price/size/side |
| stock volume/OI | Bar | normalization |
...
```
uoa = volume_z vs history, OI, and stock volume      # unusual activity
urgency = aggressive-take share, sweep score            # directional character
ENTRY = uoa_z >= uoa_z and urgency >= urgency_min and not hedged -> follow direction
```

### Exits (with post-exit cooldown)

```
drift realized -> underlying moves with flow -> scale out
flow reverses -> FLAT
timeout -> max_hold_h -> FLAT; stop on adverse
```

### Position sizing (fenced function)

```python
def size_uoa(risk_R_usd, stop_bps, px, adv_shares, urgency, cfg):
    raw = risk_R_usd / (stop_bps / 1e4 * px)
    return min(raw * urgency, cfg.adv_cap_pct / 100 * adv_shares)  # scale by urgency
```
...
| Fees | `0.30` | [example] |
| Borrow (short follow) | `30.00` | [example] |
| Impact | `k·√(adv_pct/100)`, k=`25.0` | urgent [example] |
| **Total reference (one-way)** | `~`57` one-way short-follow [example]` | [example] |
...
| C12 | C12 Flow-concentration: trigger = > 5 names same direction same hour [example] → check = concentration review → action = cap → log with code `C12` |
...
def emit(state, signals, cfg):
    sig = primary(signals, "S073")          # UOA score + urgency + direction
    ...
    if state.position != 0:
        ...
        return [], state.ok()
    if sig.uoa_z >= cfg.uoa_z and sig.urgency >= cfg.urgency_min and not sig.hedged:
        cost = expected_cost_bps(notional(), adv_pct(), venue(), "taker", "high", cfg)
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
        if cost <= cfg.cost_gate_k * sig.edge_bps:
            t = OrderTicket.intent(sig.direction, qty=size_uoa(...), limit=last_price())
...
- Acceptance tests (`modules/tests/test_T049.py` — `6` [default] tests):
...
|---|---|---|---|---|
| `0` | notional=`200000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.77` = `3.57` bps → `$71.36` | `2.0` edge, k=`0.5` [default] → `3.57 ≤ 1.0` BLOCK | `HOLD` / `OK` |
| `3` | notional=`200000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.77` = `3.57` bps → `$71.36` | `14.271067811865475` edge, k=`0.5` [default] → `3.57 ≤ 7.14` PASS | `LONG` / `OK` |
| `6` | notional=`200000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.77` = `3.57` bps → `$71.36` | `3.0` edge, k=`0.5` [default] → `3.57 ≤ 1.5` BLOCK | `BUY` / `OK` |
| `7` | notional=`200000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.77` = `3.57` bps → `$71.36` | `0.05` edge, k=`0.5` [default] → `3.57 ≤ 0.03` BLOCK | `HOLD` / `OK` |
| `9` | notional=`200000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.77` = `3.57` bps → `$71.36` | `4.0` edge, k=`0.5` [default] → `3.57 ≤ 2.0` BLOCK | `HOLD` / `OFF` |

**Hand-checks.** Row `3` (entry): qty = `250 / (25/1e4 × 100.75)` = `992` raw → ADV-capped at `20000` → `992` shares [example]; ticket `T0003`, side `LONG`, `marketable` intent. Row `6` (exit): |z| through the exit band → `LONG` intent, exits never cost-gated [default]. Row `7` (gate block): edge `0.05` bps [example] vs cost `3.57` bps → gate `3.57 ≤ 0.025` BLOCKS → no intent, state stays `OK`. Row `9` (kill): daily P&L `-2.1%` [example] ≤ −stop (`2.0%` [default]) → `ARMED → TRIPPED`, state `OFF`, no intents. Causality: entry intent at row `3` has `intent_ts` = bar-3 `event_ts`; the earliest fill is bar `4`'s open — the test asserts `fill_event > signal_event` on every intent row.
...
|---|---|---|---|---|---|---|
| R001 | veto | hedged flow | S091 hedge flag | no entries | immediate | restrictive |
| R002 | stand-down | OpEx week | expiry calendar | pause follows | 1 week | restrictive |

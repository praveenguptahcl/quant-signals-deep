...
stage: 177
seed: 177
rng: "numpy.random.default_rng(177)"
...
- **Seed / RNG:** `seed=177`, `numpy.random.default_rng(177)` [measured]
...
- **Schedule:** 24/7 hourly evaluation; owner: crypto-strategies on-call
...
- **Funding interval:** 8 h [example]; funding is the running yield of the short-perp leg — a P&L line, not a cost component.
...
- spot leg margined: 5% pa / 365 = 1.37 bps/day [example]; perp leg funding is a P&L line
...
shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost)
    notional_pair = min(available_margin / 3, 0.02 * venue_hourly_volume)   # [default]
    qty = notional_pair / spot_price
    qty = min(qty, 0.10 * hourly_bar_volume)                              # 10% cap [default]
```

- Exits (normative):

```
EXIT := (attainable_basis_ann < 2%) [default] OR (funding_ann < −5%) [default]
        OR (hold >= 30 days) [default] OR (intraday divergence > 3× [default] entry basis) [default]
ON_EXIT: cooldown_until = now + 24 h   # [default]; C10
```
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    b = signals[-1]  # hourly bar
    assert b.spot_px > 0 and b.perp_px > 0 else return [] and state UNKNOWN  # F2
    ...
    funding_cover = (b.funding_8h * (1-cfg.funding_haircut) * 3 * cfg.max_hold_days) / b.spot_fin_cost
    ...
    tickets = []
    if enter:
        qty = floor(min(state.margin/3, 0.02*b.venue_hourly_vol) / b.spot_px)
        ...
        tickets = [t1, t2]  # both legs same bar or cancel both
    return tickets
```
...
| Funding rate | float | hourly | Tier 1 | the carry; funding_cover ≥ 1.0 [default] |
...
- Data: same-venue spot + USDⓈ-M perp / venue websocket [example]; Research ~$0 (venue spot+perp feeds free) [unverified]; history free flat files or Kaiko/CoinMetrics thousands/yr [unverified]; production ~$0 + compute [unverified]
...
as such. Research ~$0 (venue spot+perp feeds free) [unverified]; history free flat files or Kaiko/CoinMetrics thousands/yr [unverified]; production ~$0 + compute [unverified] Confirm each feed's license before
...
- **Cheapest row:** min_tier = venue websocket (free) [unverified]; latency_class = hourly bar-clock; required_fields = spot+perp OHLCV, funding, order-book top; price_band = ~$0/mo [unverified]; build_or_buy = build the basis/hedge engine; buy history only if free flat files insufficient
- **Budget:** 1 core, <1 GB RAM for 2 pairs [internal-est]; compute pattern per_bar (hourly)
- **What it skips:** beta-adjusted hedge ratio (phase 2); cross-exchange legs; second-venue confirmation
- **Escalate when:** Upgrade when: measured 4-leg friction exceeds the hurdle margin [default]; pairs >4 [default]; stablecoin-collateral haircut changes
...
|---|---|---|---|---|
| `R001` | degrades | stress thins both books; S014 friction eats the quoted basis | rv_pctile > 80 [example] | halve |
| `R003` | inverts | funding flip turns the carry negative | funding_ann < -5% [default] | veto |
...
8. Validate with the fixtures: python3 -m pytest modules/tests/test_T077.py -q

...
cost_model:
    ...
    borrow_bps: "0.0"
    borrow_basis: "[default] go reference; shorts add C7"
    impact_bps: "6.0"
...
kill_switch:
    ...
    trip_conditions: ["2 consecutive gap fills [example]", "feed heartbeat missed > 2 s [example]", "clock skew > 50 ms [example]", "4 concurrent gap rides [example]"]
...
conflicts: []
unverified_sources: ["Gap-and-go P&L claims from practitioner forums — no citable published P&L found; not promoted."]
supersedes: []
changelog:
    - date: "2026-09-10"
...
**When it dies.** Unfunded gaps (RVOL gate exists for this); the gap fills by `10:00`; crowded gap-chasing.
...
Cheapest viable path: **Polygon.io Stocks Basic** — daily + 5-min OHLCV bars, news flags at ~$30–100/mo [unverified]. build the screen and continuation machine on a cheap bar feed; buy nothing.

Resource budget: `1` core, `<1` GB RAM [internal-est] [internal-est] · compute pattern `per_bar`.

Explicitly out of scope (phase two): gap fade (see T012); earnings gaps (phase `2`).
...
function emit(state, signals, bars, cfg) -> list[OrderTicket]:
    ...
    edge_bps = abs(g) * 1e4 * 0.4                                   # [example]
    ...
    if state.position == 0 and cost_ok and t.event_ts >= state.cooldown_until:
        if abs(g) >= cfg.gap_min and rv >= cfg.rvol_gap_min:
            side = 'BUY' if g > 0 else 'SELL'   # ride the gap
...
| `News flags` | reference | per event | Tier 2 | S035 context |
...
**Worked example (fixture-backed, from `fixtures/T018_tape.csv` trade 1).**

Trade 1: `BUY` `750` shares [example] @ `50.9` [example], exit `51.9` [example] (`target 1×gap`). Gross P&L = `750.00` [measured] dollars. Cost stack = `18.0` bps [example] → `68.72` [measured] dollars on `38175.00` [example] notional. Net = `681.28` [measured] dollars. The fixture's expected CSV carries the same arithmetic for all six trades; the per-module test recomputes it from the tape and asserts equality.

**Flow.**

```mermaid
flowchart LR
    PRE["pre-open screen"] --> GAP{"|gap|>=1.5%?
RVOL>=2x?"}
    GAP -->|no| SKIP["Skip"]
...
| Borrow | `0.0` | [default] go reference; shorts add C7 |
| Impact | `6.0` | [example] chasing the gap — 3¢ adverse on a $50 name |
| **Total** | `18.0` | [example] reference stack |

Fee schedule as of `2026-09-01` [example] · venue `XNAS / SIP 5-min bars [example]` · side `taker` · borrow `0.0` bps/day [example] — reference build rides the gap; short variant models borrow explicitly.
...
**Ops.** Schedule: Pre-open gap screen; entries `09:35`–`11:00` ET [example]; flat by `12:00`; no overnight carry · budget: `1` vCPU, `1` GB RAM [internal-est] [internal-est] · env: `POLYGON_API_KEY`.

**Universe.** US equities, price > `$10`, ADV > `1`M shares [example]. Screen:
overnight gap ≥ `+1.5`% [example], RVOL(gap) ≥ `2.0`× [example], no jump
against the direction. Entries `09:35`–`11:00`. Exclude: earnings days, halts.
...
|---|---|---|---|
| 1 | Gap fills | detect: `2` consecutive → action: stand down (C11) → recovery: next session [default] | `gap_fills < 2` |
| 2 | Unfunded gap | detect: RVOL < `2.0`× → action: skip (gate) → recovery: next session [default] | `rvol_gap >= 2.0` |
...
```text
Build the T018 (RVOL-Gated Gap-and-Go) strategy module from this specification.
Hard constraints:
...
Config surface:
- gap_min (float): default 0.015, range 0.005–0.05 [default]
- rvol_gap_min (float): default 2.0, range 1.0–5.0 [default]

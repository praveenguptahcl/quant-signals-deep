...
changelog:
  ...
  - "1.0.1 2026-09-10: deep-review pass — cfg-referenced normative pseudocode
    (cooldown/locate/adverse exits added), fenced sizing = shares(risk_budget_R,
    stop_distance, vol_estimate, ADV_cap, cost), dual (event_ts, asof_ts),
    machine-readable provisional regime-gate records (§T5), venue fee pin in
    COST block, Polygon price band tightened, gate = S084 flow-opposition
    proxy documented, GROK-NEEDED section added"
---
...
| Row | Value |
|---|---|
| Module | T052 — AR/ARMA Innovation Trader, v1.0.1 |
...
| Data cost | Research `~$29–199`/mo [unverified]; production `~$199 + usage`/mo [unverified] — Polygon Advanced ≈ $199/mo per third-party 2026 doc (written for options tiers; stocks-tier price unverified) — verify before budgeting |
...
| Regime gates | Provisional machine records in §T5 (R-PROV-INFORMED-FLOW / R-PROV-VOL-REGIME / R-PROV-TREND); restrictive default: unknown → veto/pause |
...
| `phi` | float | `0.15` | `−1.0`–`1.0` [default] | calibrate |
...
| `per_trade_R` | float | `600.0` | `100`–`5,000` | example |
| `participation_cap` | float | `0.0002` | `0.00005`–`0.001` | default |
...
| `n`, `vw` | `trade_count`, `vwap` [unverified] — Polygon agg fields per vendor docs; not verified in this pass |
| symbol | `symbol` (exchange-qualified) |
| signal value | `sig` (strategy-defined score, Appendix b `value`) |
| gate flag | `gate` ∈ {0, 1} (confirmation gate; **toy carrier of the S084 flow-opposition check** — `gate=1` means the upstream S084 toy asserted `sign(flow) != sign(r)` for this bar; production reads signed trade flow directly) |

### T0.4 Time contract

> Time contract: clock domain UTC, int64 ns; authoritative causality clock =
> `event_ts`; dual timestamps `(event_ts, asof_ts)` — `asof_ts` is bar
> finalization/receipt time (int64 ns UTC); staleness measured on `asof_ts`;
...
ENTER_LONG  := (z < -z_entry) AND (sign(flow) != sign(r))
               ...
               AND (expected_cost_bps(...) <= cost_gate_k * edge_bps)   # k := cost_gate_k [default]
...
def shares(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost) -> int:
    # risk_budget_R: dollar risk per trade = per_trade_R = $600 [example]
    # stop_distance: $ adverse move that exhausts R = 3 * sigma_eps * price [example]
    # vol_estimate:  innovation sigma as fraction of price (feeds sigma_eps calibration)
    # ADV_cap:      max shares as participation_cap * ADV_dollar / price; participation_cap = 0.0002 [default]
    # cost:         round-trip $ from expected_cost_bps (enforced via the cost gate, not the share count)
    q_risk = risk_budget_R / max(stop_distance, 1e-9)
    q = min(q_risk, ADV_cap, 800)   # 800 = fixed_qty reference anchor [example]
    return int(max(0, q))
```

`cost` is intentionally not a share-count input: the normative cost-gate
predicate `expected_cost_bps(...) <= cost_gate_k * edge_bps` (§T3) vets every
ticket at full size, so a cost-blowup blocks the ticket instead of shrinking
it into a dust position.
...
venue schedule before production. Venue pin for reference: Nasdaq fee to
remove liquidity `$0.0030`/share for shares ≥ `$1.00` [documented]
(nasdaqtrader.com price list, crawled 2026-06 — see §T10 source 4); the toy's
`$0.0035`/share/side [example] stays the single source of truth for the
fixture. `side: taker` [default];
venue/tier/source: XNAS / Polygon.io Stocks Advanced [example]. `borrow_bps_per_day`: `0.0`
[default] — no borrow in the reference build; shorts add C7 locate cost.
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        assert isfinite(t.event_ts, t.open, t.high, t.low, t.close, t.volume, t.sig) else return UNKNOWN   # F2
        assert t.event_ts > prev_event_ts else return UNKNOWN       # F2 monotonic
        s   = innovation z-score z = (r - f_hat)/sigma_eps         # data <= t only
        assert isfinite(s) else return UNKNOWN                      # F2
        edge_bps = abs(s) * edge_mult                              # edge_mult = 1.5 [default]
        cost = expected_cost_bps(notional, adv_pct, venue, side, urgency)
        cost_ok = expected_cost_bps(...) <= k * edge_bps
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps (k := cfg.cost_gate_k)
        flow_opposed = (t.gate == 1)   # toy: gate carries the S084 flow-opposition confirmation (sign(flow) != sign(r));
                                      # production: read signed trade flow, require sign(flow) != sign(r)
        if state.position is None and state.module_state == OK and t.event_ts >= state.cooldown_until:
            if (((s <= -cfg.z_entry) or (s >= cfg.z_entry)) and flow_opposed
                    and (abs(eps) * t.close > cfg.cost_thr) and cost_ok):
                side = BUY if s <= -cfg.z_entry else SHORT
                if side == SHORT:
                    assert locate_ok(t.symbol) else return GATE_VETO   # C7
                q = shares(cfg.per_trade_R, 3*cfg.sigma_eps_bp/1e4*t.close, cfg.sigma_eps_bp/1e4, adv_cap(t), cost)
                # adv_cap(t) = cfg.participation_cap * ADV_dollar(t) / t.close   [default]
                ticket = OrderTicket(symbol, side, q, limit, tif,
            ...
            adverse_bps = sign(position.side) * (position.entry_px - t.close)/position.entry_px * 1e4
            if ((z crosses 0 against position) or (bars_held >= cfg.time_stop_bars)
                    or (adverse_bps > 3 * cfg.sigma_eps_bp)):       # §T2 adverse exit [example]
...
### Regime-gate records (provisional, machine-readable)

R001–R050 are not yet authored; provisional IDs below, restrictive default
(unknown → veto/pause). Machine schema: `{regime_id, direction,
mechanism, condition, action, min_lag, unknown_behavior}`.

| regime_id | direction | mechanism | condition | action | min_lag | unknown_behavior |
|---|---|---|---|---|---|---|
| R-PROV-INFORMED-FLOW | degrades | informed flow makes faded quotes adverse-selected | `\|LRI\|` > 70th pct of trailing 30 days [example] | veto | 1 bar | veto (unknown → no entry) |
| R-PROV-VOL-REGIME | inverts | AR(1) phi flips sign in microstructure shifts | `sign(phi_t)` ≠ `sign(phi_{t-1})` | pause_entry | 1 bar | pause_entry |
| R-PROV-TREND | degrades | sustained trends: innovations fail to mean-revert | `\|z\|` same sign ≥ `10` bars [example] | halve | 5 bars | halve |

Toy form: R-PROV-INFORMED-FLOW is asserted per-bar via the `gate` flag
(§T0.3); the other two are checked on the nightly re-estimate pass.
...
**Web research log (2026-09-10, deep-review pass):**

4. Nasdaq "Price List – Trading" (nasdaqtrader.com/trader.aspx?id=pricelisttrading2&l, crawled 2026-06) — fee to remove liquidity $0.0030/share for all MPIDs, shares ≥ $1.00 [documented]; used only as the §T2 venue pin, not in the toy cost model.
5. Polygon pricing tiers via third-party doc: https://github.com/thechuck88/gamma-gex-scalper/blob/HEAD/POLYGON_VS_OTHER_PROVIDERS.md (dated 2026-01-10) — Polygon Advanced ≈ $199/mo [unverified] (doc written for options tiers; stocks-tier price unverified; verify before budgeting).

## GROK-NEEDED (web search could not answer — exact questions for Grok)

- **Q-T052-DR1.** Polygon.io Stocks Advanced: what is the current monthly price for the stocks-only tier covering 1-min aggregates + corporate actions, and does the stocks Advanced tier include trade-sign data (last-sale/tick history) usable for an honest Hasbrouck VAR, or is a higher tier required?
- **Q-T052-DR2.** On 2024–2026 US large-cap 1-min mid-price bars, is there any published distribution of |LRI| (Hasbrouck long-run impact) — specifically a 70th-percentile threshold usable as an informed-flow veto? Cite study + number.
- **Q-T052-DR3.** Is there a published after-cost Sharpe ratio (or per-trade bps net of costs) for a standalone AR(1) mid-price fade on US equities intraday? Cite study + metric + period.
- **Q-T052-DR4.** For a retail-routed taker executing marketable IOC orders on XNAS at ≤ 10k shares/day, what is the all-in per-share fee schedule (taker fee + SEC §31 + FINRA TAF) as of 2026-09? Cite the venue schedule + as-of date.
- **Q-T052-DR5.** Roll (1984) gives the AR(1) coefficient on trade prices under a constant spread. Is there a documented estimate of the AR(1) coefficient on 1-min mid-price log returns for modern US large caps (2015+)? Cite study + coefficient.

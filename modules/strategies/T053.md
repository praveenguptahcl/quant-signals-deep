...
| Row | Value |
|---|---|
| Module | T053 — HMM Regime-Switching Allocator, v1.0.0 |
...
```
TILT_MOM := (piM >= 0.65) AND (hysteresis: `2` consecutive bars [example] outside dead zone)
              AND (expected_cost_bps(...) <= cost_gate_k * edge_bps)
TILT_REV := (piM <= `0.35` [example]) AND (hysteresis satisfied)
              AND (expected_cost_bps(...) <= cost_gate_k * edge_bps)
...
```python
def allocate(piM, tau=0.65, lamM=1.0, lamR=1.0, NAV=1_000_000):  # defaults [default]
    # Dead-zone blend (arithmetically consistent form). [example]
    wM = min(1.0, max(0.0, (piM - (1 - tau)) / (2 * tau - 1)))
    wR = 1.0 - wM
    lamM = min(0.10 / ewma_vol_M, 2.0)  # vol targeting [example]
    lamR = min(0.10 / ewma_vol_R, 2.0)
    return wM * lamM * NAV, wR * lamR * NAV  # sleeve notionals
```
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        s = momentum posterior piM from the causal forward filter                                            # data <= t only
        assert isfinite(s) else return UNKNOWN                      # F2
        edge_bps = abs(piM - 0.5) * 30.0                                   # [example] — modeled per-rebalance edge in bps
        ...
        if state.position is None and state.module_state == OK:
            if ((piM >= 0.65 and sleeve != MOM) or (piM <= 0.35 and sleeve != REV)) and cost_ok:
...
- Acceptance tests (`modules/tests/test_T053.py`, `6` tests):
...
|---|---|---|---|---|---|---|---|
| T-T053-3-0 | 599927.30 | 0.0200 | mixed | 6.6000 | 2.7000 | 161.9804 | PASS |
| T-T053-5-X | 600198.90 | 0.0200 | mixed | 0.0000 | 2.7000 | 162.0537 | N/A |
| T-T053-7-0 | 599865.20 | 0.0200 | mixed | 6.0000 | 2.7000 | 161.9636 | PASS |
| T-T053-9-X | 600094.90 | 0.0200 | mixed | 0.0000 | 2.7000 | 162.0256 | N/A |
| T-T053-11-0 | 599868.80 | 0.0200 | mixed | 5.4000 | 2.7000 | 161.9646 | PASS |
...
Feasible on the M5 Max (Tier M+, `40–100` h ≈ `$6k–15k` eng + `~$50–500`/mo data per notes/cost-model.md §5); the forward filter is O(states²) [documented] per bar — trivial. Bottleneck is building/calibrating the two sleeve books, not the allocator.
...
| Nystrup, Madsen & Lindström (2017), Quant. Finance | Multi-asset | regime-aware (HMM) dynamic allocation improves risk-adjusted performance vs static [documented] | `BEFORE-COST` | allocation study, not the two-sleeve rule |
| Christensen, Godsill & Turner (2020), arXiv:2006.08307 | Intraday momentum | HMM with latent momentum state + side information; 2–3 states by CV; forward-algorithm timing [documented] | `BEFORE-COST` | supports causal forward-filter use |
...
pass (Stage 153/200). **Honest bottom line.** For a ≤$1M paper operation this is one of the best value-for-money strategies in the batch: `~$50–500`/mo data [unverified], `~$3k–$9k` loaded engineering estimate [internal-est], runs on a laptop. Expect crash-avoidance value, not a higher calm-year Sharpe.
...
3. Nystrup, P., Madsen, H. & Lindström, E. (2017) [documented]. "Dynamic portfolio optimization across hidden market regimes." *Quantitative Finance*. — regime-aware allocation beats static allocation.

...
stage: 197
seed: 197
rng: "numpy.random.default_rng(197)"
...
- **Seed / RNG:** `seed=197`, `numpy.random.default_rng(197)` [measured]
...
shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost)
    qty = 1000   # fixed 1,000-share clips [example]; the machine is fixed-size, not risk-scaled
    names restricted to spread <= 5 bps [example]
```
...
EXIT := 50% retracement of the faded move [example]
        ...
        OR -1.0 x the Roll spread against [example] (bounce thesis dead)
ON_EXIT: cooldown_until = now + 10 min   # [default]; C10
```
...
|---|---|---|---|---|
| HYG/JNK quotes | float | per minute | Tier 1 | ETF price |
| NAV estimates | float | intraday | Tier 2 | discount = price − NAV |
| CDX index | float | intraday | Tier 2 | credit context |
| Corporate-bond liquidity proxies | float | daily | Tier 2 | pressure signature |
...
|---|---|---|---|---|
| Bongaerts, de Jong & Driessen (2011) [documented] | corporate bonds | liquidity risk pricing in credit [documented] | BEFORE-COST | the credit-dislocation rationale; the ETF basket is the chapter's design |
| Bao, Pan & Wang (2011) [documented] | corporate bonds | bond illiquidity and return predictability [documented] | BEFORE-COST | illiquidity evidence; the HYG/JNK fade is the chapter's design |
| Feldhütter (2012) [documented] | corporate bonds | the Swiss-cheese model of bond liquidity [documented] | BEFORE-COST | liquidity structure; not a tradable rule |
...
**Verdict:** `PREDICTIVE-FEATURE-ONLY` — Bid–ask bounce mean-reversion is textbook microstructure (Roll 1984 lineage); the 1,000-share [example] fade machine's after-cost edge is the chapter's own design.

**Selection disclosure:** these are the chapter's research-pass sources; no published after-cost Sharpe for the HYG/JNK liquidity-pressure fade was found [documented-as-absence].
...
|---|---|---|---|---|
| ETF discounts are structural (creation/redemption friction), not liquidity pressure | detect: discount persists > 5 sessions [default] | action: veto (it's not pressure) | recovery: discount normalizes | `check: pressure signature fresh` |
| NAV stale: the 'discount' is a stale-NAV artifact | detect: NAV timestamp > TTL | action: veto | recovery: fresh NAV | `check: nav_staleness <= ttl` |
| Liquidity-provider capacity exhausted: no one to fade into | detect: bid depth < 2× size [default] | action: halve size | recovery: depth restores | `check: bid_depth >= 2 * size` |
| Credit event (downgrade): the pressure is information | detect: rating-action flag | action: veto — never fade real news | recovery: none | `check: no rating action` |
| Overnight gap against the basket | detect: gap > 2× edge [default] | action: staged exit | recovery: — | `check: gap_bps <= 2 * edge` |
| Creation-unit costs exceed the modeled friction | detect: realized friction > 2× modeled [default] | action: widen friction in COST block | recovery: re-calibrate | `check: friction <= 2 * modeled` |
...
|---|---|---|---|---|
| `R001` | degrades | bounce non-stationary in high-RV; spreads widen | rv_pctile > 85 [example] | veto |

### Visual

```mermaid
flowchart TD
    HYG["HYG/JNK quotes"] --> DISC{"Discount to NAV?"} 
    DISC -->|yes| PRES["Liquidity pressure\nsignature?"]
    DISC -->|no| DROP["No trade"]
    PRES -->|yes| CAP["LP capacity ok?"]
    PRES -->|no| DROP
    CAP -->|yes| COST{"Cost gate\nE[cost] <= 0.5 x edge [default]"}
    CAP -->|no| HALF["Halve size"]
...
1. Implement the entry Boolean from §T2.1 and the exits/sizing from §T3.
   Gate order is fixed: move_ok, spread_ok, time_ok, then the cost gate.
...
8. Validate with the fixtures: python3 -m pytest modules/tests/test_T097.py -q

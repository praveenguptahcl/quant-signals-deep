...
stage: 193
seed: 193
rng: "numpy.random.default_rng(193)"
...
test_cmd: "python -m pytest modules/tests/test_T093.py -q"
...
- **Provisional regimes:** `R008`
- **Parent signal(s):** `S071` (derivatives positioning); `S073` (unusual activity confirmation); `S060` (timing confirmation)
...
- **sha256:** `REPLACE_ON_INGEST`
- **Test command:** `python -m pytest modules/tests/test_T093.py -q`
...
- **Pricing model:** the equity leg is the traded instrument; options are signal inputs only — no options positions are opened by this module.
- **Greeks used as signals:** 25-delta risk-reversal skew (S071); signed call/put flow (S073); no delta-hedging is performed by this module.
- **Expiry/assignment:** not applicable — the module holds no options; the equity leg is exited on skew mean-reversion, the hard stop, or the 8-session time stop.
...
- `S071` — 25-delta risk-reversal skew (role: derivatives positioning)
- `S073` — signed options flow (role: unusual activity confirmation)
- `S060` — Hayashi–Yoshida lead-lag (role: timing confirmation)
...
g1 = composite options-to-equity score S_B (≥ 1.25 [default]); g2 = call-volume ratio vs 20-day mean (≥ 3.0× [example]); g3 = relative equity spread in bps (≤ 10 bps [example])
...
- Venue: XNAS [example].
- equity leg is long-only in the reference build; no borrow leg
...
|---|---|---|---|
| Composite score | `S_B_min` | 1.25 [default] | high |
| Call-volume ratio | `call_ratio_min` | 3.0× 20d mean [example] | high |
| Max equity spread | `spread_max_bps` | 10 bps [example] | medium |
| Skew revert tol | `skew_revert_tol` | 0.5 vol pts [example] | medium |
| Time stop | `time_stop_d` | 8 sessions [example] | low |
...
- `edge_bps`: per-case expected edge in bps, mapped from the parent signal
  score. Reference value from the gate-pass fixture row: `26.0 bps [example]`.
...
shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost)
    q = (0.0040 * equity) / stop_distance   # 0.40% of equity [example]
    q = min(q, 0.05 * adv_20d)              # 5% of ADV [example]
    max 4 concurrent names [example]
```
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    if d.sessions_to_earnings <= 2: return []                       # earnings blackout
    skew_flat = d.skew_impulse_5d < 0
    ...
    tickets = []
    if enter:
        stop = 1.1 * d.sigma_20d
        qty = int(min(0.0040 * state.equity / stop, 0.05 * d.adv_20d))
        t = OrderTicket(sym, BUY, qty, None, OPG, uuid(), f'S071@{d.ts}', now(), NEW)
...
0,1700000000000000000,1700000300000000000,LONG,26.0,1.4,3.6,7.0,150.0,266,1
1,1700000300000000000,1700000600000000000,LONG,26.0,1.1,3.6,7.0,150.0,266,1
2,1700000600000000000,1700000900000000000,LONG,26.0,1.4,2.2,7.0,150.0,266,1
3,1700000900000000000,1700001200000000000,LONG,26.0,1.4,3.6,14.0,150.0,266,1
4,1700001200000000000,1700001500000000000,LONG,0.4,1.4,3.6,7.0,150.0,266,1
5,1700001500000000000,1700001800000000000,LONG,26.0,1.4,3.6,7.0,-1.0,266,0
```
...
|---|---|---|---|---|---|
| 0 | `LONG` | 26.0 | S_B=✓, call_ratio=✓, spread_ok=✓ | PASS | all gates + cost gate |
| 1 | `LONG` | 26.0 | S_B=✗, call_ratio=✓, spread_ok=✓ | no intent | gate veto |
| 2 | `LONG` | 26.0 | S_B=✓, call_ratio=✗, spread_ok=✓ | no intent | gate veto |
| 3 | `LONG` | 26.0 | S_B=✓, call_ratio=✓, spread_ok=✗ | no intent | gate veto |
| 4 | `LONG` | 0.4 | S_B=✓, call_ratio=✓, spread_ok=✓ | no intent | cost-gate veto |
| 5 | `LONG` | 26.0 | S_B=✓, call_ratio=✓, spread_ok=✓ | no intent | invalid input -> UNKNOWN |
...
|---|---|---|---|---|
| Options quotes/trades (OPRA) | mixed | per minute | Tier 2 | skew + signed flow |
...
- Data: XNAS [example]; Research OPRA via Databento ~$200/mo + usage [unverified]; production OPRA institutional pricing [unverified]
...
as such. Research OPRA via Databento ~$200/mo + usage [unverified]; production OPRA institutional pricing [unverified] Confirm each feed's license before
...
- **Cheapest row:** min_tier = OPRA history via Databento (Tier 2) [unverified]; latency_class = daily bar-clock; required_fields = options quotes/trades, equity daily bars, earnings calendar; price_band = ~$200/mo + usage [unverified]; build_or_buy = build the skew/flow screen, buy OPRA
...
| Skew data stale > 1 session [default] | detect: freshness monitor | action: veto entries | recovery: feed healthy | `check: skew_staleness <= 1 session` |
...
|---|---|---|---|---|
| `R008` | degrades | earnings season makes options flow noise, not signal | earnings blackout [default] | veto |
...
```
You are an AI engineer implementing strategy module T093 (Options-to-Equity Lead Trader).

Build exactly what this file specifies — nothing more:

1. Implement the entry Boolean from §T2.1 and the exits/sizing from §T3.
   Gate order is fixed: S_B, call_ratio, spread_ok, then the cost gate.

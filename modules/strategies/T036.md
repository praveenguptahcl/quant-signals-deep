...
datasets:
  - name: t036-data
    vendor: MULTI-ASSET tick
    product: synchronized trade/quote ticks, both instruments
...
| Row | Value |
|---|---|
| Module | T036 — Cross-Asset Lead-Lag (Hayashi–Yoshida), v1.0.0 |
| Edge verdict | `plausible` — futures-lead is the classic documented lead-lag (Chan 1992); HY (2005) is the right estimator for non-synchronous ticks |
...
| Data cost | multi-asset tick: mid four to five figures/yr [unverified] |
...
2. **Cheapest-source row:** min_tier = MULTI-ASSET tick; latency_class = seconds;
   required_fields = synchronized trade/quote ticks, both instruments; price_band = `$0–500`/yr (indicative —
...
```python
def size_catchup(risk_R_usd, stop_bps, px, adv_shares, info_share, cfg):
    raw = risk_R_usd / (stop_bps / 1e4 * px)
    return min(raw * info_share, cfg.adv_cap_pct / 100 * adv_shares)  # scale by conviction
```
...
align. Chan (1992) [documented]: index futures lead cash. Hasbrouck (1995) [documented]:
...
def emit(state, signals, cfg):
    ...
    if sig.lead_lag_sig and abs(sig.move_z) >= cfg.catchup_z and sig.info_share_ok:
        ...
        if cost <= cfg.cost_gate_k * sig.edge_bps:
            t = OrderTicket.intent(side, qty=size_catchup(...), limit=last_price())
...
|---|---|---|---|---|
| Chan (1992) | US index futures vs cash | futures lead cash documented | before cost | 1980s–90s data |
...
|---|---|---|---|---|---|---|
| R001 | stand-down | lead-lag breakdown | HY p ≥ 0.05 | no new catch-ups | immediate | restrictive |

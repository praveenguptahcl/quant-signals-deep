...
changelog:
  ...
  - "1.1.0 2026-09-10: deep review — calibration recipe (d grid + statsmodels ADF), canonical sizing signature, regime-gate machine records, GROK-NEEDED questions, capacity $ band, cfg-referenced pseudocode literals, cheapest-source tier split (free prototype / Stocks Advanced production), gate=0 acceptance test"
---
...
| Row | Value |
|---|---|
| Module | T057 — Fractional-Differentiation Memory Trader, v1.1.0 |
...
| risk_class | medium |
| Capacity | `$50–500M` [internal-est] — book-level on liquid US large-caps at `≤2`% ADV participation [example] |
| Executability | Easy: fracdiff is a weighted-sum filter (~50 lines [internal-est]); AR fitting is least squares; ADF via `statsmodels.tsa.stattools.adfuller` (statsmodels 0.15.x [documented]) |
...
| Regime gates | Provisional machine records in §T9: Hurst persistence gate (amplify) and stationarity gate (degrade→veto); `regime_ids[]` empty pending R001–R050 annotation pass |
...
2. **Cheapest-source row:** min_tier = **Free tier for the prototype** (`$0`/mo [unverified] — delayed EOD daily OHLCV bars, `5` calls/min [unverified], verify before budgeting); **Polygon.io Stocks Advanced for production** — `$199`/mo [documented] (real-time stock data + `20`+ years history + websockets, individual use — polygon.io/pricing, checked 2026-09-10). Endpoint: `/v2/aggs/ticker/{t}/range/1/day/...` [documented — Polygon.io usage pattern]. latency_class = bar / daily; required_fields = daily OHLCV bars; build_or_buy = **build — the fracdiff filter is ~50 lines; vendor ADF/memory statistics are rarely disclosed with methodology**.
...
### Position sizing (fenced function — canonical signature)

```python
def size_shares(risk_budget_R=500.0, stop_dist=1.0, vol_estimate=None, ADV_cap=None, cost=None):
    # Dollar-risk sizing. [example]
    # vol_estimate (bps/day), ADV_cap (shares), cost (expected bps) are accepted for the
    # canonical signature but do not bind in the reference build: participation is ~2% ADV
    # [example], impact = 0 [example] in the COST block, and sizing is purely stop-based.
    # A production build applies ADV_cap and vol-scaling before the cost gate.
    return int(risk_budget_R // max(stop_dist, 1e-9))
```
...
| Exits | Forecast sign flip or `time_stop_bars` (`5` [example]) time stop |
...
### Calibration recipe (d / ADF / Hurst / AR)

1. **d grid search** (monthly, purged + embargoed walk-forward): grid d ∈ [`0.10`, `0.15`, …, `0.90`] [example]; on the trailing `60` bars [example], compute fracdiff weights (S087) and run ADF via `statsmodels.tsa.stattools.adfuller(x, maxlag=None, regression='c', autolag='AIC')` (`statsmodels` 0.15.x [documented]; p-value is MacKinnon's approximate [documented]); select the minimum d with p `≤ adf_pmax` (`0.01` [default]). Freeze the estimate for the calendar month — per-bar re-estimation is the d-mining overfit path (§T9 row 4).
2. **Hurst gate:** estimate H on the same window (S090); trade only when H `≥ H_gate` (`0.53` [example]).
3. **AR fit:** least-squares AR(`ar_k`) (`4` [example]) on the stationary fracdiff series, using data `≤ t` only.
4. **Validation:** report DSR with the variant count; if DSR is not computed, say so (§T7 honest bottom line stands).
...
| C11 | Stationarity hygiene: ADF p `> adf_pmax` [default] at current d → no entries → check ADF monitor → action = hold FLAT → log `GATE_VETO` |
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        s = AR(cfg.ar_k) forecast f_t on fractionally differentiated log price  # data <= t only
        assert isfinite(s) else return UNKNOWN                      # F2
        edge_bps = abs(s) * 10.0                                   # [example] — modeled per-trade edge in bps
        ...
        if state.position is None and state.module_state == OK:
            ADF_stationary = (adf_p <= cfg.adf_pmax)               # C11
            if ((f_t >= cfg.theta and H >= cfg.H_gate and ADF_stationary) or
                (f_t <= -cfg.theta and H >= cfg.H_gate and ADF_stationary)) and cost_ok:
            ...
            if ((f_t crosses 0) or (bars_held >= cfg.time_stop_bars) or (H < cfg.H_gate)):
...
- Acceptance tests (`modules/tests/test_T057.py`, `7` tests):
  ...
  (qty / bps / $) match the generation run; `7.` `gate = 0` on the fixture's
  entry bar suppresses the entry ticket (guard-fold semantics: H, ADF, and
  cost gates fold into the canonical `gate` flag).
...
### Regime gates (machine records — provisional; §0 `regime_ids[]` empty pending R001–R050 annotation)

| regime_id | direction | mechanism | condition | action | min_lag | unknown_behavior |
|---|---|---|---|---|---|---|
| R-PROV-PERSIST | amplifies | Hurst persistence gate (S090) confirms the memory signal is tradable, not spurious | H `≥ H_gate` (`0.53` [example]) | veto — no entries unless met | `1` bar [default] | emit `UNKNOWN` (restrictive default) |
| R-PROV-STATIONARITY | degrades | ADF rejects stationarity of the fracdiff series at current d | `adf_p > adf_pmax` (`0.01` [default]) | veto — hold FLAT | `1` bar [default] | emit `UNKNOWN` (restrictive default) |
...
**GROK-NEEDED (deep-review questions the web could not answer):**

- G1. Has any public paper or public codebase documented *after-cost* P&L for AR momentum on fractionally differentiated prices? Need a citation or a "none found" confirmation — this drives the §1 `INSUFFICIENT-EVIDENCE` verdict.
- G2. What d values do practitioners typically select for daily US equities under the minimum-d-with-ADF-p<0.01 rule, and what estimation window is standard (`60` bars vs `252` bars)?
- G3. What is conventional MacKinnon-ADF lag-selection practice when testing fracdiff-series stationarity — AIC vs t-stat autolag, and `regression='c'` vs `'ct'`?
- G4. Which Hurst estimator (R/S, variance-ratio, DFA) is most defensible for daily financial series, and what H levels are reported for large-cap US equities?
- G5. Does Polygon.io's free tier still include 15-minute-delayed daily OHLCV bars with 2 years of history at `$0`/mo as of 2026-09-10? (The polygon.io/pricing fetch on 2026-09-10 confirmed only the Stocks Advanced `$199`/mo tier.)

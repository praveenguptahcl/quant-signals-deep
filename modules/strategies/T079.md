...
stage: 179
seed: 179
rng: "numpy.random.default_rng(179)"
provisional_regimes: ["R001", "R005"]
dependency_pins:
  python: ">=3.11"
  numpy: ">=1.26"
  pandas: ">=2.2"
  pytest: ">=8.0"
parents: [S100, S070, S064]
test_cmd: "python -m pytest modules/tests/test_T079.py -q"
...
- **Seed / RNG:** `seed=179`, `numpy.random.default_rng(179)` [measured]
- **Provisional regimes:** `R001`, `R005`
- **Parent signal(s):** `S100` (window anchor); `S070` (mispricing gauge); `S064` (fair-value estimate)
- **Universe:** US common stocks with liquid listed options (front-week straddle bid-ask ≤ 10% of mid [example]), price ≥ $20 [example]. Never holds through the announcement — trades the aftermath.
- **Latency class:** bar-clock / 5-minute; **cadence:** 5-minute; **tz:** America/New_York
- **sha256:** `REPLACE_ON_INGEST`
- **Test command:** `python -m pytest modules/tests/test_T079.py -q`
...
- **Schedule:** RTH on post-announcement days only; owner: vol-strategies on-call
- **Health:** two-sided-quote monitor; fixture replay on deploy; assignment-risk scan ex-div
- **Alerts:** page on 2×-credit stop or new material announcement [default]; notify on t+3 flatten
- **When this strategy dies:** No two-sided straddle quote within 10% width [default]; new material announcement; underlying halt
...
- **Chain snapshot:** Appendix d v1.0.0 (ChainSnapshot) — front-week ATM straddle mid, width, underlying price.
...
- Venue: listed US equity options / OPRA [example].
- short straddle has no stock borrow; hard-borrow names excluded via C7 (assignment/dividend risk)
...
on the gate-pass row (bar 0 [measured]): notional = 3.2 × 100 = $320.00 [example]:
...
- Data: listed US equity options / OPRA [example]; Research OPRA history via Databento ~$200/mo + usage [unverified]; production full OPRA institutional $$$$ [unverified]
...
as such. Research OPRA history via Databento ~$200/mo + usage [unverified]; production full OPRA institutional $$$$ [unverified] Confirm each feed's license before
...
- **Cheapest row:** min_tier = Databento OPRA research [unverified]; latency_class = 5-minute bar-clock; required_fields = straddle quotes, underlying bars, announcement timestamps; price_band = ~$200/mo + usage [unverified]; build_or_buy = build the comparator, buy OPRA history
...
|---|---|---|---|---|
| `R001` | degrades | vol-of-vol spike reprices the whole surface; 2× [example] stops correlate | rv_pctile > 85 [example] | halve |
| `R005` | inverts | second news event = repricing, not overpricing | new_announcement [default] | veto |

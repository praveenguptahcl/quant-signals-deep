...
changelog:
  ...
  - "1.1.0 2026-09-10: deep review — §1 edge-verdict enum fix + Tier S cost alignment; §1.5 grounded cheapest-source row (Stooq free / Tiingo $30-mo) and Tier S resource budget; §T0.2 full Config dataclass (17 params, status enum) covering every cfg field the reference harness uses; §T0.3 dual-timestamp mapping; §T2 domain block, mandated shares=f(...) sizing fn, expanded fill-model table, COST callable aligned to the test reference (one-way spread leg + taker fee), compliance C4/C10 tag fixes; §T3 normative pseudocode now wires the veto-multiple rule actually exercised by the tape (was z-entry-only in the harness); §T4 spread_hat_bps tape column + 7th veto-rule test; §T7 BEFORE-COST/AFTER-COST tokens; §T9 regime record enums fixed (direction=amplifies, action=veto); §T10 GROK-NEEDED added"
---
...
| Row | Value |
|---|---|
| Module | T029 — Spread-Estimate Edge Filter, v1.1.0 |
| Edge verdict | `PREDICTIVE-FEATURE-ONLY` — daily-data spread estimators (Roll 1984; Corwin–Schultz 2012; Hasbrouck 2009) are the documented calibration standard, but the module emits no standalone edge of its own |
| After-cost | `narrow` — a filter has no edge of its own — value = avoided losses minus vetoed winners |
| Build cost | Tier S · `5–15` h [documented] · ≈ `$750–2,250` loaded [documented] |
...
| Regime gates | R001 (provisional): high-spread regime amplifies veto activity → `veto`; restrictive default |
...
2. **Cheapest-source row:** min_tier = DAILY (OHLCV); latency_class = end-of-day;
   required_fields = daily open/high/low/close; price_band = `$0–300`/yr [documented]
   (Stooq daily bulk CSV free, `$0`/yr [documented] — `21,000+` securities [documented],
   `30+` yr history [documented], CSV download; escalation: Tiingo EOD API flat
   `$30`/mo ≈ `$300`/yr [documented]; free tier `500` symbols/mo [documented] —
   verify current pricing before budgeting); build_or_buy = **build**.
3. **Resource budget:** Tier S · `5–15` h [documented] · ≈ `$750–2,250` loaded [documented]
   (matches the §1 card); compute pattern `vectorized batch (polars/numpy)`.
...
6. **Escalation gate:** upgrade from cheapest when any holds — downstream strategies complain of over-vetoing; spread estimates drift from realized costs; Stooq terms/key friction (free key via on-site CAPTCHA as of early 2026 [unverified]) blocks the nightly pull.
...
One dataclass; every field the reference harness (`modules/tests/test_T029.py`)
and the §T3 pseudocode read is listed. Status ∈ {fixed, default, calibrate,
example, internal-est} per Appendix i v1.0.0.

| Parameter | Type | Default | Range | Status |
|---|---|---|---|---|
| sid | str | `"T029"` | — | fixed |
| primary_signal | str | `"S032"` | — | fixed |
| roll_window_days | int | `60` | [21, 252] | default |
| cs_window_days | int | `21` | [10, 60] | default |
| veto_multiple | float | `2.0` | [1.0, 4.0] | default |
| spread_floor_bps | float | `1.0` | [0.5, 5.0] | default |
| spread_full_bps | float | `4.0` | [1.0, 20.0] | example — calibrated-constant fallback when no live S032 estimate is available |
| cost_gate_k | float | `0.50` | [0.1, 1.0] | default |
| z_entry | float | `2.0` | [1.0, 3.0] | default — downstream trigger modeled by the reference harness, not the filter |
| z_exit | float | `0.8` | [0.2, 1.5] | default — downstream trigger modeled by the reference harness, not the filter |
| risk_R_usd | float | `250.0` | [50, 5000] | example |
| stop_bps | float | `25.0` | [5, 200] | default |
| adv_cap_pct | float | `1.0` | [0.1, 10.0] | default |
| taker_fee_bps | float | `0.30` | [0, 2.0] | example |
| maker_rebate_bps | float | `-0.20` | [-1.0, 0.5] | example |
| side_exec | str | `"taker"` | {taker, maker} | default |
| borrow_bps | float | `0.0` | [0, 50] | example — filter is direction-agnostic; forwarded SHORT intents carry borrow via the downstream module |
| impact_k | float | `0.0` | [0, 2.0] | example — filter adds no impact |
| daily_loss_stop_pct | float | `1.0` | [0.25, 3.0] | default |
| venue | str | `"primary"` | — | example |
| default_side | str | `"LONG"` | {BUY, SELL, SHORT, LONG} | default |

`spread_hat_bps` source priority: live S032 estimate (`max(roll, cs)`) → calibrated
constant `spread_full_bps / 2` (cheap path, [example]) → neither available:
`UNKNOWN`, fail closed (veto all), per F1.
...
| `roll_spread_bps`, `cs_spread_bps` | Signal: S032 | estimator legs; tape column `spread_hat_bps` plays this role [example] |
| `event_ts`, `asof_ts` | Bar/Event | int64 ns UTC; assert `asof_ts ≥ event_ts` (F2) |

### T0.4 Time contract

> Time contract: clock domain UTC, int64 ns; authoritative causality clock =
> `event_ts`; max_skew = `5` s [default]; jitter budget = `1` s [default];
> bar-label = open time; dual timestamps `(event_ts, asof_ts)` with `asof_ts ≥ event_ts`
> asserted; staleness TTL = `15` min [default]; gap policy = mask, no
...
| Field | Value |
|---|---|
| Asset class | US equities (estimators are daily-OHLC based; portable to any OHLC market) |
| Venues | primary listing exchange; filter is venue-agnostic |
| Session | end-of-day batch, America/Chicago |
| Calendar | NYSE trading calendar; no signal on holidays/early closes [default] |
| Settlement | T+1 [documented] |

### Entry rule (Boolean — no prose-only conditions)

```
spread_hat_bps = max(roll_spread_bps, cs_spread_bps, spread_floor_bps)  # floor 1.0 [default]
VETO  = edge_bps < veto_multiple * spread_hat_bps                       # 2.0 [default]
...
post-exit cooldown: n/a at the filter — enforced by the downstream strategy;
  the filter logs GATE_VETO with code C10 when a downstream re-entry arrives
...
def shares(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost):
    """Forwarded-intent sizing. The filter never enlarges an intent: it passes
    or vetoes. `cost` is enforced by the §T3 cost-gate predicate, not inside
    sizing; `vol_estimate` is accepted for signature conformance and unused
    (the filter sizes on stop distance, not volatility)."""
    raw_qty = risk_budget_R / max(stop_distance, 1e-9)  # stop_distance in $/share
    return max(1, min(int(raw_qty), int(ADV_cap)))
```

Mapping on the reference tape: `risk_budget_R` = `cfg.risk_R_usd` (`250` [example]);
`stop_distance` = `stop_bps / 1e4 × close` $/share; `ADV_cap` = `adv_cap_pct / 100 × adv_shares`
(`1.0%` [default] of `2,000,000` = `20,000` shares [example] on the tape).

```python
def filter_intent(intent, edge_bps, spread_hat_bps, cfg):
    # the filter never sizes: it passes or vetoes
    if edge_bps < cfg.veto_multiple * spread_hat_bps:
        return None            # vetoed
...
| Daily loss stop | `1.0%` of equity → no new entries; state `OFF` [default] |
| Max gross | `4×` equity, hard cap [default] |
...
|---|---|---|
| Spread (one-way leg of the round-trip estimate) | `spread_full_bps / 2 = 2.0` | [example] — the filter's whole job |
| Fees (taker) | `0.30` | [example] |
| Borrow | `0.00` | direction-agnostic: the filter carries no borrow leg [example] |
| Impact | `0.00` | the filter itself adds no impact [example] |
| **Total reference** | `2.3` | [example] |

`fee_schedule_as_of`: `2026-09-01` [unverified] — placeholder; pin the real venue
schedule before production. `side`: `mixed` [default]; venue/tier/source:
n/a (filter) [default]. `borrow_bps_per_day`: `0.0` [example] — filter is
direction-agnostic; forwarded SHORT intents carry their borrow via the
downstream module (C7).

```python
def expected_cost_bps(notional, adv_pct, venue, side, urgency, cfg):
    """COST-block callable. Prices the one-way spread leg + fee. Identical to
    the reference implementation in modules/tests/test_T029.py."""
...
| Latency | end-of-day batch; intents emitted after the close — no intraday latency budget [example] |
| Slippage model | realized effective spread tracked for monthly calibration |
| Partial fills | n/a — filter forwards at the downstream's fill model |
| Impact | filter adds none (`impact_k = 0.0` [example]) |
| Adverse-fill | n/a — adverse selection lives in the downstream strategy's fill model |
...
| Spread floor | s_floor | `1.0` bp | [default] | low — only binds when estimators collapse |
...
def emit(state, signals, cfg):
    ...
    if sig is None or invalid(sig):
        return [], state.unknown()          # F1: invalid input -> UNKNOWN, never interpolate
    if market_state() != "CONTINUOUS_TRADING":
        return [], state.unknown()          # F2: halt/auction per §T0.5
    spread_hat = max(sig.roll_spread_bps, sig.cs_spread_bps, cfg.spread_floor_bps)
    if sig.edge_bps < cfg.veto_multiple * spread_hat:
        log("GATE_VETO", reason="veto-multiple",
            edge=sig.edge_bps, spread=spread_hat)
        return [], state.ok()               # the §T2 veto rule, enforced
    cost = expected_cost_bps(sig.notional, sig.adv_pct, cfg.venue,
                             cfg.side_exec, sig.urgency, cfg)
    # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
    if cost > cfg.cost_gate_k * sig.edge_bps:
        log("GATE_VETO", reason="cost-gate", cost=cost, edge=sig.edge_bps)
        return [], state.ok()
    t = OrderTicket.intent(sig.side, qty=sig.qty, limit=sig.limit)  # forward unchanged
...
- Tape: `modules/fixtures/T029_tape.csv` — `# TYPE: validation-run`;
  ...
  UTC; `spread_hat_bps` column carries the S032 spread estimate [example]
  (the reference harness wraps the normative filter around a minimal
  downstream z-entry/z-exit trigger so the tape exercises pass/veto paths);
  every number synthetic.
...
- Acceptance tests (`modules/tests/test_T029.py` — `7` [default] tests):
  ...
  7. veto-multiple rule: `edge_bps < m × spread_hat_bps` → `veto-block`, no intent.
...
| `7` | notional=`100000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `2.0` + fee `0.3` + borrow `0.0` + impact `0.0` = `2.3` bps → `$23.0` | `0.05` edge, k=`0.5` [default] → VETO (`0.05 < 2.0×2.0`); gate `2.3 ≤ 0.025` also BLOCKS | `HOLD` / `OK` |
...
**Hand-checks.** Row `3` (entry): qty = `250 / (25/1e4 × 99.55)` = `1004` raw → ADV-capped at `20000` → `1004` shares [example]; ticket `T0003`, side `LONG`, `marketable` intent. Row `6` (exit): |z| through the exit band → `BUY` intent, exits never cost-gated [default]. Row `7` (veto): edge `0.05` bps [example] < veto bound `2.0 × 2.0 = 4.0` bps → `veto-block`, no intent (gate `2.3 ≤ 0.025` BLOCKS too); state stays `OK`. Row `9` (kill): daily P&L `-1.1%` [example] ≤ −stop (`1.0%` [default]) → `ARMED → TRIPPED`, state `OFF`, no intents. Causality: entry intent at row `3` has `intent_ts` = bar-3 `event_ts`; the earliest fill is bar `4`'s open — the test asserts `fill_event > signal_event` on every intent row.
...
|---|---|---|---|---|
| Roll (1984) | model | implicit spread from serial covariance | BEFORE-COST | breaks under trending flow (Harris 1990) |
...
|---|---|---|---|---|---|---|
| R001 | amplifies | high-spread regime widens spread_hat → veto rate rises | spread_hat > 2× trailing median [example] | veto (raise veto_multiple `2.0 → 3.0` [example]) | `1 day` [default] | restrictive — veto all [default] |
...
**GROK-NEEDED** (exact questions web search could not answer — do not invent answers):

1. For US large-cap equities, what veto-multiple ranges (edge ÷ estimated round-trip spread) do live pre-trade cost filters actually calibrate to, and how is the multiple re-fit (walk-forward window, shrink rule) when realized shortfall drifts from the Roll/Corwin–Schultz estimate?
2. On 2024–2026 US equity data, what is the measured bias of the Roll estimator vs realized effective spreads in trending vs mean-reverting flow regimes (Harris 1990 warns of breakdown — is there a published refit)?
3. Is there any published desk-level methodology for attributing P&L to *avoided* losses from a cost-filter veto (vetoed-paper-trade attribution), or is filter value always reported qualitatively?
4. For the monthly recalibration (§T2), which realized-effective-spread definition do practitioners standardize on (Lee–Ready, bulk-volume classification, exchange-flagged), and what tolerance on estimate-vs-realized drift triggers a forced refit?
...
> DAILY (OHLCV); compute pattern `vectorized batch (polars/numpy)`.

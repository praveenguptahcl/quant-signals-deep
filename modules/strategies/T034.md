...
| Row | Value |
|---|---|
| Module | T034 — Index Futures Cash-and-Carry, v1.0.0 |
| Edge verdict | `plausible` — cash-and-carry is textbook arbitrage (Hull) with exchange-documented mechanics (CME) |
...
| Data cost | index + futures + rates: low four figures/yr [unverified] |
...
| Regime gates | widen the trigger in dividend season (forecast error); stand down when borrow specials spike |
...
|---|---|---|
| index level | Bar | spot S |
| futures price | Bar | F per expiry |
| rate/dividend | Bar | r, q forecasts |
...
ENTRY rich: basis_bps >= trigger_bps -> long basket / short future
ENTRY cheap: basis_bps <= -trigger_bps -> short basket / long future
```

### Exits (with post-exit cooldown)

```
converge -> |basis_bps| <= 1.0 [example] -> unwind both legs
expiry   -> T < 2 days [example] -> roll or unwind
...
```python
def size_carry(risk_R_usd, stop_bps, index_px, cfg):
    # balance-sheet sized: notional = risk budget scaled by balance-sheet limit
    raw = risk_R_usd / (stop_bps / 1e4 * index_px)
    return min(raw, cfg.balance_sheet_notional / index_px)
```
...
| Fees (both legs) | `0.80` | [example] |
| Borrow/financing | `3.00` | spread over r [example] |
| **Total reference (one-way)** | `~`6.3` round-trip [example]` | [example] |
...
futures exchange + primary equity [example].
...
| Aspect | Assumption |
|---|---|
| Fill venue | futures exchange + equity basket algo |
| Slippage model | basis at execution vs signal basis |
| Partial fills | basket completion algo; future leg hedges residual |
| Halt behavior | no new basis trades on limit moves |
...
def emit(state, signals, cfg):
    ...
    if state.position != 0:
        if abs(sig.basis_bps) <= 1.0 or sig.expiry_near or stopped(state, cfg):
            return exit_tickets(state), state.flat()
        return [], state.ok()
    if abs(sig.basis_bps) >= cfg.trigger_bps and sig.financing_ok:
        ...
        if cost <= cfg.cost_gate_k * sig.edge_bps:
            t1 = OrderTicket.intent(side_pair[0], qty=size_carry(...), limit=px_basket())
            t2 = OrderTicket.intent(side_pair[1], qty=size_carry(...), limit=px_future())
...
|---|---|---|---|---|---|---|
| R001 | widen | dividend season | announcement window | trigger 5→8 bps | 1 day | restrictive |
| R002 | stand-down | financing stress | borrow specials spike | no new carry | immediate | restrictive |

```mermaid
flowchart LR
    SIG["S057 basis + S098 financing"] --> B{"|basis| ≥ 5bps?"}
    B -->|yes| G{"cost gate"}
    B -->|no| WAIT["wait"]
...
> DAILY + FUTURES; compute pattern `vectorized batch (polars/numpy)`; u.

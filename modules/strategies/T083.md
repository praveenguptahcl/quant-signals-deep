...
stage: 183
seed: 183
rng: "numpy.random.default_rng(183)"
...
- short clips 2–5 day hold at locate rate: ~0.5 bps/day [example]; locate verified at entry (C7)
...
shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost)
    qty = 200                              # fixed clips [example]
    qty = min(qty, 0.10 * visible_depth)   # 10% of depth [default]
```
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    if enter:
        ...
        t = OrderTicket(sym, side, cfg.clip_shares, b.touch_px, DAY, uuid(), f'S020@{b.close_ts}', now(), NEW)
...
|---|---|---|---|---|
| `R001` | degrades | vol spike → flow turns informed; theta gate does the work | theta > cap [default] | pause_entry |
...
```
You are an AI engineer implementing strategy module T083 (Retail-Flow Internalizer Fade).

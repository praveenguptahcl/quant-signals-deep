...
datasets:
  - name: US equities 1-min bars
    vendor: Polygon.io (Massive, fka Polygon.io)
    product: Stocks Advanced
    price_band: "$79-199/mo (third-party docs — Massive 2026 pricing post-rebrand; verify at massive.com)"
    schema_version: "1"
...
changelog:
  ...
  - "1.0.1 2026-09-11: deep review — §0 regime_ids [R008, R-halt] (provisional) + dataset price_band; §1/§1.5 data cost re-pinned to Massive (fka Polygon.io) tier names (Stocks Developer $79/mo delayed, Stocks Advanced $199/mo real-time, third-party docs, verify before budgeting); §T0.7 de-duplicated max_adverse_per_trade tags; §T2 sizing function to full 5-arg form shares=f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost); §T2 COST: venue corrected to XNAS (data vendor is not a venue), spread basis re-pinned to the $250 fixture, fee component anchored to the Nasdaq $0.0030/share removal fee (compendium-wide documented anchor), borrow_bps_per_day reason given; §T3 pseudocode: added kill-state + cooldown guards; §T9: machine-readable regime-gate records (R008, R-halt); §T10: what-would-change-our-mind + GROK-NEEDED block."
---
...
| Row | Value |
|---|---|
| Module | T066 — VWAP-Cross Institutional Follower, v1.0.1 |
...
| Data cost | Research `$79`/mo [unverified]; production `$199`/mo [unverified] — Massive (fka Polygon.io) 2026 tiers: Stocks Developer (15-min delayed 1-min bars) for research, Stocks Advanced (real-time) for production; third-party docs, verify at massive.com before budgeting |
...
| Regime gates | R008 (range compression — degrades, pause_entry), R-halt (halt/auction — inverts, veto); machine records in §T9 |
...
2. **Cheapest-source row:** min_tier = Massive (fka Polygon.io) Stocks Developer — cheapest data source candidate [internal-est]; latency_class = bar / 1-minute; required_fields = 1-min OHLCV bars; price_band = `$79–199`/mo [unverified] (Stocks Developer 15-min delayed for research, Stocks Advanced real-time for production — third-party docs, verify at massive.com); build_or_buy = **build — VWAP is arithmetic; spend on the persistence definition**.
...
risk_contract:
  ...
  max_adverse_per_trade: `0.5` ATR beyond VWAP [example]
...
`POLYGON_API_KEY` via environment only (vendor rebranded to Massive in 2026;
existing keys and `api.polygon.io` endpoints remain backward-compatible [unverified]).
No secrets in this file, fixtures, or logs
(CI secrets-grep).
...
def shares(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost):
    # Normative sizing: dollar-risk over stop distance, capped by
    # participation, cost-aware. All inputs in USD except where noted.
    n_risk = risk_budget_R / max(stop_distance, 1e-9)          # base sizing
    n_part = ADV_cap * vol_estimate                            # participation cap (shares)
    n_cost = risk_budget_R / max(cost, 1e-9) if cost > 0 else n_risk
    return max(1, int(min(n_risk, n_part, n_cost)))             # [default] floor 1 share
```
Reference instantiation: `risk_budget_R = $400` [default],
`stop_distance = 0.5 × ATR` [example], `ADV_cap = 0.10` [default],
`cost = expected_cost_bps(...) × price / 1e4` [default].
...
|---|---|---|
| Spread | `1.50` | [example] — ≈ `4`¢ effective spread [example], half crossed, on a `~$250` stock [example] (fixture-consistent) |
| Fees | `1.20` | [example] — ≈ `$0.0030`/share Nasdaq default removal rate [documented] on `400` shares [example] of `~$250` [example] = `~$100,000` notional [example] |
| Borrow | `0.0` | [default] — intraday holds only; any overnight short accrues real borrow via C7 |
| Impact | `0.0` | [example] — flagged |
| **Total reference** | `2.70` | [example] |

`fee_schedule_as_of`: `2026-09-01` [unverified] — placeholder; pin the real
venue schedule (e.g. Nasdaq Rule 7018 pricing) before production.
`side: mixed` [default]; venue: XNAS (data vendor is not a venue); data
source: Massive (fka Polygon.io) Stocks Advanced [example].
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    spread_bps = 1.5   # [example] — ~`4`c effective spread, half crossed, on a ~`$250` stock
    fee_bps = 1.2      # [example] — ~`$0.0030`/share Nasdaq default removal [documented] on `400` shares of ~`$250`
    borrow_bps = 0.0   # [default] — intraday holds only
...
function emit(state, signals, cfg) -> list[OrderTicket]:
    ...
    for bar t in signals (newest last):
        ...
        if state.position is None and state.module_state == OK:
            assert state.kill_state == ARMED                      # kill guard (§T0.7)
            if t.event_ts < state.cooldown_until:                 # C10 post-exit cooldown
                continue
            if ((beyond VWAP >= `3` bars [example]) and (vol >= `1.3`x median [example])) and cost_ok:
...
|---|---|---|---|---|---|---|
| R008 | degrades | range compression pins price to VWAP; crosses are whipsaw, not flow | bandwidth_pctile < 20 [default] | pause_entry | 1 bar [default] | treat as degrades |
...
- Instinet Intraday VWAP Cross (Institutional Investor, c. 2010) — a broker crossing venue matching at VWAP; execution infrastructure, not alpha evidence [unverified — not independently verified this review].

**What would change our mind:** a purged/embargoed walk-forward of the
persistence-filtered VWAP-cross rule with full costs showing a DSR-significant
after-cost edge outside the original sample; or a documented implementation of
the cross-as-signal rule (not VWAP as benchmark) with published after-cost P&L.

**GROK-NEEDED** (expert questions web search could not resolve — do not answer from priors):
1. "T066 (persistence-filtered VWAP-cross follower, US large caps, 1-min bars, intraday only, liquid names) gates entries with `expected_cost_bps(...) <= k * edge_bps`, k default `0.5` [default]. Is there any published fee-covering multiple guidance for VWAP-benchmarked intraday strategies — e.g., a documented k in {`0.5`, `1`, `2`} used by a desk or paper for VWAP-relative or momentum-follower strategies? A good answer cites the source and the exact k with its cost-stack definition."
2. "Is there any published evidence (paper, desk study, or documented strategy) that a *persistence-filtered* VWAP-cross rule — price beyond VWAP for `3`+ consecutive bars [default] with volume confirmation — earns positive after-cost alpha intraday, as opposed to VWAP-reversion (fading deviations)? A good answer distinguishes cross-following from cross-fading evidence and gives the exact persistence/volume definitions and cost assumptions used."
3. "T066's reference cost stack is `2.7` bps [example] on `~$100,000` notional [example] (`$250` stock [example], `400` shares [example]): `1.5` bps spread (≈`4`¢ effective, half crossed) + `1.2` bps fees (≈`$0.0030`/share Nasdaq default removal). For a non-tiered prop/retail router hitting XNAS on US large caps in 2026, what is a realistic all-in one-way cost per share (exchange fee + routing + ECN pass-through) at `~$250` prices, and is `2.7` bps conservative, realistic, or optimistic? A good answer gives the component breakdown with as-of dates."
4. "For T066's exit (VWAP recross or `0.5`-ATR stop [example] on 1-min bars), is there any documented adverse-exit haircut for VWAP-following strategies — i.e., how many bps of the entry edge typically survive the recross exit after slippage on fast recrosses? T066 currently models exits as instant [example]. A good answer cites a paper or desk study with numbers."

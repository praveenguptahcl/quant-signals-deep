...
changelog:
  - "1.1.0 2026-09-10: deep review — §1 edge verdict mapped to the locked enum (AFTER-COST-EDGE/narrow); §1.5 cheapest-source row made concrete (Ken French 49-industry daily returns [free, documented], EODHD $19.99/mo [documented]); Config dataclass aligned with test (z_entry 1.25 [documented], z_exit 0.50 [documented], cost-component params added); risk-contract daily_loss_stop 2%→1.5% to match Config [default]; §T3 pseudocode hardened with market-state/cooldown/locate guards; OrderTicket sides restricted to BUY|SELL|SHORT per Appendix C; fixture bar 2 note flat→gate-block; COST block fee_schedule_as_of anchored to Nasdaq price list 2026-09-10; Barroso–Santa-Clara added to source log; GROK-NEEDED section added; typos fixed (`±±`, stray `u.`)"
...
| Row | Value |
|---|---|
| Module | T038 — Sector Momentum + Idiosyncratic Fade, v1.1.0 |
| Edge verdict | `AFTER-COST-EDGE` — industry momentum (Moskowitz–Grinblatt 1999) and eigenportfolio reversal (Avellaneda–Lee 2010) are both documented |
...
| Data cost | `$0–240`/yr on the cheapest path — Ken French 49-industry daily returns free [documented], EODHD EOD plans from `$19.99`/mo [documented]; production venue + borrow feeds higher [unverified] |
...
2. **Cheapest-source row:**
   ...
   | Field | Value |
   |---|---|
   | min_tier | DAILY (OHLCV) + reference sector map |
   | latency_class | end-of-day |
   | required_fields | daily industry returns (49-industry series), daily OHLCV for the tradable universe |
   | vendor / product | Ken French Data Library — 49 Industry Portfolios, daily returns 1926-07-01 → 2026-06-30 [documented], free download; EODHD end-of-day API for OHLCV, plans from `$19.99`/mo [documented] |
   | price_band | `$0–240`/yr for the prototype path [documented]; production venue + borrow feeds higher [unverified] |
   | build_or_buy | **build** the sleeves; the sector map itself is bought-or-free, never hand-built |

   The French 49-industry daily series (assigning each NYSE/AMEX/NASDAQ stock to an
   industry by 4-digit SIC code at end-June of year t, held July(t)→June(t+1) [documented])
   covers sleeve A's formation sort with zero data cost; only sleeve B's stock-level
   OHLCV needs a paid feed. Sector map rights: French data is a free academic library;
   a production GICS map is licensed per-seat — see GROK-NEEDED for the price band.
...
4. **Skip list:** intraday factor timing; multi-factor optimization; hand-rolling a sector map (use French 49-industry free series).
...
6. **Escalation gate:** upgrade from cheapest when any holds — sleeve correlation spikes; residual half-lives stretch; borrow data needed for the short sleeve (paid borrow feed, see GROK-NEEDED).
...
| residual_entry_z | float | `1.25` | [0.75, 2.0] | [documented] — Avellaneda–Lee s-score open |
| residual_exit_z | float | `0.50` | [0.25, 0.75] | [documented] — Avellaneda–Lee close |
...
| adv_cap_pct | float | `1.0` | (0, 25] | [example] |
| spread_full_bps | float | `3.0` | [1.0, 10.0] | [example] |
| taker_fee_bps | float | `0.30` | [0.0, 0.5] | [documented] — Nasdaq `$0.0030`/share removal fee at `~$100` [internal-est] |
| maker_rebate_bps | float | `-0.20` | [-0.5, 0.0] | [example] |
| borrow_bps | float | `50.0` | [0, 500] | [example] — per round trip on short legs |
| impact_k | float | `15.0` | [5.0, 30.0] | [example] — monthly rebalance |
| side_exec | str | `taker` | {taker, maker} | [default] |
| crowd_cap | float | `2.0` | [1.5, 3.0] | [default] — crowding multiple over median |

All defaults tagged per the tag law. Status `documented` values are the
Avellaneda–Lee (2010) s-score conventions, independently confirmed in the
deep-review web pass (entry |s| > 1.25 / exit |s| < 0.50, see §T10).
...
|---|---|---|
| `c` | Bar.close | return panel input |
| `industry_returns` (French 49) | Bar-derived feature | daily industry return series, value-weighted [documented] |
| sector map | reference | GICS-class [example]; cheapest = French SIC-based 49-industry map (free) |
| `signal_z`, `edge_bps` | SignalVector fields | from consumed S-modules (S062/S039/S080), newest last |
| `notional`, `adv_pct`, `adv_shares`, `stop_bps`, `urgency` | OrderTicket sizing inputs | per-bar execution context |
| `market_state` | Bar.market_state | CONTINUOUS_TRADING / HALTED / AUCTION / CLOSED |
| `event_ts`, `asof_ts` | Bar.event_ts / Bar.asof_ts | int64 ns UTC; `asof_ts >= event_ts` else UNKNOWN |
...
Formation ranks use the Ken French 49-industry daily series (`N = 49` [documented];
Moskowitz–Grinblatt (1999) used 20 industries with top/bottom-3 formation [documented]):

```
sleeveA_long  = (sector_rank <= 3) AND (now >= last_rebalance + 1_month) AND cost_gate
sleeveA_short = (sector_rank >= 47) AND (now >= last_rebalance + 1_month) AND cost_gate AND locate_ok
sleeveB_fade  = (|z_resid| >= 1.25) AND (|z_resid| <= 4.0 [default]) AND (crowding < crowd_cap)
                AND cost_gate AND (side==BUY OR locate_ok)
entry         = (sleeveA_long OR sleeveA_short OR sleeveB_fade)
                AND market_state==CONTINUOUS_TRADING
                AND (now >= cooldown_until) AND kill_ARMED AND not sig.crash_signal
```

Sleeve A ranks sectors by 6-month formation return [default]; sleeve B fades the
stock-specific PCA residual via the s-score. `cost_gate` is the §T3 predicate;
risk exits in the exit block are never cost-gated [rule].
...
cooldown_until = exit_ts + 1 session [default]   # post-exit re-entry lock (C10)
```

Exits are never cost-gated [rule]; risk exits (crash/crowding) flatten the
affected sleeve and start the cooldown before any re-entry.

### Position sizing (fenced function)

```python
def size_sleeve(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost):
    """shares = f(risk_budget_R, stop_distance, vol_estimate, ADV_cap, cost).
    stop_distance: fractional stop per share, stop_bps/1e4 × px [example].
    vol_estimate: annualized vol [default]; unused by the risk-parity base
        rule, reserved for vol-scaled variants.
    cost: not deducted from size [default]; entry is gated by the COST-block
        predicate instead. Risk-parity across sleeve positions is handled by
        the caller through risk_budget_R.
    """
    raw = risk_budget_R / max(stop_distance, 1e-9)
    return int(min(raw, ADV_cap))
```

With `risk_budget_R = 250` [example], `stop_distance = 25/1e4 × 100.07` [example],
`ADV_cap = 1.0% × 2,000,000` [example]: `250 / 0.250175 ≈ 999` shares [example].
...
| Fees | `0.30` | [documented] — Nasdaq price list: `$0.0030`/share to remove liquidity; bps conversion at `~$100` share price [internal-est] |
| Borrow (short sleeve) | `50.00` | [example] — per round trip on short legs |
...
`borrow_bps_per_day`: `0.0` — this build models borrow as a per-round-trip
charge on short legs (see `borrow_bps` above), not a daily accrual; reason: sleeve
rebalances monthly and the fixture resolves to SHORT legs only via `default_side`
[default]. If the production sleeve holds shorts multi-day, replace with a dated
daily borrow curve (see GROK-NEEDED for real borrow-rate sources).

`fee_schedule_as_of`: `2026-09-10` [documented] — Nasdaq price list
(nasdaqtrader.com, accessed 2026-09-10 in the deep-review web pass): fee to
remove liquidity `$0.0030`/share for executions ≥ `$1.00` [documented]; fee to
remove sub-`$1.00` liquidity `0.30%` of dollar volume [documented]. `side`:
`mixed`; venue/tier/source: primary lit [example] — the strategy emits intents
only; the executor's venue choice fixes the tier.
...
def emit(state, signals, cfg):
    ...
    if sig is None or invalid(sig):
        return [], state.unknown()          # F1/F2: invalid -> UNKNOWN, never interpolate
    if sig.market_state != "CONTINUOUS_TRADING":
        return [], state.halt()             # §T0.5: cancel working intents, emit nothing
    if state.kill != "ARMED":
        return [], state                         # TRIPPED/RECOVERY: OFF, no intents
    if sig.crash_signal or sig.crowding > cfg.crowd_cap:
        return exit_tickets(state), state.flat()   # risk exits never cost-gated [rule]
    tickets = []
    for pos in target_positions(sig, cfg):          # both sleeves, §T2 Boolean entry
        if now < state.cooldown_until:
            log("GATE_VETO", code="C10"); continue  # post-exit cooldown
        if pos.side in ("SELL", "SHORT") and not pos.locate_ok:
            log("GATE_VETO", code="C7"); continue   # Reg-SHO locate check
        cost = expected_cost_bps(pos.notional, adv_pct(), venue(), "taker", "low", cfg)
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
        if cost <= cfg.cost_gate_k * pos.edge_bps:
            t = OrderTicket.intent(pos.side, qty=size_sleeve(...),
                                  limit=(last_price() if cfg.side_exec == "maker" else None),
                                  ticket_id=new_ticket_id(), parent_signal=sig.ref)
            t.intent_ts = sig.computed_at
            assert fill_event > signal_event  # t->t+1 causality: no-signal-bar fills
            log("EMIT_INTENT", ticket=t.ticket_id)
            tickets.append(t)
        else: log("GATE_VETO", code="C2", cost=cost)
...
- Acceptance tests (`modules/tests/test_T038.py` — `7` [default] tests):
  ...
  7. Avellaneda–Lee entry/exit conventions — entry `|z| ≥ 1.25`, exit `|z| ≤ 0.50` [documented].
...
| `2` | same inputs | `2.86` bps → `$28.61` | `2.0` edge, k=`0.5` [default] → `2.86 ≤ 1.0` BLOCK (entry candidate: `|z|=1.6 ≥ 1.25` [documented]) | `HOLD` / `OK` (gate-block) |
| `3` | notional=`100000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.06` = `2.86` bps → `$28.61` | `11.442640687119285` edge, k=`0.5` [default] → `2.86 ≤ 5.72` PASS | `BUY` / `OK` |
| `6` | notional=`100000` [example], adv_pct=`0.5` [example], venue=`primary`, side=`taker`, urgency=`normal` | spread `1.5` + fee `0.3` + borrow `0.0` + impact `1.06` = `2.86` bps → `$28.61` | `3.0` edge, k=`0.5` [default] → `2.86 ≤ 1.5` BLOCK | `SELL` / `OK` (exit — never cost-gated [rule]) |
...
**Hand-checks.** Row `3` (entry): qty = `250 / (25/1e4 × 100.07)` = `999` raw → ADV-capped at `20000` → `999` shares [example]; ticket `T038-0003`, side `BUY` (Appendix C v1.0.0: `BUY | SELL | SHORT` only), `marketable` intent (`limit=None` under `side_exec=taker` [default]). Row `6` (exit): |z| = `0.25` through the exit band (`≤ 0.50` [documented]) → `SELL` intent covering the long; exits never cost-gated [rule]. Row `7` (gate block): edge `0.05` bps [example] vs cost `2.86` bps → gate `2.86 ≤ 0.025` BLOCKS → no intent, state stays `OK`. Row `9` (kill): daily P&L `-1.6%` [example] ≤ −stop (`1.5%` [default]) → `ARMED → TRIPPED`, state `OFF`, no intents. Causality: entry intent at row `3` has `intent_ts` = bar-3 `event_ts`; the earliest fill is bar `4`'s open — the test asserts `fill_event > signal_event` on every intent row.
...
|---|---|---|---|---|---|---|
| R001 | degrades | momentum crash | crash signal fires | pause_entry (flatten sleeve A) | immediate | restrictive |
| R002 | degrades | crowded residuals | crowding > 2× median [default] | halve (sleeve B) | 1 day | restrictive |
...
6. Barroso, P. & Santa-Clara, P. (2015). "Momentum Has Its Moments." *Review of Financial Studies* 28(4), 1118–1169 — cited for the §1 momentum-crash risk; the crash-stop rule derives from this result.

**Web-pass confirmations (2026-09-10 deep review).** Avellaneda–Lee (2010) s-score conventions independently confirmed: open long at s < −1.25 / open short at s > +1.25, close at |s| < 0.50 (quoted in the stat-arb survey at arxiv.org/pdf/1811.00200 §5, Eq. 6, citing the paper). Ken French Data Library: 49 Industry Portfolios daily returns, July 1 1926 – June 30 2026, free download; construction assigns each NYSE/AMEX/NASDAQ stock to an industry by 4-digit SIC code at end-June of year t, held July(t)→June(t+1) (mba.tuck.dartmouth.edu data library, accessed 2026-09-10). Nasdaq price list: fee to remove liquidity `$0.0030`/share (≥$1.00 executions), `0.30%` of dollar value below `$1.00` (nasdaqtrader.com price list, accessed 2026-09-10). EODHD end-of-day API: plans from `$19.99`/mo (eodhd.com, accessed 2026-09-10).
...
## GROK-NEEDED — exact questions for the web-unanswerable gaps

(Research answers land in the Q-batch; anything still unverifiable stays in §T10 unverified leads. Do not assert these in evidence tables.)

1. **Borrow-rate reality for the short sleeve.** What retail-accessible US-equity borrow-rate sources exist as of Sept 2026 (ORTEX, S3 Partners, IHS Markit, broker HTB lists via API), what are their subscription price bands, and what daily borrow rates (bps/day) are representative for large-cap vs hard-to-borrow names? This decides whether `borrow_bps_per_day` stays a modeled round-trip charge or becomes a dated daily curve.
2. **GICS sector-map licensing.** What is the current (2026) per-seat or research-use price band for a production GICS classification feed (S&P/MSCI), and is there a licensed daily point-in-time sector history product? This pins the §1.5 escalation-gate cost for moving off the free French SIC map.
3. **Fee-schedule tier realism.** For a monthly-rebalance strategy trading ~$100k notionals per name on primary lit venues, which fee tier actually applies on Nasdaq/NYSE at that volume, and has the `$0.0030`/share removal fee changed since the 2026-09-10 price-list read? Confirm the maker rebate the same volume band earns.
4. **Crowding-monitor data.** Which short-interest / utilization datasets (FINRA, Markit Securities Finance, broker consortium) can feed the sleeve-B crowding heat cap at daily cadence, at what price, and what is the publication lag? The `min_lag: 1 day` assumption in §T9 rides on this.
5. **Stat-arb factor-model practice.** For PCA eigenportfolio stat-arb on US large caps circa 2024–2026: what estimation windows (252d?), factor counts (10–15?), and z-score entry/exit thresholds are practitioners publishing now, and has the post-2007 decay profile in §T7 changed? Needed to calibrate the `[default]` ranges in §T0.2.
...
> Ken French 49-industry daily series (free) for sleeve A + EODHD (from
> `$19.99`/mo) for OHLCV; compute pattern `per_bar` vectorized batch (polars/numpy).

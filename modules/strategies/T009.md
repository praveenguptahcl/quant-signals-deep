...
kind: "strategy"
version: "1.0.2"
test_cmd: "python3 -m pytest modules/tests/test_T009.py -q"
...
datasets:
    ...
    - name: "US equities SIP bars"
      ...
      schema_version: "3"
      fingerprint: "not computed — no live data pull in this workspace [unverified]"
      vintage: "live feed; pricing re-verified against vendor page https://massive.com/stocks-data 2026-09-11"
      price_band: "$199/mo, real-time, 20+ years history [documented — vendor pricing page https://massive.com/stocks-data, fetched 2026-09-11]"
      rights: "licensed"
...
cost_model:
    ...
    fee_schedule_as_of: "2026-09-10 [documented — Nasdaq price list, Trading (nasdaqtrader.com), checked 2026-09-10; see §T10 source log]"
...
Cheapest viable path: **Massive Stocks Advanced — $199/mo [documented — vendor pricing page https://massive.com/stocks-data, fetched 2026-09-11]** (real-time 1-min bars, 20+ years history). Build the NAV-dev machine on this cheap bar feed; the creation/redemption pipeline is a broker capability you buy.
...
**Source log (deep-review verification, 2026-09-10/11).**

- Petajisto, A. (2017). "Inefficiencies in the Pricing of Exchange-Traded Funds." Financial Analysts Journal 73(1): 24–54 [documented] — RePEc citation record confirms journal, volume, issue, pages. Intraday indicative value (IIV) updated by the exchange every `15` seconds [documented].
- Ben-David, I., Franzoni, F. & Moussawi, R. (2018). "Do ETFs Increase Volatility?" J. Finance 73(6): 2471–2535 [documented] — NBER working paper 20071 record; DOI 10.1111/jofi.12727. (Deep review corrected the issue from 73(3) to 73(6).)
- Blitz, D. & Huij, J. (2012). "Evaluating the Performance of Global Emerging Markets Equity Exchange-Traded Funds." Emerging Markets Review 13(2): 149–158 [documented] — RePEc citation record.
- Madhavan, A. N. (2016). Exchange-Traded Funds and the New Dynamics of Investing. Oxford University Press. ISBN-13 9780190279394 [documented]. (Deep review corrected the ISBN from 0190279398.)
- Nasdaq price list — Trading (nasdaqtrader.com): fees to remove liquidity, shares ≥ $1.00, all MPIDs $0.0030/share [documented], checked 2026-09-10.
- Massive (rebranded from Polygon.io, verified 2026-09-11) Stocks pricing: Starter $29/mo (15-min delayed, 5-yr history), Developer $79/mo (15-min delayed, 10-yr history), Advanced $199/mo (real-time, 20+ yr history) [documented — vendor pricing page https://massive.com/stocks-data, fetched 2026-09-11].
- Databento: US Equities Standard plan $199/mo (effective Jan 2025) incl. unlimited live + free historical OHLCV-1s/1m [documented]; DBEQ.BASIC MBP-1 historical usage-based $2.00/GB [documented] (unit pricing 2024-12-11).
...
1. "T009 (ETF-vs-Basket Arbitrage) uses a provisional local regime ID `R-IDX` for index-rebalance-day degradation. Which canonical regime module (R0xx) covers index-rebalance days, and what is its machine-readable gate record {regime_id, direction, mechanism, condition, action, min_lag, unknown_behavior}? A good answer names the R-module ID and gives the condition and action fields verbatim."
2. "T009's edge verdict is NO-DOCUMENTED-EDGE. Is there a citable, purged/embargoed, after-cost walk-forward of a 1-minute NAV-dev ETF-vs-basket arbitrage rule (entry ≥ ~1.5 bps, PCF match ≥ 99%, taker-only fills, full cost stack ≥ 10 bps round-trip) with the data source, fill model, and cost stack stated? A good answer gives the citation, sample period, cost level, and net P&L figures — or confirms none exists."
3. "T009 needs a licensed US data product delivering PCF plus iNAV/iIV per-minute reference data across the liquid equity-ETF universe. Which vendor product does this, with what published price band and schema version? A good answer gives the vendor, product name, price band, and whether PCF/iNAV is included or must be built from basket pricing."
4. "For T009's C12 creation-unit discipline: what are the actual creation-unit sizes (shares per unit and typical USD size) for SPY, QQQ, and IWM as of 2026, and the primary-market cutoff times for creations/redemptions? A good answer gives shares/unit and cutoff ET for each ticker with a citable source."
5. "T009's cost stack charges borrow_bps_per_day = 2.0 bps [example] for multi-leg short borrow. What are realistic 2026 borrow rates (bps/day) for hard-to-borrow vs general-collateral US equity ETF legs at a retail-accessible prime broker, so the module can split the stack into GC vs HTB legs? A good answer gives bps/day bands for both with the broker/venue convention."

## AI build prompt

```text
Build the T009 (ETF-vs-Basket Arbitrage) strategy module from this specification (v1.0.2, template 1.0.0).
Hard constraints:
- The strategy emits OrderTicket intentions ONLY; it never creates orders (C1).
- Causality: fill_event > signal_event (t->t+1); no signal-bar fills (C4).
- Timing box: signal@t (1-min, America/New_York) → earliest fill @open(t+1).
- Cost gate: expected_cost_bps(...) <= k * edge_bps before any ticket (C3); borrow_bps_per_day = 2.0 bps [example] with the stated reason.
- Kill switch ARMED -> TRIPPED -> RECOVERY -> ARMED on the §T0 trip conditions (C2) with the re-arm checklist.
- Every numeric literal in prose carries an honesty tag [documented]/[measured]/[example]/[unverified]/[default]/[internal-est]; exempt identifiers only.
- Sources must be real and cited with cost-level tokens (Petajisto 2017 FAJ 73(1):24-54; Ben-David et al. 2018 J. Finance 73(6):2471-2535; Madhavan 2016 ISBN-13 9780190279394); chatbot-only material goes to §T10.
- Compliance C1–C19: include STP/self-trade prevention, bona-fide intent, message-rate/cancel-to-trade limits, the pre-trade fat-finger trio, the halt/auction state table, the decision-log schema (C17), the locate check, the public-dissemination gate, and data-entitlement assertions.
...
EXIT := (|NAVdev_t| <= navdev_exit) [convergence]
        OR (pcf_match < 0.99 → unwind) OR (t >= 15:30 → flatten)
        OR (module_state != OK)
Sizing: implement the §T3 sizing_fn verbatim; enforce the §T0 risk contract YAML.
...
signature, and doc-contract consistency. Definition of done: test_cmd exits 0.
Note the GROK-NEEDED questions in §T10 (R-IDX canonical ID, after-cost walk-forward
citation, PCF/iNAV product, creation-unit sizes, borrow-rate bands) — do not
answer them from priors; leave [unverified] where they bite.
```

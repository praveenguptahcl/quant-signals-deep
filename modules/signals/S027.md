...
| Row | Value |
|---|---|
| Module | S027 — End-of-day momentum / last-hour drift, v1.0.0 |
...
| `c` at 09:30 / 15:30 / close | `p930` / `p1530` / `pclose` (session anchors) |
...
| Borrow | `0.0` | [default] — reason: reference build long-biased, flat by close |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    rl = e.pclose / e.p1530 - 1.0                                 # hold return (ex post; never an input)
    mech = classify_mechanism(e)                                  # INFORMATION vs MECHANICAL [example] heuristic
    edge_bps = abs(rf) * 1e4 * 0.1                                       # [example]
...
- Acceptance tests (`modules/tests/test_S027.py`, `5` tests):
  `1.` fixture recomputes to expected (`r_first`/`r_last`/`s`; day-1/day-2 chapter hand-checks); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (non-positive anchor) → `UNKNOWN`.
...
|---|---|---|---|---|
| O/H/L/C anchors | float | 1-min bars, RTH | Tier 0–1 | Only 9:30, 15:30, close strictly needed |
| Volume | int | 1-min bars | Tier 1 | Gao: effect stronger on high-volume days |
| Close-imbalance feed | imbalance $/shares | event, 15:30–16:00 | Tier 1–2 | NYSE imbalance / Nasdaq NOII near-freeze |
| Exchange calendar | date/bool | daily | Tier 0–1 | Early closes move the window |
| Corporate actions | ratio/date | event | Tier 1 | Adjusted closes or the return is fiction |
...
| Tier-1 (Polygon Stocks Advanced / Alpaca SIP) | Full SIP 1-min bars, corporate actions | ~`$30–200`/mo | Correct anchors + consolidated volume | Still no official imbalance feed |

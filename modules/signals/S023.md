...
| After-cost | `BEFORE-COST` — a *conditional intraday trigger* [documented]: gap-and-go works only when the gap holds past the first 15 minutes - most gaps fill, and fading the fill is the documented complement |
...
| Regime gates | Provisional: R001 elevated RV amplifies (gaps larger, fills faster); earnings seasons amplify gaps |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = prior close; opening auction price; 1-min bars for confirmation; price_band = `~$30–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the detector, buy the intraday feed** (the detector is `~70` lines [internal-est]; the value is the confirmation discipline)**.
...
6. **Escalation gate:** gap-fill rate `>70`% [default] -> pattern absent; live universe `>20` symbols [default]
...
|---|---|
| `prior close` | `prev bar_c` |
| `open` | `bar_o (opening auction)` |
| `1-min bars` | `bar_o/h/l/c/v` |
...
```
ENTER_LONG  := (gap_pct >= gap_min_pct) AND (15-min low >= open) -> LONG at next open (t+1)
ENTER_SHORT := (gap_pct <= -gap_min_pct) AND (15-min high <= open) -> SHORT at next open (t+1)
FLAT        := otherwise
```

Gap confirmed only when the first 15 minutes hold the gap extreme; entry at the next bar's open.

### Exits (with post-exit cooldown)

```
EXIT := (gap fills: price crosses prior close) OR (time_stop_min elapsed) OR (stop hit)
...
Gap: $g = (O_t - C_{t-1})/C_{t-1}$. **Long confirmation:** $g \ge 1.0\%$ [default] and the first-`15`-min low $\ge O_t$ (gap holds) -> LONG at the next bar's open. **Short confirmation** mirrors. **Gap fill** (price crosses $C_{t-1}$) invalidates. Fixture (*example*): `C_{t-1} = 100.00`, `O_t = 101.50` (`g = 1.5%`); lows hold at/above `101.50` from bar 2 on -> LONG confirmed on bar 4. Most gaps fill - the confirmation window is the edge [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    (prev_close, open_px, bars) = gap_session(events)
    g = (open_px - prev_close) / prev_close
    assert isfinite(g) else return UNKNOWN                        # F2
    conf = confirm_window(bars, cfg.confirm_min, open_px, g)
    side = 1 if conf.long else (-1 if conf.short else 0)
    ...
    sig = SignalVector(symbol, direction=side, confidence=0.6 if side else 0.0, capital=0.0,
                       computed_at=bars[-1].event_ts, staleness=now()-bars[-1].asof_ts,
                       module_state=state.module_state)
    log_decision(sig, inputs_hash, params_hash)                    # C5 / Appendix g
    assert fill_event > signal_event  # t->t+1 causality: confirmation window closes first
    return sig
```

Causality: the 15-min confirmation window must close before the signal fires; entry is at the next bar's open. Mandatory unit test: no-signal-bar fills.

## §S4 Worked example (fixture-backed)

- Tape: `modules/fixtures/S023_tape.csv` — `# TYPE: validation-run`; `5` 1-min bars after a `+1.5`% gap (`100.00 -> 101.50`), all numbers synthetic, seed `20260923` [example].
- Expected outputs: `modules/fixtures/S023_expected.csv`; per-bar gap-hold state and confirmation flag; hand-checked below.
...
- `g = (101.50 - 100.00)/100.00 = 1.5% >= 1.0%` ✓
- bar 1: low `101.40 < 101.50` -> gap not yet held (window still forming) ✓
- bar 2: low `101.55 >= 101.50` -> holds ✓
- bar 4: confirmation window held -> LONG confirmed ✓
- bar 5: close `100.90 < 100.00`? No - gap-fill check: `100.90 > 100.00`, no fill ✓
...
Selection disclosure: the `1` source surfaced by the chapter's research pass; the gap-fill complement is the documented risk control.
...
`../../images/S023_example.png` — synthetic `5`-bar gap tape (seed `20260923` [example]): gap, hold, confirmation.
...
> candidate Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; compute pattern `per_event`; skip earnings/news-overlays.
> Implement `signal(state, events, cfg) -> SignalVector` (Appendix b v1.0.0)
> with the §S3 normative pseudocode, including the cost-gate predicate
> `expected_cost_bps(...) <= k * edge_bps` and `assert fill_event >
> signal_event`. Wire F1–F5 (Appendix f v1.0.0): invalid input -> `UNKNOWN`,
> never interpolate. Log decisions per Appendix g v1.0.0. Secrets via env
> only. Run the fixture `modules/fixtures/S023_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S023_expected.csv`

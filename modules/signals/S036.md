...
| Row | Value |
|---|---|
| Module | S036 — Overnight-gap fade, v1.0.0 |
...
vector (`sessions`, `gap`, `cooldown_until`, `module_state`); `events` are canonical Events (Appendix a v1.0.0),
...
|---|---|---|---|---|
| Gap gates | `gap_min`/`gap_max` | `0.5`% / `5`% | [example] | high — noise vs news |
| Hold horizon | `max_hold_bars` | `10` bars | [example] | medium |
| Fade target | — | `30`% of gap | [example] | medium |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    in_gate = (gap_min <= abs(gap) <= gap_max)
    direction = -sign(gap) if in_gate else 0   # fade, never chase
    edge_bps = abs(gap) * 1e4 * 0.3                                       # [example]
...
- Tape: `modules/fixtures/S036_tape.csv` — `# TYPE: validation-run`;
  `13` synthetic bars across `2` sessions (`id,event_ts,session,prior_close,open,close`); `s1` reproduces the chapter's S4 tape, `s2` is a below-gate contrast. All numbers synthetic, seed `36` [example]; timestamps int64 ns UTC.
- Expected outputs: `modules/fixtures/S036_expected.csv` — `gap`, `in_gate`, `dir`, `fade_ret` per bar; session-level features repeat per bar by construction.
- Tolerance: `1e-9` on floats; integer counts exact.
- Acceptance tests (`modules/tests/test_S036.py`, `5` tests):
  `1.` fixture recomputes to expected (gap `+2.00`% and fade hand-checks; `s2` below-gate flat); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (negative open) → `UNKNOWN`.
- `test_cmd`: `python3 -m pytest modules/tests/test_S036.py -q` → exits `0`.

**Hand-check:** `s1`: `gap = 102.00/100.00 − 1 = +2.00`% [example] → `dir=−1`; bar-1 fade `101.50/102.00 − 1 = −0.49`% [example]; `s2` (`+0.20`% [example] `< 0.50`%) → `dir=0` — asserted in the test.
...
|---|---|---|---|---|
| Prior close / open | float | 1-min bars | Tier 1 | Both must exist |
| Intraday closes | float | 1-min bars | Tier 1 | Fade path |
| Corporate actions | events | as-occurring | Tier 1 | Unadjusted gaps are fiction |
| Earnings calendar | event | as-occurring | Tier 1 | News-gap filter |
...
| Tier-0 (Stooq) | Daily OHLC, free | ~`$0` | Free gap measurement | No intraday fade path |
| Tier-1 (Polygon Stocks Advanced) | 1-min bars, corporate actions | ~`$30–200`/mo | True open + fade path | None material |
...
**Selection disclosure:** the chapter's worked tape is the only fixture-backed evidence in this build; the gap-fill pattern is widely replicated in practitioner literature but no single citable after-cost study was pinned here [documented-as-absence].
...
> Build module **S036 — Overnight-gap fade** per `notes/module-template.md` v1.0.0
...
> only. Run the fixture `modules/fixtures/S036_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S036_expected.csv`
> (tolerance `1e-9`).

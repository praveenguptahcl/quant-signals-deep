...
changelog:
  ...
  - "1.0.1 2026-09-10: deep-review pass — full Markov estimator pseudocode (fit + online lookup), per-parameter calibration recipes, corrected microprice formula P^micro = M + s*(I-0.5), sourced fee basis (Nasdaq $0.0030 taker), verified vendor tiers, regime-direction enum fix, fixture time/seq columns, 8 acceptance tests"
---
...
| Row | Value |
|---|---|
| Module | S004 — Microprice (Stoikov), v1.0.1 |
...
| Data cost | Research `~$29–99`/mo (Polygon/Massive Stocks Starter `$29` delayed / Developer `$99` real-time, SIP L1) [unverified — third-party Jun-2026 pricing comparison]; production `~$199`/mo (Advanced) + usage (exchange-timestamped L1) [unverified] — verify before budgeting |
...
1. **Prototype hour band:** `20–60` h [internal-est] — L1 snapshot reader + (imbalance, spread) state discretizer + transition-matrix estimator + reference validation against the fixture; +`40–120` h [internal-est] for the full Markov estimator (phase 2: Q/R1/R2 fit + convergence checks per §S3)
2. **Cheapest-source row:** min_tier = Polygon/Massive Stocks **Developer** (real-time SIP L1 — Starter `$29`/mo is 15-min delayed and insufficient for microstructure) [unverified] — cheapest *usable* candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = L1 bid/ask price + displayed size per quote event, exchange (SIP) timestamps, full tick quote history for transition estimation; price_band = `~$99–199`/mo [unverified] (Developer `$99` / Advanced `$199`, per a third-party Jun-2026 pricing comparison; vendor rebranded Polygon→Massive in late 2025 [unverified] — verify before budgeting); build_or_buy = ****build the estimator, buy the feed** (closed-form microprice is `~10` lines [internal-est]; the value is the state discretization and transition calibration)**.
...
| Vendor (Databento MBP-1, DBN v3) | Canonical | Notes |
|---|---|---|
...
| `ts_in_delta` | `in_delta (int32 ns)` | network transit delta; SIP jitter proxy [documented] |
| `bid_px_00` / `ask_px_00` | `bid_px` / `ask_px` (float $) | raw int64 fixed-point ÷ `1e9` [documented] |
| `bid_sz_00` / `ask_sz_00` | `bid_sz` / `ask_sz` (uint32 shares) | displayed size [documented] |
| `sequence` | `seq` | venue-specific; gap >`1,000` [default] → §S0.6 DEGRADED [documented] |
| `action` / `side` / `flags` | filter | keep book-update actions; drop trade prints (`T`) from the quote chain [default] |
| *(not in MBP-1)* | `tick_size` | from instrument reference (`min_price_increment`) joined on `instrument_id` [documented] |
| *(not in MBP-1)* | `halt_flag` | venue trading-status feed, not a quote field — §S0.5 behavior [default] |

Polygon/Massive mapping [unverified]: quote event `t` → `event_ts` (SIP time — label SIP work *simulated only — requires MBO/ITCH* for latency claims), `p`/`s` → `bid_px`/`bid_sz`, `P`/`S` → `ask_px`/`ask_sz`, `q` → `seq`; tick size from ticker details; drop delayed-plan rows at ingest.
...
| Fees (taker incl. regulatory) | `0.30` | [example] — ≡ Nasdaq `$0.0030`/share remove-liquidity fee at a `$100` price [documented — Nasdaq price list]; re-price per symbol before production |
...
`fee_schedule_as_of`: `2026-09-10` [unverified] — reference basis is the Nasdaq remove-liquidity row (`$0.0030`/share, all MPIDs, shares ≥`$1.00`) [documented]; pin the real venue schedule before production. `side: taker` [default]; venue/tier/source: XNAS / Databento MBP-1 [example]. Re-stating cost numbers outside this block is a validation failure.
...
|---|---|---|---|---|---|
| Imbalance bins | `imb_bins` | `5` | [default] | high | grid {`3`,`5`,`10`} [example] on held-out quote data; pick max validation log-likelihood per state-sample; paper uses `5` [documented — Stoikov slides] |
| Max spread states | `spread_ticks_max` | `5 ticks` | [default] | medium | set from the regular-session spread histogram: cap at the `95`th percentile of tick spreads [default procedure]; wider caps grow the state space ×spread |
| Estimation window | `est_window_days` | `20 days` | [default] | high | refit daily on a rolling `10`–`30`-session window [default]; require min bin visits `≥100` [default] and successive-Q̂ total-variation `<0.05` [default]; lengthen the window if sparse |
| Entry z-threshold | `z_entry` | `2.0` | [default] | high | sweep `0.5`–`5.0` [default] on embargoed OOS; pick max `edge_bps − expected_cost_bps`; σ-scale per symbol: `z·tick` must exceed `~0.5`× typical half-spread [example] |
| Exit z-threshold | `z_exit` | `0.5` | [default] | medium | grid `0`–`z_entry` [default]; maximize mean-reversion round-trip net of `2`× costs |
| Post-exit cooldown | `cooldown_s` | `60 s` | [default] | low | ≥ `2`× median OOS time between threshold crossings [default]; raise if churn |
...
The paper's approximation sequence (Stoikov's own Bloomberg talk slides [documented]): $p_t^i = M_t + \sum_{k=1}^{i} g_k(I_t)$ with $g_{i+1}(x) = \mathbb{E}[g_i(I_{\tau_1}) \mid I_t = x]$, and the microprice is the limit $p_t^{\text{micro}} = \lim_{n\to\infty} p_t^n$. The slides' estimation setup: imbalance discretized to $\{1,\dots,5\}$ ($5$ bins [documented]), spread fixed at $1$ tick [documented], mid-price increments $M_{t+1} - M_t \in (-1, 0, 1)$ ticks [documented]. Reported empirical result: the microprice has flatter volatility signature plots than the midprice or the naive microprice [documented]. Write $P^{\text{micro}} = M + g(I, S)$ with $M$ mid, $I = Q^b/(Q^b+Q^a)$ imbalance, $S$ spread; the paper's program is to **estimate $g$ from data**.

**Finite-state estimator** (phase 2, normative): generalize to $n$ imbalance bins × $m$ spread-tick states ($nm$ states; the paper's estimation is the $n=5, m=1$ case [documented]). State index $s = b \cdot m + (sp - 1)$ with normative binning $b = \min(n-1, \lfloor n I \rfloor)$ for $I \in [0,1]$ (clamp outside), $sp = \mathrm{clamp}(\mathrm{spread\_ticks}, 1, m)$. Estimate by MLE on quote history $\le t$ only — no future transitions:

```
function fit_markov(hist_events, n, m) -> (Q, R1, R2, K):   # offline; history <= t only
    counts = zeros(nm, nm); moves1 = zeros(nm, 3); moves2 = zeros(nm, nm)
    for each consecutive event pair (e_i, e_{i+1}) in hist (both CONTINUOUS_TRADING, same session, no splits):
        u = state(e_i); v = state(e_{i+1})
        counts[u, v] += 1
        if mid(e_{i+1}) != mid(e_i):
            k = sign(mid(e_{i+1}) - mid(e_i)) in {-1, 0, +1}   # [documented] move classes
            moves1[u, k] += 1
            moves2[u, v] += 1
    Q  = row_normalize(counts)   # MLE transient transition matrix [documented]
    R1 = row_normalize(moves1)   # one-step mid-move distribution given state [documented]
    R2 = row_normalize(moves2)   # transitions coincident with a mid move [documented]
    K  = (-1, 0, +1)             # tick increments of the move classes [documented]
    assert min_bin_visits(counts) >= 100 else mark DEGRADED     # [default]; S10.1
    return Q, R1, R2, K

function microprice_markov(M, s, Q, R1, R2, K, iters=6):   # [default] iters; online lookup
    g1 = solve(I - Q, R1 @ K)        # first-order adjustment [documented]
    G = g1; gi = g1
    for _ in 2..iters:
        gi = solve(I - Q, R2 @ gi)  # recursion g_{i+1} = (I-Q)^{-1} R2 g_i [documented]
        G += gi
    assert isfinite(G[s]) else return fallback closed-form     # F2
    return M + G[s]                 # E[M_{tau_inf} | F_t] [documented]
```

**Reference closed form** (first-order/naive approximation, Gatheral & Oomen 2009 via Stoikov's slides [documented] — kept in the reference build): the depth-weighted mid
$$P^{\text{micro}}_t \approx \frac{Q^a_t P^b_t + Q^b_t P^a_t}{Q^a_t + Q^b_t} = M_t + s_t\,(I_t - \tfrac{1}{2}),$$
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert e.tick_size > 0 else return UNKNOWN                     # F2
    assert isfinite(e.bid_px) and isfinite(e.ask_px) else return UNKNOWN  # F2
    if e.seq - state.prev_seq > 1000:                             # [default] §S0.6
        state.module_state = DEGRADED                             # restart estimation windows
        state.trans_counts.zero(); state.micro_window.clear()
    state.prev_seq = e.seq
    M = (e.bid_px + e.ask_px) / 2
    I = e.bid_sz / (e.bid_sz + e.ask_sz)
    tick = e.tick_size
    spread_ticks = round((e.ask_px - e.bid_px) / tick)
    ...
    gates_ok = (spread_ticks <= cfg.spread_ticks_max) \
               and (now_ns() >= state.cooldown_until) \
               and (state.module_state == OK)
    locate_ok = consumer_locate_flag(e.symbol)                     # [default] consumer asserts; Reg-SHO threshold -> False
    direction = +1 if (micro_dev >= cfg.z_entry * tick and cost_ok and gates_ok) else
                -1 if (micro_dev <= -cfg.z_entry * tick and cost_ok and gates_ok and locate_ok) else 0
...
- Tape: `modules/fixtures/S004_tape.csv` — `# TYPE: validation-run`; `6` L1 snapshots with `event_ts`/`seq` columns (synthetic, seed `20260904` [example]); validates the closed-form reference `P^micro = (Q^a·P^b + Q^b·P^a)/(Q^a + Q^b) = M + s·(I − 0.5)`.
...
- Acceptance tests (`modules/tests/test_S004.py`, `8` tests): `1.` fixture recomputes to expected exactly (tol `1e-9`, determinism asserted); `2.` microprice inside [`bid`,`ask`] (F2 bound); `3.` stub emits valid `SignalVector` (direction in {+1,-1,0}, confidence/capital in [0,1]); `4.` no-signal-bar fills (`fill_event_ts > computed_at`); `5.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `6.` invalid input -> `UNKNOWN`; `7.` normative imbalance-binning `b = min(n−1, ⌊n·I⌋)` incl. clamps (phase-2 estimator); `8.` fixture pins the time contract (`event_ts` strictly increasing, `seq` consecutive).
- `test_cmd`: `python -m pytest modules/tests/test_S004.py -q` → exits `0`.

**Hand-checked rows** (arithmetic verified by hand against the fixture; `micro = M + s·(I − 0.5)`, `s = 0.02`):

- r0: bid `100.0`×600, ask `100.02`×400 → `M = 100.01`, `I = 0.6` → `100.01 + 0.02×0.1 = 100.012` ✓
- r1: `I = 0.9` → `100.01 + 0.02×0.4 = 100.018` ✓
- r2: bid `100.01`×200, ask `100.03`×800 → `M = 100.02`, `I = 0.2` → `100.02 + 0.02×(−0.3) = 100.014` ✓
- r3: `I = 0.5` → `100.02 + 0 = 100.02` ✓
- r4: bid `100.02`×100, ask `100.04`×900 → `M = 100.03`, `I = 0.1` → `100.03 + 0.02×(−0.4) = 100.022` ✓
- r5: `I = 0.7` → `100.03 + 0.02×0.2 = 100.034` ✓
...
| Tick size | float ($) | per instrument | Tier 2 | `min_price_increment` from instrument reference joined on `instrument_id`; MBP-1 carries none [documented] |
| Sequence numbers | uint32 | per event | Tier 2 | gap >`1,000` [default] → §S0.6 DEGRADED [documented] |
| Halt/auction flags | reference | intraday | Tier 1 | venue trading-status feed — not an MBP-1 quote field [default] |
| Corporate actions / halts | reference | daily + intraday halt feed | Tier 1 | contributions garbage across splits/halt reopens |

**Validation on ingest** [default]: `ask_px > bid_px`, `bid_sz + ask_sz > 0`, `event_ts` strictly increasing per symbol, `tick_size > 0`; violations → drop event, §S0.6 error taxonomy.
...
| Stoikov Bloomberg talk (slides) [documented] | US equities (BAC shown) | microprice has flatter volatility signature plots than midprice or naive microprice | `BEFORE-COST` | slides, not a peer-reviewed paper |
...
4. Stoikov, S. "Defining the Microprice in HFT" / "Understanding Microprice Estimation." Bloomberg Lightning Talk slides [documented]. https://www.scribd.com/document/541079934/Microprice-BBG — 5-bin imbalance discretization, spread fixed at 1 tick, mid-moves in (−1, 0, 1), approximation sequence p_t^i = M_t + Σ g_k(I_t), flatter volatility signature plots vs midprice/naive microprice.
5. Nasdaq Trader, "Price List — Trading" [documented]. https://www.nasdaqtrader.com/trader.aspx?id=pricelisttrading2 — fees to remove liquidity, shares ≥$1.00: $0.0030/share, all MPIDs (source for the 0.30 bps fee component at $100 price).
6. Databento MBP-1 field set [documented — working implementations]: ts_event/ts_recv/ts_in_delta/sequence/action/side/flags, bid_px_00/ask_px_00 (fixed-point ÷1e9), bid_sz_00/ask_sz_00. https://github.com/cyb3rghoul/blockhouse-quantitative-developer-work-trial-task · https://github.com/ldbld/iv-sniper-gate0/blob/HEAD/CLAUDE.md
7. Meneses González, K. (Jun 2026), "Stock Price API: Best Free vs Paid Options (2026)", Medium [unverified — third-party]. https://medium.com/@kevinmenesesgonzalez/stock-price-api-best-free-vs-paid-options-2026-82370a377819 — Polygon rebranded to Massive late 2025; tiers Starter $29 (delayed), Developer $99 (real-time), Advanced $199, All-Access $399.
...
- Open-source re-implementation (`general-liquidity/gordon`, microstructure commit): discretize book → estimate Q/R1/R2 → solve `(I−Q)g₁ = R1·K` → recurse `gₙ₊₁ = (I−Q)⁻¹·R2·gₙ`, sum 6 iterations, fall back to mid on unobserved states — matches the paper's structure; kept as a lead because it is code, not peer-reviewed evidence.
...
> `expected_cost_bps(...) <= k * edge_bps`, the seq-gap rule (`>1000` [default]
> -> DEGRADED), defined helpers `gates_ok`/`locate_ok`, and `assert fill_event >
> signal_event`. The phase-2 upgrade path is the §S3 `fit_markov` /
> `microprice_markov` pseudocode (Q/R1/R2 MLE, `(I−Q)g₁ = R1·K`, recursion).

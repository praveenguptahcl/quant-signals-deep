...
| Row | Value |
|---|---|
| Module | S021 — Opening-range breakout, v1.0.0 |
| Edge verdict | `PREDICTIVE-FEATURE-ONLY` — documented intraday pattern (Fisher; Crabel): before-cost only - the edge is in the failure fade, per the chapter |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = 1-min bars (regular session); opening-range length; volume z-score window; price_band = `~$30–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the detector, buy the intraday feed** (the detector is `~80` lines [internal-est]; the value is the confirmation and time-stop discipline)**.
3. **Resource budget:** `1` core, `<2` GB RAM for `<=20` symbols live [internal-est]; compute pattern `per_event`.
4. **Skip list:** multi-timeframe ORB (phase `2`); overnight-gap variants; options-flow confirmation
5. **DO-NOT-CUT list:** causality assertions (`assert fill_event >
   signal_event`, no-signal-bar-fills test); COST block + callable
   `expected_cost_bps`; F1–F5 fail-safes + module-state enum; decision-log
   audit records; bona-fide-intent + self-trade prevention; provenance tags on
   every numeric literal; fixtures + `test_cmd`; secrets via env only.
6. **Escalation gate:** breakout win rate `<40`% over `60` days [default] -> retire; live universe `>20` symbols [default]
...
```
ENTER_LONG  := (close > or_high + buffer) AND (vol_z >= vol_z_min) -> LONG at next open (t+1)
ENTER_SHORT := (close < or_low - buffer) AND (vol_z >= vol_z_min) -> SHORT at next open (t+1)
FLAT        := otherwise
```

Breakout confirmed only on a 1-min close beyond the range plus volume confirmation; entry is at the next bar's open (t+1 causality).

### Exits (with post-exit cooldown)

```
EXIT := (time_stop_min elapsed) OR (close back inside range) OR (stop hit)
...
Opening range from the first `or_minutes` of the session: $OR = [L_{OR}, H_{OR}]$. **Long trigger:** a 1-min close $c_t > H_{OR} + \text{buffer}$ with volume z-score $\ge 1.5$ [default] -> LONG at the next bar's open. **Short trigger** mirrors. **Time stop:** flat after `60` min [default] regardless of P&L. Fixture (*example*): `OR = [100.00, 100.50]`; bar closes `100.56` with `vol_z = 2.0` -> LONG trigger (buffer `0.02` cleared: `100.56 > 100.52`). The chapter's verdict: most breaks fail - size for the fade, not the follow-through [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    (orh, orl) = opening_range(bars, cfg.or_minutes)
    assert orh > orl else return UNKNOWN                          # F2
    trig = breakout_trigger(bars, orh, orl, cfg)                  # close + buffer + vol_z
    assert trig.side in (-1, 0, 1) else return UNKNOWN             # F2
    ...
    sig = SignalVector(symbol, direction=trig.side, confidence=min(trig.vol_z/3.0, 1.0), capital=0.0,
...
- Tape: `modules/fixtures/S021_tape.csv` — `# TYPE: validation-run`; `6` 1-min bars: `30`-min opening range `[100.00, 100.50]`, then a confirmed upside break, all numbers synthetic, seed `20260921` [example].
- Expected outputs: `modules/fixtures/S021_expected.csv`; per-bar range state and trigger flag; hand-checked below.
- Tolerance: `1e-9` [default] on floats; integer contributions exact.
- Acceptance tests (`modules/tests/test_S021.py`, `6` tests): `1.` fixture recomputes to expected; `2.` stub emits valid `SignalVector` (direction in {+1,-1,0}, confidence/capital in [0,1]); `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input -> `UNKNOWN`; `6.` extra: stub is deterministic across calls.
...
- bars 1-3 form `OR = [100.00, 100.50]` ✓
- bar 4 closes `100.56 > 100.50 + 0.02` with `vol_z = 2.0 >= 1.5` -> LONG trigger ✓
- bar 5 (no break): close `100.45` inside range -> no trigger ✓
- bar 6: close `99.90 < 100.00 - 0.02` with `vol_z = 1.8` -> SHORT trigger ✓
...
Selection disclosure: the `2` sources surfaced by the chapter's research pass; the chapter's own verdict (fade the failure) is the operative edge statement.

## §S10 Failure modes & pitfalls

| # | Trigger → Detect → Action → Recovery | check: |
|---|---|---|
| 1 | Range-break fakeout: most breaks fail -> detect: close back inside range within 15 min [default] -> action: flip to fade mode / exit -> recovery: next session | `check: not faked_out` |
...
`../../images/S021_example.png` — synthetic `6`-bar ORB tape (seed `20260921` [example]): opening range and confirmed breaks.
...
> candidate Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; compute pattern `per_event`; skip multi-TF/gap-variants.
> Implement `signal(state, events, cfg) -> SignalVector` (Appendix b v1.0.0)
> with the §S3 normative pseudocode, including the cost-gate predicate
> `expected_cost_bps(...) <= k * edge_bps` and `assert fill_event >
> signal_event`. Wire F1–F5 (Appendix f v1.0.0): invalid input -> `UNKNOWN`,
> never interpolate. Log decisions per Appendix g v1.0.0. Secrets via env
> only. Run the fixture `modules/fixtures/S021_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S021_expected.csv`

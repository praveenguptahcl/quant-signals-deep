...
| Row | Value |
|---|---|
| Module | S011 — Amihud illiquidity, v1.0.0 |
...
| When it dies | Zero-volume share `>10`% [default] (S10.1); unadjusted splits (spurious |r|); cutoff miscalibration |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert len(days) >= 21 else return FLAT(DEGRADED)             # [default]
...
- Tape: `modules/fixtures/S011_tape.csv` — `# TYPE: validation-run`; `6` days of `(close, dollar_vol)` with `1e6` scaling, all numbers synthetic, seed `20260911` [example].
- Expected outputs: `modules/fixtures/S011_expected.csv`; per-day `|r|` and `ILLIQ×1e6`; `ILLIQ̄×1e6 = 0.0201`; hand-checked below.
...
- d2: `r = (100.5 − 100)/100 = 0.005`, `ILLIQ = 0.005/2e7 × 1e6 = 0.00025 → 0.0003` (fixture rounding) ✓
- d3: `r = 0.00995`, `ILLIQ×1e6 = 0.00995/1.5e7×1e6 = 0.000663 → 0.0007` ✓
- mean of `6` ILLIQ×1e6 values `= 0.0201` [example] ✓

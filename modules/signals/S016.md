...
| Row | Value |
|---|---|
| Module | S016 — Hasbrouck information share, v1.0.0 |
...
2. **Cheapest-source row:** min_tier = Alpaca IEX / Stooq (Tier 0) for research; Polygon.io Stocks Advanced for production — cheapest candidates [internal-est]; latency_class = SIP;
   required_fields = synchronized bid/ask quotes from `2`+ venues; `60`+ days of 1-min bars [example]; price_band = `~$0–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the VECM, buy the multi-venue feed** (the estimator is `~120` lines [internal-est]; the value is the synchronization discipline)**.
3. **Resource budget:** `1` core, `<1` GB RAM; daily bars `3000` stocks × `10`y `~2–5` GB [internal-est]; compute pattern `per_bar`.
4. **Skip list:** intraday IS variants; Gonzalo-Granger alternatives as primary; single-venue fallback
5. **DO-NOT-CUT list:** causality assertions (`assert fill_event >
   signal_event`, no-signal-bar-fills test); COST block + callable
   `expected_cost_bps`; F1–F5 fail-safes + module-state enum; decision-log
   audit records; bona-fide-intent + self-trade prevention; provenance tags on
   every numeric literal; fixtures + `test_cmd`; secrets via env only.
6. **Escalation gate:** venue leadership flips persistently [default]; live universe `>20` symbols [default]
...
```
ENTER_LONG  := (leader == preferred_venue) -> routing screen PASSED (venue input, not a directional trigger)
ENTER_SHORT := (leader == preferred_venue) -> routing screen PASSED (venue input, not a directional trigger)
FLAT        := otherwise
```

The information share is a *venue-routing input*: it tells the router whose quotes to trust. It is never a directional trigger.
...
venue $j$'s **information share**: $IS_j = ([\psi F]_j)^2 / (\psi \Omega \psi')$ with $\psi$ the common-row vector and $F F' = \Omega$ (Cholesky) [documented]. Fixture (illustrative - *example*): `IS_A = 0.7`, `IS_B = 0.3` -> venue A leads. IS bounds (upper/lower via variable ordering) must be reported, not just the midpoint [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    panels = synchronized_venue_panels(events, cfg.n_venues)
    assert all(len(p) >= 21 for p in panels) else return FLAT(DEGRADED)  # [default]
    is_shares = information_share(panels)                         # VECM + Cholesky
    assert abs(sum(is_shares) - 1.0) < 1e-6 else return UNKNOWN    # F2
    leader = argmax(is_shares)
    edge_bps = 0.0                                                 # [example] routing input only
    # normative cost-gate predicate: expected_cost_bps(...) <= k * edge_bps
    sig = SignalVector(symbol, direction=0, confidence=max(is_shares), capital=0.0,
                       computed_at=events[-1].event_ts, staleness=now()-events[-1].asof_ts,
                       module_state=state.module_state,
                       aux={"is_shares": is_shares, "leader": leader})
...
- Tape: `modules/fixtures/S016_tape.csv` — `# TYPE: validation-run`; `2` venues with illustrative information shares (`IS_A = 0.7`, `IS_B = 0.3`), all numbers synthetic, seed `20260916` [example].
- Expected outputs: `modules/fixtures/S016_expected.csv`; venue leadership call; hand-checked below.
- Tolerance: `1e-9` [default] on floats; integer contributions exact.
- Acceptance tests (`modules/tests/test_S016.py`, `6` tests): `1.` fixture recomputes to expected; `2.` stub emits valid `SignalVector` (direction in {+1,-1,0}, confidence/capital in [0,1]); `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input -> `UNKNOWN`; `6.` extra: stub is deterministic across calls.
...
> candidate Alpaca IEX / Stooq (Tier 0) for research; Polygon.io Stocks Advanced for production — cheapest candidates [internal-est]; compute pattern `per_bar`; skip intraday-variants/single-venue.
> Implement `signal(state, events, cfg) -> SignalVector` (Appendix b v1.0.0)
> with the §S3 normative pseudocode, including the cost-gate predicate
> `expected_cost_bps(...) <= k * edge_bps` and `assert fill_event >
> signal_event`. Wire F1–F5 (Appendix f v1.0.0): invalid input -> `UNKNOWN`,
> never interpolate. Log decisions per Appendix g v1.0.0. Secrets via env
> only. Run the fixture `modules/fixtures/S016_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S016_expected.csv`

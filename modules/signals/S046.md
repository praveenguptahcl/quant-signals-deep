...
| Row | Value |
|---|---|
| Module | S046 — Intraday U-shape timer, v1.0.0 |
...
FLAT        := otherwise  # inactive windows never signal
```

`edge_bps` = `10` bps [example] — window-move edge when active.
...
function signal(state, events, cfg) -> SignalVector:
    ...
    z_w = (obs_abs_w - typ_w) / sd_w        # per window: open/lunch/close
    active_w = |z_w| >= 1.5
    direction = -sign(signed_w) if active_w else 0   # fade the signed move
    edge_bps = 10.0                                       # [example]
...
- Acceptance tests (`modules/tests/test_S046.py`, `5` tests):
  `1.` fixture recomputes to expected (window z hand-checks; lunch-only activity; fade direction); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (negative abs move) → `UNKNOWN`.
...
|---|---|---|---|---|
| Bucket moves | float | 10-min buckets | Tier 1 | Absolute + signed |
| Typical levels | float | quarterly | Tier 1 | Re-estimated |
| Session calendar | reference | daily | Tier 0–1 | Half-day handling |

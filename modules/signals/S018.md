...
provenance_tier: D
sources:
  - "Lillo & Farmer (2004), Studies in Nonlinear Dynamics & Econometrics 8(3)"
  - "Bouchaud et al. (2004), Quantitative Finance 4(2)"
unverified_sources:
consumes:
...
| Row | Value |
|---|---|
| Module | S018 — Order-sign autocorrelation, v1.0.0 |
| Edge verdict | `PREDICTIVE-FEATURE-ONLY` — documented microstructure fact (Lillo-Farmer, Bouchaud): execution-timing input, never a standalone trigger |
| After-cost | `BEFORE-COST` — a *microstructure fact* [documented]: order signs are long-memory persistent - consumed by execution scheduling (expect continuation) and impact models |
...
| Regime gates | Provisional: R001 elevated RV distorts (persistence breaks in stress); news shocks flip the sign process |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = S007-signed trade signs (event time); `500`+ signs per estimate [example]; price_band = `~$30–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the estimator, buy the signed tape** (the estimator is `~40` lines [internal-est]; the value is the horizon discipline)**.
...
Documented fact: $C(\ell) > 0$ and decays slowly (long memory), attributed to order splitting (Lillo-Farmer [documented]). Fixture (*example*): signs `+ + - + + - - +` -> `C(1) = (1 - 1 - 1 + 1 - 1 + 1 - 1)/7 = -1/7 = -0.1429`. The fixture checks the estimator arithmetic, not a trading decision [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert len(signs) >= cfg.min_signs else return FLAT(DEGRADED) # [default]
    acf = [mean(signs[i]*signs[i+l] for i in range(len(signs)-l)) for l in 1..cfg.max_lag]
    ...
    sig = SignalVector(symbol, direction=0, confidence=min(abs(acf[0])*3, 1.0), capital=0.0,
                       ...
                       aux={"acf_1": acf[0], "persistent": acf[0] > 0})
...
- products at lag 1: `+1, -1, -1, +1, -1, +1, -1` -> sum `-1`, `C(1) = -1/7 = -0.1429` ✓
- products at lag 2: `-1, +1, -1, -1, -1, -1` -> sum `-4`, `C(2) = -4/6 = -0.6667` ✓
...
|---|---|---|---|---|
| Lillo & Farmer (2004) [documented] | LSE | the long memory of the efficient market (order splitting) | `BEFORE-COST` | microstructure fact, not a rule |
| Bouchaud et al. (2004) [documented] | Paris Stock Exchange | fluctuations and response in financial markets | `BEFORE-COST` | microstructure fact, not a rule |

Selection disclosure: the `2` papers surfaced by the chapter's research pass; Hurst-exponent extensions kept in S12 leads [unverified].
...
1. Lillo, F., Farmer, J. D. (2004) [documented]. "The long memory of the efficient market." *Studies in Nonlinear Dynamics & Econometrics* 8(3). https://arxiv.org/abs/cond-mat/0310077
2. Bouchaud, J.-P., Gefen, Y., Potters, M., Wyart, M. (2004) [documented]. "Fluctuations and response in financial markets: the subtle nature of 'random' price changes." *Quantitative Finance* 4(2), 176-190. https://arxiv.org/abs/cond-mat/0307332
...
- Hurst-exponent estimation of sign long memory circulates in practitioner notes; no checkable specification found - treated as unverified.
...
> candidate Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; compute pattern `per_event`; skip Hurst/cross-stock.

...
| Row | Value |
|---|---|
| Module | S030 — Bollinger bandwidth squeeze → expansion, v1.0.0 |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    bw = (bb_up - bb_lo) / sma                                    # bandwidth [documented]
    pct = 100 * mean(bw[t-R : t-1] <= bw[t])                       # trailing-R percentile
    armed = (pct <= p)                                             # squeeze: direction-neutral
    edge_bps = 8.0                                       # [example]
...
- Acceptance tests (`modules/tests/test_S030.py`, `5` tests):
...
|---|---|---|---|---|
| OHLC bars | float | `1`–`15` min or daily | Tier 0/1 | Daily bars suffice for the 6-month-minimum variant |
...
|---|---|---|---|---|---|---|
| R001 | amplifies | volatility-clustering regime: compression reliably precedes expansion | clustered_vol_flag | none | `1` day [default] | veto |
| R002 | degrades | dead low-liquidity names: permanent "squeeze" bandwidth | adv < adv_min | veto | `5` days [default] | veto |
| R003 | degrades | news-spike fake expansions | news_spike_flag | veto | `1` day [default] | veto |

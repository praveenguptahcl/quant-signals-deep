...
test_cmd: "python -m pytest modules/tests/test_S026.py -q"
datasets:
  - name: US equities L1
    vendor: Databento
    product: MBP-1
...
| Row | Value |
|---|---|
| Module | S026 — First-half-hour to last-half-hour momentum, v1.0.0 |
| Edge verdict | `PREDICTIVE-FEATURE-ONLY` — documented intraday pattern (Gao-Han-Saadka / Heston-Sadka): before-cost only - the lead-lag is the edge |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = 1-min bars (regular session); session half-hour boundaries; price_band = `~$30–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the trigger, buy the intraday feed** (the trigger is `~60` lines [internal-est]; the value is the boundary discipline)**.
3. **Resource budget:** `1` core, `<2` GB RAM for `<=20` symbols live [internal-est]; compute pattern `per_event`.
4. **Skip list:** overnight lead-lag variants (phase `2`); multi-day lead-lag
5. **DO-NOT-CUT list:** causality assertions (`assert fill_event >
   signal_event`, no-signal-bar-fills test); COST block + callable
   `expected_cost_bps`; F1–F5 fail-safes + module-state enum; decision-log
   audit records; bona-fide-intent + self-trade prevention; provenance tags on
   every numeric literal; fixtures + `test_cmd`; secrets via env only.
6. **Escalation gate:** lead-lag correlation `<0.1` over `60` days [default] -> retire; live universe `>20` symbols [default]
...
| `flat_min` | int | `390` | ``380`-`390`` | default |
...
```
ENTER_LONG  := (fh_ret >= ret_min_bps) -> LONG at entry_min (t+1)
ENTER_SHORT := (fh_ret <= -ret_min_bps) -> SHORT at entry_min (t+1)
FLAT        := otherwise
```

First-half-hour return measured 9:30-10:00; entry in the last hour; flat by the close.

### Exits (with post-exit cooldown)

```
EXIT := (flat_min reached) OR (stop hit) - always flat into the close
        ON_EXIT: cooldown_until = next session open
...
First-half-hour return: $r_{FH} = p_{10:00}/p_{9:30} - 1$. **Trigger:** $r_{FH} \ge 20$ bps [default] -> LONG at minute `330` (3:00 PM); mirror for SHORT. **Flat:** always flat by minute `390` (close). Fixture (*example*): `p_{9:30} = 100.00`, `p_{10:00} = 100.30` -> `r_{FH} = 30` bps `>= 20` -> LONG at `330`, flat at `390`. The lead-lag is *intraday only*: never hold overnight on this signal [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    rfh = first_half_hour_return(bars)
    assert isfinite(rfh) else return UNKNOWN                      # F2
    side = 1 if rfh >= cfg.ret_min_bps/1e4 else (-1 if rfh <= -cfg.ret_min_bps/1e4 else 0)
    ...
    sig = SignalVector(symbol, direction=side, confidence=min(abs(rfh)*1e4/50.0, 1.0), capital=0.0,
                       computed_at=bars[-1].event_ts, staleness=now()-bars[-1].asof_ts,
                       module_state=state.module_state, aux={"fh_ret_bps": rfh*1e4})
    log_decision(sig, inputs_hash, params_hash)                    # C5 / Appendix g
    assert fill_event > signal_event  # t->t+1 causality: entry at minute 330, after the FH window
    return sig
```

Causality: the first-half-hour window closes at 10:00; entry is hours later at minute 330 - never inside the measurement window. Mandatory unit test: no-signal-bar fills.

## §S4 Worked example (fixture-backed)

- Tape: `modules/fixtures/S026_tape.csv` — `# TYPE: validation-run`; `1` session: `9:30 = 100.00`, `10:00 = 100.30` (`r_FH = 30` bps), all numbers synthetic, seed `20260926` [example].
- Expected outputs: `modules/fixtures/S026_expected.csv`; session row with `r_FH` and trigger; hand-checked below.
- Tolerance: `1e-9` [default] on floats; integer contributions exact.
- Acceptance tests (`modules/tests/test_S026.py`, `6` tests): `1.` fixture recomputes to expected; `2.` stub emits valid `SignalVector` (direction in {+1,-1,0}, confidence/capital in [0,1]); `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input -> `UNKNOWN`; `6.` extra: stub is deterministic across calls.
- `test_cmd`: `python -m pytest modules/tests/test_S026.py -q` → exits `0`.

**Hand-checked rows** (arithmetic verified by hand against the fixture):

- `r_FH = (100.30 - 100.00)/100.00 = 30` bps `>= 20` -> LONG ✓
...
|---|---|---|---|---|
| Gao, Han & Saadka (academic literature) [documented] | US equities intraday | first-half-hour / last-half-hour momentum | `BEFORE-COST` | pattern evidence, not a tested rule |
...
Selection disclosure: the `2` sources surfaced by the chapter's research pass; the always-flat-into-close rule is the operative risk control.
...
1. Gao, Han & Saadka (academic literature) [documented]. First-half-hour / last-half-hour momentum (no single canonical citation).
...
> Build module **S026 — First-half-hour to last-half-hour momentum** per
> `notes/module-template.md` v1.0.0 and this file's §S0 contract.
> Cheapest path (§1.5): prototype 20–60 h [internal-est]; cheapest data
> candidate Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; compute pattern `per_event`; skip overnight/multi-day.
...
> (`# TYPE: validation-run`) against `modules/fixtures/S026_expected.csv`

...
| Row | Value |
|---|---|
| Module | S040 — VWAP-deviation mean reversion, v1.0.0 |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    dev = close - vwap
    sigma = sample_std(dev[0..t])   # EXPANDING window incl. current bar
    z = dev / sigma                 # undefined until min_periods bars: warmup guard
    direction = -sign(z) if (abs(z) >= thresh and warmup_ok) else 0
    edge_bps = abs(f["z"]) * 20.0                                       # [example]
...
- Acceptance tests (`modules/tests/test_S040.py`, `5` tests):
  `1.` fixture recomputes to expected (z-series hand-checks; bar-5 trigger; warmup guard); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (non-positive VWAP) → `UNKNOWN`.
...
|---|---|---|---|---|
| Closes | float | 1-min bars | Tier 1 | Tick-cleaned |
| Session VWAP | float | 1-min bars | Tier 1 | From the open, tick-cleaned |
| Bad-tick mask | bool | per bar | Tier 1 | VWAP integrity |

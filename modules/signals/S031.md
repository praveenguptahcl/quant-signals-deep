...
sources:
  ...
  - "Haas, M. (2023). The Cowles-Jones test with unspecified upward market probability. Data Science in Finance and Economics, 3(4), 324-336. doi:10.3934/DSFE.2023019."
...
changelog:
  ...
  - "1.1.0 2026-09-11: deep review vs v1.0.0 — (1) normative pseudocode hardened: streak state carry, warmup branch, zero-return rule branch, locate_ok short gate, explicit cost-gate inputs, causality assertions; fixed undefined-feats reference; (2) added per-parameter calibration grids + OOS walk-forward/embargo selection recipes in §S2; (3) §S6 vendor mapping pinned (Polygon aggs v3, split-adjusted closes, corporate-action stream, XNAS session calendar); (4) COST block values tagged inline, borrow as bps/day held with reason; (5) S0.4 time units clarified; (6) fixture tape extended to 15 bars with a zero-return bar pinning the break rule; (7) acceptance tests 5 → 8 (cost-gate blocking, locate veto, empty-events UNKNOWN, zero-return reset); (8) Cowles-Jones 2023 source now cites Haas, M. + DOI, web-verified."
---
...
| Row | Value |
|---|---|
| Module | S031 — Consecutive-bar streaks (runs), v1.1.0 |
...
vector (`bars`, `streak`, `cooldown_until`, `locate_ok`, `module_state`);
`locate_ok` is the consumer-asserted Reg-SHO locate flag (default `False` [default] → shorts vetoed). `events` are canonical Events (Appendix a v1.0.0),
...
> `event_ts`; max_skew = `1` [default] (s); jitter budget = `500`
> [default] (ms); bar-label = open time; staleness TTL = `180` [default]
> (s); gap policy = mask, no interpolation; halt/auction → discard
...
|---|---|---|
| Spread (half-spread, liquid large-cap) | `0.50` [example] | flagged; calibrate per venue at scale-up |
| Fees (taker incl. regulatory) | `0.30` [example] | flagged; pin the fee schedule before production |
| Borrow (bps per day held) | `0.0` [default] | reference build long-biased; shorts add C7 locate cost |
| Impact | `1.0` [example] | flagged; calibrate per venue at scale-up |
| **Total reference** | `1.80` [example] | sum of components |
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    borrow_bps = 0.0    # [default] bps per day held; long-biased reference; reason in table
...
### Calibration recipes (grid + OOS selection, per parameter)

Method (all parameters): walk-forward with expanding train, embargo of `5`
trading days [default] after each train/test boundary; test windows of `20`
trading days [default]; selection on **FULL-COST** net Sharpe computed with the
COST block; require `>=200` OOS test trades [default] across `>=4` windows
[default]; compute DSR on the variant grid or declare "DSR not computed";
pick the plateau (the `3`-grid cell [default] whose neighbors stay within `±1` step [default]),
never the peak; variant count = grid size product, disclosed in the run log.

| Parameter | Grid | Selection rule | Re-tune cadence |
|---|---|---|---|
| `k_cont` (`2`–`5` [default]) | {`2`,`3`,`4`,`5`} [default] | Maximize OOS net Sharpe; veto if `≥5%` [default] of entries fire on `1`–`2`-bar noise | quarterly [default] |
| `k_exhaust` (`5`–`9` [default]) | {`5`,`6`,`7`,`8`,`9`} [default] | Fade-leg OOS edge must be `>0` net of costs; veto if fade P&L flips sign across `>1` window [default] | quarterly [default] |
| `zero_return_rule` | {break, carry, ignore} [example] | Must be identical in research and live (C11 config-hash check); select by OOS net Sharpe, log the hash | fixed unless zero-bar distribution shifts |
| `cooldown_s` (`0`–`3,600` [default]) | {`0`,`300`,`600`,`1,800`,`3,600`} [default] | Maximize OOS net; cap at typical streak-break recovery time; veto if re-entry whipsaw dominates | quarterly [default] |
| `cost_gate_k` (`0.1`–`2.0` [default]) | {`0.25`,`0.5`,`1.0`,`2.0`} [default] | Tighten until the marginal gated-out bar shows `≤0` net edge [default]; open item: `0.5` vs `2.0` default pending calibration data | on every fee-schedule re-pin [default] |

Re-tune triggers (any): fee-schedule re-pin; regime-gate table changed; bar
grid or zero-return rule edited; two consecutive quarters of live fade-leg P&L
`≤0` [example].
...
function signal(state, events, cfg) -> SignalVector:
    assert len(events) >= 1 else return UNKNOWN                      # F1
    e = events[-1]
    assert e.close > 0 and e.high >= e.low else return UNKNOWN       # F1/F2: never interpolate
    if len(events) < 2:
        streak, direction, confidence = 0, 0, 0.0                    # warmup: no prior close
    else:
        prev = events[-2]
        s_t = sign(e.close - prev.close)                             # +1/-1; 0 = zero-return bar
        if s_t == 0:
            streak = 0                                               # zero-return rule: "break" [example]
        elif (state.streak >= 0 and s_t > 0) or (state.streak <= 0 and s_t < 0):
            streak = state.streak + s_t
        else:
            streak = s_t
        edge_bps = 6.0 * abs(streak) / cfg.k_cont                     # [example]
        cost_bps = expected_cost_bps(notional=1.0,                    # [example] reference notional
                                     adv_pct=0.001,                   # [example] 0.1% of ADV
                                     venue="XNAS", side="taker",      # [example]
                                     urgency="normal")                # [example]
        # ^^^ normative cost-gate predicate: expected_cost_bps(...) <= cost_gate_k * edge_bps
        cost_ok = cost_bps <= cfg.cost_gate_k * edge_bps
        now_ok = now() >= state.cooldown_until                        # C10 post-exit cooldown
        long_leg  = ((streak >= cfg.k_cont) and (streak < cfg.k_exhaust)) \
                    or (streak <= -cfg.k_exhaust)                     # ride runs; fade exhaustion
        short_leg = ((streak <= -cfg.k_cont) and (streak > -cfg.k_exhaust)) \
                    or (streak >= cfg.k_exhaust)
        if not (cost_ok and now_ok):
            direction = 0
        elif long_leg:
            direction = +1
        elif short_leg and state.locate_ok:                          # C7: no locate -> no short
            direction = -1
        else:
            direction = 0
        confidence = min(1.0, conviction(abs(streak))) if direction != 0 else 0.0  # 1.0 [default]
        # conviction(x) := min(1.0, x / cfg.k_exhaust) — streak-length conviction [example]
    capital = 0.5 * confidence                                       # 0.5 [default]
    assert all inputs have event_ts <= e.event_ts                    # causality: features use data <= t
    ...
    state.streak = streak                                             # carry state forward
    return sig
```
...
- Tape: `modules/fixtures/S031_tape.csv` — `# TYPE: validation-run`;
  `15` synthetic 5-minute bars (seed `31`): sign pattern `+,+,-,+,+,+,-,-,-,-,-,-,-,+,0` — a `3`-bar up-run, a `7`-bar down-run, a flip, then a zero-return bar that pins the break rule (bar `15`, `s=0` → `0`). All numbers synthetic, seed `31` [example]; timestamps int64 ns UTC.
...
- Acceptance tests (`modules/tests/test_S031.py`, `9` tests):
  `1.` fixture recomputes to expected (bar-6 `s=3`→`+1`; bar-9 `s=-3`→`-1`; bar-12 `s=-6`→`+1` exhaustion fade; bar-15 zero-return `s=0`→`0` — hand-checks); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (negative close) → `UNKNOWN`; `6.` cost-gate blocking wired end-to-end (`cost_gate_k = 0.0` [default] zeroes the bar-6 signal); `7.` SHORT vetoed when `locate_ok = False` [default] (C7); `8.` empty events → `UNKNOWN`; `9.` synthetic two-bar equal-close input resets the streak to `0` (break rule).
- `test_cmd`: `python3 -m pytest modules/tests/test_S031.py -q` → exits `0`.

**Hand-check:** signs `+,+,-,+,+,+,-,-,-,-,-,-,-,+,0`: bar `6` `s=+3` → continuation `+1`; bar `9` `s=−3` → continuation `−1`; bar `12` `s=−6` → exhaustion fade `+1`; bar `14` `s=+1` → `0`; bar `15` `s=0` (zero-return breaks the streak) → `0`. All recomputed by hand and asserted in the test.
...
| Field | Type | Granularity | Vendor mapping (Polygon Stocks Advanced, aggs v3) | Notes |
|---|---|---|---|---|
| OHLC bars | float | `1`–`15` min [example] | `results[].{o,h,l,c,v,n,t}`; `t` is ms→ns UTC; split/dividend-adjusted closes | Close-sign variant needs `c`; range variant needs `h`/`l` |
| Corporate actions | events | as-occurring | splits and symbol-change endpoints | A split breaks sign continuity — reseed streak to `0` on the action bar |
| Session calendar | reference | daily | XNAS regular session `09:30`–`16:00` America/New_York | Overnight and weekend gaps are not streak bars |

Required fields: `event_ts` (int64 ns UTC), adjusted open/high/low/close, `volume`, `trade_count` (`n`), split-adjusted flag on closes; price band `~$30–200`/mo [unverified] (indicative — verify before budgeting).
...
Feasible: trivial. One integer counter per symbol; O(`1`) per bar. `500` symbols [example] × `390` bars/day [example] is a sub-second pass. Tier L per `notes/cost-model.md` §5: `4–12` h ≈ `$600–1,800` [internal-est] at `$150`/hr loaded.
...
> never interpolate. SHORT requires `state.locate_ok` (C7). Log decisions
> per Appendix g v1.0.0. Secrets via env only. Run the fixture
> `modules/fixtures/S031_tape.csv` (`# TYPE: validation-run`) against
> `modules/fixtures/S031_expected.csv` (tolerance `1e-9` [default]); bar `15`
> is a zero-return bar pinning the break rule (`s=0` → `0`).
> **Definition of done:** `python3 -m pytest modules/tests/test_S031.py -q`
> exits `0` (`9` acceptance tests). Tag every numeric literal you add with the
> unified enum (Appendix i v1.0.0); put anything unverifiable in §S12
> unverified leads.

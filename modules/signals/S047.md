...
unverified_sources: []
consumes: []
regime_ids: ['R001', 'R002', 'R003']  # provisional — pending R001–R050 annotation
...
changelog:
  ...
  - "1.1.0 2026-09-11: deep review vs v1.0.0 — added per-parameter calibration recipes (grids + OOS selection) for min_trades/cost_gate_k; concretized §S6 data spec (Polygon Stocks Advanced schema v3, required fields, price bands [unverified]); fixed §1/§1.5 data-tier inconsistency (tick trades, not SIP 1-min bars); completed §S3 normative pseudocode branches (window anchoring, invalid/too-few/undefined paths, market-state freeze, inline causality assertions); added provisional regime_ids [R001–R003]; tests 5→11 (empty-events, too-few-trades, trending-tape undefined branch, pure-alternation identity, determinism, harvest-veto consistency); tagged $150/hr [internal-est], fixture size/tolerance tags"
---
...
| Row | Value |
|---|---|
| Module | S047 — Bid–ask bounce / Roll spread, v1.1.0 |
...
| Data cost | Research `~$0–99`/mo (delayed trades; free tier available)` [unverified]; production `~$199`/mo Advanced [unverified] (tick trades reportedly unlock from `$99`/mo Developer and above [unverified], third-party reviews 2026 — verify on the vendor pricing page before budgeting) |
...
| Regime gates | R001 amplifies / R002–R003 degrade (provisional — pending R001–R050 annotation) |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced [internal-est] (tick trades reportedly unlock at Developer `$99`/mo and above [unverified], third-party reviews 2026 — verify); latency_class = bar-level / minutes; required_fields = tick trade prices + bid/ask side markers + exchange timestamps (ms→int64 ns UTC) + trade condition codes (last-sale filter) + session calendar + corporate-action adjustments; price_band = `~$0–199`/mo [unverified] (delayed/free-tier research → `$199`/mo Advanced production — verify before budgeting); build_or_buy = **build the estimator, buy the feed**.
...
Vendor field names are logical placeholders [unverified] — pin the exact names
against the vendor schema on ingest (§0 fingerprint):

| Vendor (logical) | Canonical |
|---|---|
...
> `event_ts`; max_skew = `1` [default] ms; jitter budget = `100` [default] ms; bar-label = open time; staleness TTL = `300` [default]; gap
...
| Borrow (`borrow_bps_per_day`) | `0.0` | [default] — reason: estimator role, no position |
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    borrow_bps_per_day = 0.0    # [default] estimator role; reason in table
...
**`min_trades`** (grid {`20`, `40`, `80`, `120`, `160`, `200`} [default]) —
walk-forward: for each candidate, estimate `roll_spread` on the trailing window
over `20` liquid US equities × `60` trading days [example]; target = realized
effective spread over the next `60` min [default] (trade-price distance to the
prevailing mid, volume-weighted) [example]. Metric: OOS mean absolute error in
bps [default]. Embargo: `1` trading day [default] between estimation and
evaluation windows. Minimum `500` OOS estimate points per candidate [default].
Selection: argmin OOS MAE; tie-break within `5`% [default] → larger window
(stability). Re-run quarterly [default] or when a quoted-spread regime flag
(R-gate) fires.

**`cost_gate_k`** (grid {`0.1`, `0.25`, `0.5`, `1.0`, `2.0`} [default]) — this
S-module never enters, so `k` is calibrated on downstream consumer harvest
attempts, not in the module itself: replay `≥200` observed/simulated harvest
attempts [default] per candidate. Metric: after-cost net edge per permitted
attempt, minimum `100` permitted attempts [default]; embargo `1` trading day
[default]. Selection: max OOS net edge; tie-break → smaller `k` (more
conservative gate). With no consumer data, hold the `0.5` [default]
conservative default and log `gate_uncalibrated` [example] in the decision log.
...
```
function signal(state, events, cfg) -> SignalVector:              # §S0.1
    assert events non-empty else return UNKNOWN                   # F1
    e = events[-1]
    assert valid_event(e) else return UNKNOWN                     # F1/F2: never interpolate
    assert e.event_ts >= state.last_event_ts                      # causality: clocks never rewind
    if market_state(e) != CONTINUOUS_TRADING:                     # §S0.5 table
        return freeze(e, state)                                   # HALTED→UNKNOWN / AUCTION→DEGRADED
    w = events                                                    # caller supplies the calibrated
                                                                  # estimation window, all data ≤ t
    assert w[-1].event_ts == e.event_ts                           # window anchored at newest event
    assert all(x.event_ts <= e.event_ts for x in w)               # no lookahead, ever
    if len(w) < cfg.min_trades:                                   # too few trades
        return SignalVector(symbol=e.symbol, direction=0, confidence=0.0,
                            capital=0.0, computed_at=e.event_ts, module_state=OK)
    dp = diffs(prices(w))                                         # Δp_t = p_t − p_{t−1}
    gamma1 = cov(dp[1:], dp[:-1])                                 # mean-adjusted; Roll (1984)
    if gamma1 < 0:
        spread = 2*sqrt(-gamma1); valid = 1
    else:
        return SignalVector(symbol=e.symbol, direction=0, confidence=0.0,
                            capital=0.0, computed_at=e.event_ts, module_state=OK)
    # γ1 ≥ 0 → estimator undefined: missing information, never a sign (never forced)
    edge_bps = 0.0                                                # estimator role: measures cost [default]
    cost_ok = expected_cost_bps(notional, adv_pct, venue, side, urgency) \
              <= cfg.cost_gate_k * edge_bps                       # normative cost-gate predicate
    direction = 0                                                 # §S2 NO ENTRY — estimator/veto only
    confidence = 0.0; capital = 0.0
    sig = SignalVector(symbol=e.symbol, direction, confidence, capital,
                       computed_at=e.event_ts, staleness=now()-e.asof_ts,
                       module_state=state.module_state, features={gamma1, spread, valid})
    log_decision(sig, inputs_hash(w), params_hash(cfg))           # C5 / Appendix g
    state.last_event_ts = e.event_ts
    assert fill_event > signal_event                              # t→t+1: no signal-bar fills
    return sig
```

The reference stub passes the full tape as the estimation window; production
callers truncate to the calibrated `min_trades`–`200`-trade [default] trailing
window per the §S2 calibration recipe.
...
- Tape: `modules/fixtures/S047_tape.csv` — `# TYPE: validation-run`;
  `60` synthetic trades [example] (`id,event_ts,price,side`) around mid `50.00` [example], bid/ask `49.99`/`50.01` [example] (seed `47`). All numbers synthetic, seed `47` [example]; timestamps int64 ns UTC.
...
- Acceptance tests (`modules/tests/test_S047.py`, `11` tests):
  `1.` fixture recomputes to expected (gamma1/spread hand-checks; undefined branch via constant-price helper); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (negative price) → `UNKNOWN`; `6.` empty events → `UNKNOWN`; `7.` fewer than `min_trades` events → `OK` with direction `0` (wait, no information); `8.` trending tape (constant positive drift → gamma1 = `0` [example]) → `valid=0`, NaN spread, `OK` with direction `0`; `9.` pure alternation (`50.01`/`49.99`, `41` prices [example]) reproduces the Roll identity `spread = 2*sqrt(-gamma1) = 0.04` [example] within `1e-9` [default]; `10.` recomputation is deterministic (bit-identical across runs); `11.` direction `0` and confidence `0` on every event even when the estimate is valid (harvest-veto consistency).
...
|---|---|---|---|---|---|
| Trade prices | float | tick | Polygon.io Stocks Advanced — REST trades endpoint, schema v3 [unverified pin] | trade price, exchange timestamp (ms → int64 ns UTC), trade condition codes for last-sale filtering [unverified — pin exact vendor field names on ingest] | Positive, sane; corporate-action adjusted |
| Trade sides | str enum | tick | same feed | bid/ask marker, or the exchange side field where provided; otherwise quote-rule derivation documented in code [default] | Derivation method must be logged (C5) |

Price band: Polygon Stocks Advanced `~$199`/mo [unverified]; tick-level trades
reportedly unlock from Developer `~$99`/mo and above [unverified] (third-party
reviews, 2026); research prototype can start on delayed/free-tier trades
`~$0`/mo [unverified] — verify on the vendor pricing page before budgeting.
...
Feasible: trivial. One covariance per window. Tier L per `notes/cost-model.md` §5: `4–12` h ≈ `$600–1,800` [internal-est] at `$150`/hr loaded [internal-est]. Its value is feeding honest spreads into every other module's COST block.
...
- Polygon.io 2026 tier structure: Starter `$29`/mo, Developer `$99`/mo, Advanced `$199`/mo, All-Access `$399`/mo; tick trades reportedly unlock at Developer and above [unverified] — sourced from third-party Medium reviews (Jun–Oct 2026), not the vendor page; verify on polygon.io pricing before budgeting.
- Polygon reportedly rebranded to Massive in late 2025 [unverified] — single Medium source; verify before referencing in build notes.

...
provenance_tier: D
sources:
  - "NYSE American / NYSE Arca imbalance data products (Regulation NMS Plan participants' opening/closing imbalance feeds) — exchange data product [documented]."
...
| Row | Value |
|---|---|
| Module | S033 — Open-auction imbalance continuation, v1.0.0 |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    d_bps = (e.ind - e.ref) / e.ref * 1e4                         # displacement
    gate = (rho >= cfg.rho_min) and (abs(d_bps) >= cfg.d_min_bps)  # both gates
    edge_bps = abs(d_bps) * 0.2                                       # [example]
...
- Expected outputs: `modules/fixtures/S033_expected.csv` — `rho`, `d_bps`, `gate`, `dir` per message; the `m1` row demonstrates the point-in-time replay property (gate fails → `dir=0`).
- Tolerance: `1e-9` on floats; integer counts exact.
- Acceptance tests (`modules/tests/test_S033.py`, `5` tests):
  `1.` fixture recomputes to expected (`m8` rho/displacement hand-checks; `m1` replay property); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (zero paired shares) → `UNKNOWN`.
...
|---|---|---|---|---|
| Imbalance messages | event | pre-open, per symbol | Tier 1–2 | NYSE imbalance / Nasdaq NOII |
| Reference price | float | per message | Tier 1–2 | Prior close or regulatory reference |
...
Feasible: trivial. Eight messages per symbol per day, two divisions per message. For `500` symbols the full pre-open book is a few thousand messages — microseconds in polars. Tier L per `notes/cost-model.md` §5: `4–12` h ≈ `$600–1,800` [internal-est] at `$150`/hr loaded. The engineering is the feed handler (message sequencing, revision handling), not the math.

## §S8 Buy vs build

| Tier-1 (Polygon Stocks Advanced) | Aggregates, no official imbalance | ~`$30–200`/mo | Cheap prototyping | No true imbalance book |
...
|---|---|---|---|---|
| Chapter S4 worked tape (8 messages, `09:28:00`–`09:29:55`) | Single-name synthetic example | Final `rho=22.3`% [example], `d=+50.1` bps [example] → continuation long | `BEFORE-COST` (arithmetic illustration) | Synthetic tape — demonstrates the computation, not the edge |
...
1. NYSE imbalance data products (opening/closing auction imbalance feeds, Regulation NMS Plan participants) — exchange data product [documented].
2. Nasdaq Net Order Imbalance Indicator (NOII) / TotalView — pre-open imbalance dissemination [documented].

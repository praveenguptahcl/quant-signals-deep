...
consumes: []
regime_ids:
  - R001  # provisional — pending R001–R050 annotation
  - R002  # provisional — pending R001–R050 annotation
  - R003  # provisional — pending R001–R050 annotation
...
changelog:
  ...
  - "1.1.0 2026-09-11: deep review v1.0.0 — hardened §S3 normative pseudocode (executable cost-gate predicate, t→t+1 causality assertions, fixed undefined edge reference); added beta_window_days/vol_window_days config + per-parameter calibration recipes (S0.2.1); borrow_bps_per_day row in COST; regime_ids pinned provisional; §S6 vendor mapping concretized (Polygon Stocks Advanced, schema v3); R001 action made enum-compliant; §1.5 cheapest-source row as a structured table; tests 5→9 (empty-input UNKNOWN, cooldown suppression, fixture TYPE/provenance, fixture-edge cost gate)"
---
...
| Row | Value |
|---|---|
| Module | S039 — Idiosyncratic residual reversal, v1.1.0 |
...
2. **Cheapest-source row** (structured; cheapest data source candidate [internal-est]):

| min_tier | latency_class | required_fields | price_band | build_or_buy |
|---|---|---|---|---|
| Polygon.io Stocks Advanced | bar-level / minutes | 1-min OHLCV bars, corporate-action adjustments, session calendar | `~$30–200`/mo [unverified] (indicative — verify before budgeting) | build the estimator, buy the feed |
...
6. **Escalation gate:** upgrade from cheapest when any holds — live universe beyond the cheapest band; jitter persistently over budget; cost gate failing on `[measured]` costs; entitlement class changes; production trading requires moving from delayed to real-time entitlement.
...
|---|---|---|---|---|
| `beta_A` / `beta_B` | float | `1.0` / `0.9` | `0.3`–`2.0` | example — pinned fixture constants; production betas come from the rolling estimator, not these values |
| `beta_window_days` | int | `60` | `20`–`250` | example — rolling beta estimation window (trading days) |
| `vol_window_days` | int | `20` | `5`–`60` | example — trailing residual-vol estimation window (trading days) |
...
### S0.2.1 Calibration recipes (grid + OOS selection, per parameter)

Selection protocol (applies to every parameter): walk-forward folds, embargo `1` day [default] between train and test, ≥ `100` OOS trades [default] per fold, variant count recorded for DSR. Re-fit on schedule; never re-fit on the evaluation fold.

| Parameter | Grid | OOS selection recipe |
|---|---|---|
| `z_thresh` | `1.5`, `2.0`, `2.5`, `3.0` [default] | max after-cost DSR; tie → smaller threshold |
| `cost_gate_k` | `0.25`, `0.5`, `1.0`, `2.0` [default] | smallest `k` with after-cost DSR ≥ `0.9`× best [default] |
| `beta_window_days` | `30`, `60`, `120` [example] | min `1`-day-ahead [default] beta forecast error; then verify trigger-rate stability across folds |
| `vol_window_days` | `10`, `20`, `60` [example] | min z-thresh hit-rate drift across folds |
| `cooldown_s` | `0`, `900`, `3_600`, `14_400` [default] | max after-cost mean per trade; tie → `3_600` [default] |
| `beta_A` / `beta_B` | n/a | not calibrated — fixture constants [example]; replaced by the rolling estimator in production |
...
| `beta` | `beta` (name beta) |
| `beta_age_days` | `beta_age_days` (consumer-supplied days since the beta estimate; drives C11) |
...
> [default]; bar-label = open time; staleness TTL = `3_600` s [default]; gap
...
Guard definitions (evaluated per event `e`):

```
beta_fresh := (e.beta_age_days <= beta_window_days)   # C11; beta_window_days = 60 [example]
locate_ok  := consumer-asserted locate flag; Reg-SHO threshold names → SHORT vetoed (C7)
in_cooldown := (e.event_ts < cooldown_until)          # C10; entries suppressed while true
```
...
| Borrow (possible short residual) | `0.50` | [example] — reason: fade may short; C7 locate required. `borrow_bps_per_day = 0.50` [example] — illustrative per-trade allowance, not a pinned lender rate; the fade horizon is intraday (<`1` day [example]), so the reference total applies it without day-scaling; replace with the lender's pinned schedule (fees as of `fee_schedule_as_of`) before production |
...
Edge basis for the cost gate (normative):

$$edge\_bps_{i,T} = 50 \times |z_{i,T}| \qquad \text{[example] — fifty bps per unit of residual z-score}$$
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert market_state(e) == CONTINUOUS_TRADING else freeze per §S0.5
    # --- idiosyncratic residual: every input uses data <= e.event_ts (t→t+1 discipline)
    u         = e.r_raw - e.beta * e.r_mkt                          # u_{i,t}
    state.cum_u[e.name] = state.cum_u.get(e.name, 0.0) + u          # C_{i,T}
    z         = state.cum_u[e.name] / e.sigma_hat                   # z_{i,T}
    fade      = abs(z) >= cfg.z_thresh
    direction = (-1 if z > 0 else (1 if z < 0 else 0)) if fade else 0   # fade the extreme
    # --- §S2 entry guards (Boolean, all normative)
    beta_fresh = e.beta_age_days <= cfg.beta_window_days            # C11
    locate_ok  = (direction != -1) or e.locate_asserted              # C7
    edge_bps   = 50.0 * abs(z)                                      # [example]
    cost_ok    = expected_cost_bps(...) <= cfg.cost_gate_k * edge_bps  # normative cost-gate predicate
    entered    = (direction != 0) and beta_fresh and locate_ok and cost_ok \
                 and (e.event_ts >= state.cooldown_until)           # C10
    # --- emission
    confidence = min(1.0, abs(z) / 3.0) if entered else 0.0          # [example]: |z|=3 -> full conviction
    capital    = 0.5 * confidence                                    # 0.5 [default]
    sig = SignalVector(symbol=e.symbol, direction=(direction if entered else 0),
                       confidence, capital, computed_at=e.event_ts,
                       staleness=now_ns() - e.asof_ts, module_state=state.module_state)
    log_decision(sig, inputs_hash, params_hash)                     # C5 / Appendix g
    return sig
```

Causality assertions (normative, pinned by unit tests):
1. `u/cum/z` consume only events with `event_ts <= e.event_ts` — no lookahead.
2. The module emits signals only; the consumer MUST assert `fill_event_ts > sig.computed_at` — earliest legal fill is the next bucket's open (t+1). The mandatory no-signal-bar-fills test pins this.
3. `staleness = now_ns() − e.asof_ts` must be ≤ staleness TTL `3_600` s [default], else the state flips to `UNKNOWN` (S0.6).
...
- Tape: `modules/fixtures/S039_tape.csv` — `# TYPE: validation-run`;
  `12` [example] synthetic bucket/name rows (`id,event_ts,bucket,name,mkt,beta,raw`); the chapter's S4 six-bucket tape for names A and B. All numbers synthetic, seed `39` [example]; timestamps int64 ns UTC.
...
- Acceptance tests (`modules/tests/test_S039.py`, `9` [documented] tests):
  `1.` fixture recomputes to expected (cum/z hand-checks; fade-B-only); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (negative beta) → `UNKNOWN`; `6.` empty events → `UNKNOWN` (F1); `7.` cooldown suppresses re-entry (C10); `8.` fixture TYPE header = `validation-run` with synthetic/seed-`39` provenance labels; `9.` cost gate on the fixture edge (`z_B = −2.15` [example] → `edge_bps = 107.5` [example] → gate passes at `k = 0.5` [default]; sub-`0.01` [example] bps edge blocked).
- `test_cmd`: `python3 -m pytest modules/tests/test_S039.py -q` → exits `0` [default].
...
|---|---|---|---|---|---|
| Name returns | float | 1-min bars | Polygon.io Stocks Advanced, schema v3 | adjusted 1-min OHLCV (o,h,l,c,v,t), corporate-action adjustments | Universe ± watchlist; session calendar from the same feed |
| Market returns | float | 1-min bars | Polygon.io Stocks Advanced, schema v3 | index-ETF 1-min adjusted bars (e.g., SPY [example]) | Broad index future or ETF |
| Beta estimates | float | daily | built (rolling OLS regression on daily adjusted closes) | name & market daily adjusted closes | Window `beta_window_days = 60` [example] |
| News calendar | event | as-occurring | bought (news/calendar feed) | name, event_ts, event_type | Informed-residual filter; pending vendor pin |

Price bands: see the §1.5 cheapest-source row (`~$30–200`/mo [unverified], indicative — verify before budgeting); beta estimation and the residual are built.
...
|---|---|---|---|---|---|---|
| R001 | amplifies | uninformed-flow dislocation deepens the fade edge | flow_flag | double — downstream strategy may scale the emitted capital up to ×`2` [example]; conservative default = keep §S2 size | `0` [default] | veto |
...
- Vendor rebrand / pricing (community docs, 2026): Polygon.io reportedly rebranded to Massive (massive.com); community pricing posts list Stocks tiers ~`$29` (Starter, 15-min delayed) / ~`$99` (Developer) / ~`$199` (Advanced) / ~`$399` (All-Access) per month [unverified] — the §1/§1.5 `~$30–200`/mo band is consistent with the Starter→Advanced range; confirm against the official schedule before budgeting.
...
> exits `0` [default]. Tag every numeric literal you add with the unified enum

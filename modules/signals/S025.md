...
provenance_tier: D
sources:
  - "Gao & Han (intraday momentum)"
  - "Heston & Sadka (intraday patterns)"
unverified_sources:
consumes:
regime_ids: []
...
| Row | Value |
|---|---|
| Module | S025 — Intraday time-series momentum, v1.0.0 |
| Edge verdict | `PREDICTIVE-FEATURE-ONLY` — documented intraday pattern (Gao-Han; Heston-Sadaka): before-cost only - decays within minutes |
| After-cost | `BEFORE-COST` — a *conditional intraday trigger* [documented]: time-series momentum works at short horizons with fast decay - the edge is in the first minutes, size for the decay |
...
| Risk summary | Decay risk: the effect attenuates within minutes; dies on decay and over-holding |
| When it dies | Sharpe `<0.5` over `60` days [default] (retire); holding past decay; lookback miscalibration |
| Regime gates | Provisional: R001 elevated RV amplifies (momentum stronger but noisier); trend regimes amplify |
...
2. **Cheapest-source row:** min_tier = Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; latency_class = SIP-delayed;
   required_fields = 1-min bars (regular session); lookback and holding horizons; price_band = `~$30–200`/mo [unverified] (indicative — verify before budgeting); build_or_buy = ****build the trigger, buy the intraday feed** (the trigger is `~60` lines [internal-est]; the value is the horizon calibration)**.
3. **Resource budget:** `1` core, `<2` GB RAM for `<=20` symbols live [internal-est]; compute pattern `per_event`.
4. **Skip list:** cross-sectional intraday momentum (phase `2`); overnight-momentum variants
...
`SignalVector` per Appendix b v1.0.0. `state` is the module-owned named state vector (`lb_ret`, `mom_state`, `module_state`); `events` are canonical Events (Appendix a v1.0.0), newest last. The function handles documented edge cases: empty events → `UNKNOWN`; crossed/locked quotes → `UNKNOWN` (F1, never interpolate); halt/auction events → freeze per the market-state table.
...
```
ENTER_LONG  := (lb_ret >= ret_min_bps) -> LONG at next open (t+1)
ENTER_SHORT := (lb_ret <= -ret_min_bps) -> SHORT at next open (t+1)
FLAT        := otherwise
```

Momentum confirmed on the lookback return; entry at the next bar's open; flat after the holding horizon.

### Exits (with post-exit cooldown)

```
EXIT := (hold_min elapsed) OR (stop hit)
...
Lookback return over `lb` minutes: $r_{t-lb,t} = p_t/p_{t-lb} - 1$. **Trigger:** $r_{t-lb,t} \ge 10$ bps [default] -> LONG at the next bar's open; mirror for SHORT. **Holding:** flat after `30` min [default]. Fixture (*example*): `p_{t-30} = 100.00`, `p_t = 100.15` -> `r = 15` bps `>= 10` -> LONG. Documented decay: the effect attenuates within minutes - the holding window is the edge [documented].
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert len(bars) > cfg.lookback_min else return FLAT(DEGRADED)  # [default]
    r = bars[-1].c / bars[-1-cfg.lookback_min].c - 1
    assert isfinite(r) else return UNKNOWN                        # F2
    ...
    sig = SignalVector(symbol, direction=side, confidence=min(abs(r)*1e4/30.0, 1.0), capital=0.0,
...
- Tape: `modules/fixtures/S025_tape.csv` — `# TYPE: validation-run`; `4` closes (`100.00 -> 100.15 -> 100.10 -> 99.95`), all numbers synthetic, seed `20260925` [example].
- Expected outputs: `modules/fixtures/S025_expected.csv`; per-bar lookback return (vs bar 1) in bps and trigger; hand-checked below.
- Tolerance: `1e-9` [default] on floats; integer contributions exact.
- Acceptance tests (`modules/tests/test_S025.py`, `6` tests): `1.` fixture recomputes to expected; `2.` stub emits valid `SignalVector` (direction in {+1,-1,0}, confidence/capital in [0,1]); `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input -> `UNKNOWN`; `6.` extra: stub is deterministic across calls.
...
- bar 2: `(100.15-100.00)/100.00 = 15` bps `>= 10` -> LONG ✓
- bar 3: `10` bps `>= 10` -> LONG (boundary) ✓
- bar 4: `-5` bps -> no trigger ✓
...
|---|---|---|---|---|
| Gao & Han (academic literature) [documented] | US equities intraday | intraday time-series momentum | `BEFORE-COST` | pattern evidence, not a tested rule |
| Heston & Sadka (academic literature) [documented] | US equities | intraday return patterns | `BEFORE-COST` | pattern evidence, not a tested rule |

Selection disclosure: the `2` sources surfaced by the chapter's research pass; the decay profile is the operative risk statement.
...
`../../images/S025_example.png` — synthetic `4`-bar momentum tape (seed `20260925` [example]): lookback returns and triggers.
...
1. Gao & Han (academic literature) [documented]. Intraday time-series momentum (no single canonical citation).
2. Heston & Sadka (academic literature) [documented]. Intraday return patterns (no single canonical citation).
...
- Claims of specific intraday-momentum Sharpe ratios circulate in trading literature; no checkable after-cost study found - treated as unverified.
...
> candidate Polygon.io Stocks Advanced (SIP L1) — cheapest data source candidate [internal-est]; compute pattern `per_event`; skip cross-sectional/overnight.
> Implement `signal(state, events, cfg) -> SignalVector` (Appendix b v1.0.0)
> with the §S3 normative pseudocode, including the cost-gate predicate
> `expected_cost_bps(...) <= k * edge_bps` and `assert fill_event >
> signal_event`. Wire F1–F5 (Appendix f v1.0.0): invalid input -> `UNKNOWN`,
> never interpolate. Log decisions per Appendix g v1.0.0. Secrets via env
> only. Run the fixture `modules/fixtures/S025_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S025_expected.csv`

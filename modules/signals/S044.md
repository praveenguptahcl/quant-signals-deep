...
sources:
  ...
  - "Chapter S4 event rows (seed 44): bar 16 %K=95.4/%D=95.0; bar 17 %K=94.3/%D=94.8/%R=−5.7 — chapter example values [documented-as-chapter-value]."
unverified_sources: []
consumes: []
regime_ids: []
...
| Row | Value |
|---|---|
| Module | S044 — Stochastic oscillator / Williams %R, v1.0.0 |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    K = 100 * (C - Ln) / (Hn - Ln)          # Ln/Hn over n bars incl. current
    D = SMA3(K);  R = K - 100                # Williams %R
    crossdn = (K[t-1] >= D[t-1]) and (K[t] < D[t]) and (K[t] > 80)
    direction = -1 if crossdn else 0
    edge_bps = abs(f["K"] - f["D"]) * 10.0                                       # [example]
...
- Acceptance tests (`modules/tests/test_S044.py`, `5` tests):
  `1.` fixture recomputes to expected (bar-16/17 %K/%D/%R hand-checks; single cross-down); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (high<low) → `UNKNOWN`.
...
**Selection disclosure:** the chapter's worked tape plus the documented construction reference; no verified after-cost study of the standalone crossover rule was pinned in this build [documented-as-absence].
...
3. Chapter S4 event values: bar 16 `%K=95.4`/`%D=95.0`; bar 17 `%K=94.3`/`%D=94.8`/`%R=−5.7` [documented-as-chapter-value].

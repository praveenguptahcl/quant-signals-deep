...
| Row | Value |
|---|---|
| Module | S035 — Short-term reversal, v1.0.0 |
...
|---|---|
| `sym` | `symbol` |
| `ret_form` (%, formation) | `r_form` |
| `ret_hold` (%, holding, ex post) | `r_hold` (never an input) |
...
`edge_bps` = `250` bps [example] — expected weekly long-minus-short reversal spread.
...
| Borrow (3-name short leg) | `1.00` | [example] — reason: short-leg borrow; C7 locate required |
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    borrow_bps = 1.00   # [example] short-leg borrow; reason in table
...
function signal(state, events, cfg) -> SignalVector:
    ...
    order = argsort(r_form)                       # 1 = worst formation return
    leg = +1 for order[:n_long], -1 for order[-n_short:], else 0
    # r_hold is EX POST: it must never appear on the right-hand side above
    edge_bps = 250.0                                       # [example]
...
- Acceptance tests (`modules/tests/test_S035.py`, `5` tests):
  `1.` fixture recomputes to expected (leg hand-checks; correlation `−0.302` and spread `+2.50`% portfolio checks); `2.` stub emits valid `SignalVector`; `3.` no-signal-bar fills (`fill_event_ts > computed_at`); `4.` cost-gate predicate passes on large edge, blocks on tiny edge (`k = 0.5` [default]); `5.` invalid input (NaN formation return) → `UNKNOWN`.
...
|---|---|---|---|---|
| Adjusted closes | float | daily | Tier 0–1 | Corporate-action-adjusted |
| Volume/ADV | int | daily | Tier 1 | Liquidity screen |
| Borrow/locate | reference | daily | Tier 1–2 | Short-leg feasibility |
| Earnings calendar | event | as-occurring | Tier 1 | Information-regime filter |

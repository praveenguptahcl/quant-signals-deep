...
| Row | Value |
|---|---|
| Module | S056 — ETF vs basket / iNAV arbitrage, v1.0.0 |
...
| Regime gates | Provisional: R001 stress — iNAV staleness; widen the bound or stand down |
...
|---|---|---|---|---|
| `cost_bound_bps` | float | `8.0` | `2–25` | example |
...
- Acceptance tests (`modules/tests/test_S056.py`, `5` tests):
...
| 8 | Overfitting the bound → tuning 8 bps on history until it "works" → detect: — → action: bound comes from your fee schedule, not optimization → recovery: re-derive | `bound_from_fee_schedule` |

...
changelog:
  ...
  - "1.1.0 2026-09-11: deep review vs template v1.0.0 — re-pinned SEC §31 to FY2026 $20.60/million (was stale $0.00), fee_schedule_as_of 2026-09-11, composite taker/maker 0.57→0.78 / 0.23→0.45; borrow_bps renamed borrow_bps_per_day; regime-gate direction enum fixed (kills→inverts); calibration grids added for z_exit/max_spread_ticks/cost_gate_k/forecast_horizon_ms; staleness TTL (3 s) enforced in normative pseudocode + reference stub + new test; cost-gate full-path test added (k=0 blocks)"
---
...
| Row | Value |
|---|---|
| Module | S001 — Order-flow imbalance (Cont–Kukanov–Stoikov), v1.1.0 |
...
| SEC Section 31 (`$20.60`/million ÷ `$231.40` ref px) | `0.21` | [documented] — `$20.60`/million eff. `2026-04-04` (was `$0.00`/million through `2026-04-03`; FY2025 advisory had `$27.80`/million through `2025-05-13`); **sell-side-only** — applied to the reference taker stack as a [example] conservative convention |
| FINRA TAF (`$0.000195`/share ÷ `$231.40`) | `0.01` | [documented] — eff. `2026-01-01`, max `$9.79`/trade; sell-side-only — same [example] convention |
...
| **Total reference** | `0.78` | [example] composite (`0.43 + 0.13 + 0.21 + 0.01`) |

`fee_schedule_as_of`: `2026-09-11` [default — deep-review date];
component pins: TAF eff. `2026-01-01` [documented], SEC §31 FY2026 advisory
`2026-02-27` [documented], exchange tier fee [example]. Regulatory rates move
yearly — re-pin the venue schedule before production. `side: taker`
[default]; venue/tier/source: XNAS / Databento MBP-1 [example]. T-modules
consuming this stack should zero the §31/TAF rows on the buy leg (both are
sell-side-only); the composite keeps them as a [example] conservative bound.
...
def expected_cost_bps(notional, adv_pct, venue, side, urgency) -> float:
    ...
    sec31_bps    = 0.21   # SEC §31 $20.60/million eff. 2026-04-04 [documented] (sell-side-only)
    taf_bps      = 0.01   # FINRA TAF $0.000195/share eff. 2026-01-01 ÷ $231.40 [documented] (sell-side-only)
    ...
    if side == "maker":
        fee_bps = -0.20 + sec31_bps + taf_bps  # rebate [example]; §31/TAF kept as conservative composite
...
2. **Grid:** `window_events ∈ {5, 10, 20, 50}` [example];
   ...
   **Remaining parameters (each gets a grid + OOS selection rule):**
   - `z_exit ∈ {0.25, 0.5, 0.75}` [example] × the chosen `z_entry` —
     select on round-trip net P&L retention vs re-entry churn [default];
     OOS folds only; too-close `z_exit` churns exits, too-far gives back
     the move.
   - `max_spread_ticks ∈ {2, 3, 4}` [example] — select by entry rate vs
     adverse-fill rate on the OOS fold [default]; keep `3` [default] unless
     a flicker regime dominates the bucket (entry rate collapses).
   - `cost_gate_k ∈ {0.25, 0.5, 1.0}` [example] — per the proposal's open
     item (`0.5` vs T002-style `2`): select by maximizing cost-aware OOS
     utility subject to a minimum OOS trade count of `500` [default];
     keep `0.5` [default] until `[measured]` cost stacks justify otherwise.
     Never set `cost_gate_k` above `2.0` [default] without documented
     impact accounting — the gate is the only barrier to toxic fills.
   - `forecast_horizon_ms` stays `[example]` (fixed at `500` ms) — select
     from the lead-lag R² decay knee [default]: compute lead-lag R² at
     `{100, 250, 500, 1000, 5000}` ms [example]; pick the horizon past
     which marginal R² gain is `< 10`% [default]. `cooldown_s` (`60` s
     [default]) is C10 hygiene, not fitted — leave fixed.
...
| R001 | inverts | extreme RV: spread convexity — costs grow faster than edge; depth evaporates | `p > 0.95` (R001 extreme band) [example] | halve confidence [default]; emit `DEGRADED` | `1` session [example] | veto entries |
| R001+R010 | inverts | extreme RV *and* extreme spread: costs exceed any plausible per-trade edge | R001 `p > 0.95` AND R010 spread percentile `> 90` [example] | veto entries (direction `0`) [default] | `1` session [example] | veto entries |
...
function signal(state, events, cfg) -> SignalVector:
    ...
    assert e.asof_ts - e.event_ts <= 3e9 else return UNKNOWN              # F4: staleness TTL 3 s [default]
    ...
    # t→t+1 causality is asserted by the test_cmd's test_no_signal_bar_fills:
    # fill_event_ts > sig.computed_at for every fill event (fills are broker-layer).
    return sig
```
...
- Acceptance tests (`modules/tests/test_S001.py`, `12` tests):
  ...
  direction `0`; `10.` maker-rebate branch (`0.78` taker / `0.45` maker
  [example] — FY2026 §31 pin); `11.` stale arrival
  (`asof_ts − event_ts > 3` s [default] TTL) → `UNKNOWN` without advancing
  `prev_book`; `12.` cost gate through the full stub path: `cost_gate_k = 0`
  vetoes the pinned `z = 5.73` edge to direction `0`.
...
3. Replication: `xecuterisaquant/replication-cont-ofi`,
   ...
   not peer-reviewed.
4. U.S. SEC, Section 31 Transaction Fee Rate Advisory for Fiscal Year 2026
   (Feb `27`, `2026`) [documented].
   https://www.sec.gov/rules-regulations/fee-rate-advisories/2026-2 — §31 rate
   `$0.00`/million for charge dates through Apr `3`, `2026`; `$20.60`/million
   from Apr `4`, `2026`, remaining in effect until `60` calendar days after
   FY2027 appropriation legislation [documented]. Supersedes the module's
   earlier FY2025-advisory pin (see changelog 1.1.0).

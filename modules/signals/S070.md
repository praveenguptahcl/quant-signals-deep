...
| Row | Value |
|---|---|
| Module | S070 — Straddle-implied move vs realized, v1.0.0 |
...
| Regime gates | Provisional: event regime --- requires VRP positive (S069) and unusual-activity confirmation (S073) |
...
2. **Cheapest-source row:** min_tier = Tier-0 Cboe delayed chains + SEC EDGAR calendars --- cheapest data source candidate [unverified]; latency_class = per_event, point-in-time adjusted;
   required_fields = ATM option bid/ask snapshots, underlying price, earnings/macro calendar, ≥`12` events/name [example]; price_band = `~$0`/mo [unverified] (indicative --- verify before budgeting); build_or_buy = **build the comparator yourself on bought chains --- the formula is a desk standard**.
...
vector (`edge_history`, `module_state`); `events` are canonical Events
(Appendix a v1.0.0), newest last. The function handles documented edge cases:
empty input → `UNKNOWN`; non-positive/non-finite implied or realized →
`UNKNOWN` (F1/F2, never interpolate); <`2` [example] events → z = `0` [example],
...
|---|---|---|---|---|
| `z_star` | float | `1.0` | `0.5–2.0` | example |
| `iv_pct_gate` | float | `70.0` | `50–90` | example |
...
```
FADE_RICH    := (z > z_star) AND (iv_pct > iv_pct_gate) AND (expected_cost_bps(...) <= cost_gate_k * edge_bps)   # sell the rich straddle [example convention]
...
`z = (edge − μ_L) / σ_L` vs trailing `L = 6` [example] events (fixture convention);
...
| Fade threshold | `z > 1.0` [example] AND `iv_pct > 70` [example] |
...
| Borrow | `0.00` | [default] — reason: short premium via straddle; no stock borrow |
...
| History lookback | `L` | `6` events (fixture); `8`–`20` live | [example] | high |
| Fade threshold | `z_star` | `1.0` | [example] | high |
| IV percentile gate | `iv_pct_gate` | `70` | [example] | medium |
...
function signal(state, bars, cfg) -> SignalVector:
    ...
    b = bars[-1]
    if any(v <= 0 or not finite(v)) for v in (b.implied_move, b.realized_move): return UNKNOWN   # F2 [default]
    edges = [r.implied_move - r.realized_move for r in bars]             # edge [example]
    mu = mean(edges); sd = std(edges) if len(edges) > 1 else 0.0
    z = (edges[-1] - mu) / sd if sd > 0 else 0.0                         # z vs history [example]
    direction = -1 if (z > cfg.z_star and b.iv_pct > cfg.iv_pct_gate) else 0   # fade rich straddle [example]
    confidence = min(1.0, z / 2.0) if direction else 0.0                  # [example] scale
    edge_bps = z * 80.0                                                 # modeled per-trade edge [example]
    assert fill_event > signal_event                                    # causality: realized move measured post-print at t+1
...
Causality: computed from the chain snapshot *before* the event close; the position is opened pre-announcement and the realized move is measured after --- strictly causal, no lookahead, but the signal only resolves at *t+1*. Mandatory unit test: no-signal-bar fills.

## §S4 Worked example (fixture-backed)

- Tape: `modules/fixtures/S070_tape.csv` — `# TYPE: validation-run`;
  `6` rows (synthetic `6`-event [example] tape, fictional tickers, seed `70`), all numbers synthetic,
...
- Acceptance tests (`modules/tests/test_S070.py`, `5` tests):
  ...
  `5.` invalid input (non-positive implied/realized) → `UNKNOWN`
...
|---|---|---|---|---|
| Options quotes (ATM chain) | bid/ask | intraday snapshots / EOD | Tier 1–2 | OPRA via Databento/Cboe LiveVol; delayed Cboe for prototyping |
| Underlying price | float ($) | 1-min / daily | Tier 0–1 | implied-move denominator and realized move |
| Earnings/macro calendar | dates | daily | Tier 0–1 | SEC EDGAR, vendor calendars |
...
> Build module **S070 — Straddle-implied move vs realized** per
> `notes/module-template.md` v1.0.0 and this file's §S0 contract.
> Cheapest path (§1.5): prototype `20`–`40` h [internal-est]; cheapest data
> candidate Tier-0 Cboe delayed chains + SEC EDGAR calendars --- cheapest data source candidate [unverified]; compute pattern
> `per_event`; skip ex-event IV stripping and the forward-denominator variant for now.
> Implement `signal(state, events, cfg) -> SignalVector` (Appendix b v1.0.0)
> with the §S3 normative pseudocode, including the cost-gate predicate
> `expected_cost_bps(...) <= k * edge_bps` and `assert fill_event >
> signal_event`. Wire F1–F5 (Appendix f v1.0.0): invalid input → `UNKNOWN`,
> never interpolate. Log decisions per Appendix g v1.0.0. Secrets via env
> only. Run the fixture `modules/fixtures/S070_tape.csv`
> (`# TYPE: validation-run`) against `modules/fixtures/S070_expected.csv`

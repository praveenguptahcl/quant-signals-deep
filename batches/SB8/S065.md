...
Provenance **[D]**: ARCH (Engle 1982) and GARCH (Bollerslev 1986) are documented, Nobel-cited
...
Economically, GARCH works as a sizing input because of **adverse selection and inventory
risk**: spreads widen and algos slow down when conditional vol is high, so expected slippage
scales with σ. Picture it: 11:20, NVDA prints three −40 bp bars in a row. Your GARCH σ steps
up; your participation cap drops from 15% to 8%; overnight carry is cut before the close.
...
```
where **σ²_{t,i}** is the *conditional variance* (the variance of the next return given all
...
- **GJR-GARCH**: σ² = ω + α·ε²_{t−1} + γ·ε²_{t−1}·1{ε_{t−1}<0} + β·σ²_{t−1} — bad news hits harder.
- **EGARCH**: `ln σ²_t = ω + α(|z_{t−1}| − E|z|) + γ·z_{t−1} + β·ln σ²_{t−1}` — log form, no
...
how the three pieces (season × GARCH → σ path → returns) assemble. Not a backtest.**
...
Hand-checkable steps (first 3 bins of the script's output): h₀ = 4.0×10⁻⁶ (20.0 bp). With the
seed-65 first Gaussian draw z₀ ≈ −0.28 (illustrative; the script computes it), ε₀² ≈
(0.28 × 20bp)² ≈ 31.4×10⁻⁸ → h₁ = 2.0×10⁻⁷ + 0.06×31.4×10⁻⁸ + 0.89×4.0×10⁻⁶ ≈ 3.78×10⁻⁶
(19.4 bp) — the persistence term dominates, as it should with β = 0.89.
...
bounce that inflates real 5-minute σ estimates.
...
- **T017 — Scheduled Macro-Announcement Drift (sizing):** scales pre-announcement position size
  by the GARCH one-day-ahead forecast (consumes S065 alongside S034/S092 in the plan's
  composition — role: vol-targeting sizer).
...
| Python + polars/numpy hand-rolled | If you need the recursion inside a hot loop with custom likelihoods |
| Rust | Only for tick-level per-event σ updates in the execution path |
...
| Tier 2: Databento | L1 for exact bar construction | ~$200/mo + usage | Microstructure-honest bars | Overkill for a daily refit |
...
3. **Gaussian innovations on fat-tailed residuals** — underestimates tail σ, VaR breaches.
   Mitigation: Student-t innovations; backtest VaR coverage (Kupiec/Christoffersen).
4. **Jumps contaminating the recursion** — a single jump bar permanently lifts σ̂. Mitigation:
   winsorize or jump-filter (S064) the input residuals; consider GJR/EGARCH for asymmetry.
...
6. **Overfitting via model mining** — GARCH vs EGARCH vs GJR vs FIGARCH chosen by in-sample
   likelihood. Mitigation: fix the family a priori; compare out-of-sample on a frozen test set.

...
| **When it works** | Around scheduled events (earnings, CPI, FOMC) where implied volatility is systematically elevated pre-announcement and realized moves come in below implied *on average*. |
...
- The P&L is decided in dollars after the spread, not in vol points: a 60% hit rate with 20% round-trip costs can still lose money.
...
What to notice: even in this friendly toy tape, 4 of 10 events lose — E2, E3, E6, E10 are the genuine-shock cases where realized exceeded implied. The example has no partial fills, no early assignment, no macro overlap, and uses mid-based implied moves; a real backtest must execute both legs at bid/ask and report the worst decile alongside the mean.
...
Data-quality checklist: snapshot the *actual* chain at the signal timestamp (no closing-chain lookahead), remove stale quotes, handle early close/half-days, corporate actions shifting strikes, American-exercise premium on single names.
...
| Tier-1 retail (Polygon Options, Cboe delayed products) | Queryable chains, Greeks | ~$0–200/mo | Cheap full pipeline prototype | Incomplete expiries, rate limits |
...
- **Cost blowup:** the 20%-of-premium arithmetic above — cheap wings are untradeable after costs; model both legs at bid/ask plus fees.
...
- **Microstructure confusion:** the straddle mid moves with the underlying into the print — measure the move consistently or delta-hedge.
...
flowchart LR
    ...
    ING -->|ATM strip + spot series| FEAT["Feature compute\n(straddle/S, realized |return|, z-score)"]
    FEAT -->|per-event edge series| SIG["Signal S070\nimplied-vs-realized edge"]
    SIG -->|edge z-score + IV percentile| GATE{"Cost / toxicity\ngate?"}

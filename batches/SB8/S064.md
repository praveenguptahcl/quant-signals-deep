...
| **When it dies** | As a standalone alpha: it measures volatility, it does not predict direction. On raw 1-minute returns without deseasonalizing, microstructure noise corrupts the split. |
...
jump series (gating, ignition) are practitioner reconstructions.

### S2. How it works — plain human explanation

At 14:02 on a Fed-day afternoon, SPY prints a +0.9% bar in five minutes, then drifts back. A
...
discontinuous repricing on news, the other was genuinely elevated diffusive trading.
...
macro prints, block repricings — and the literature finds individual-stock jumps cluster on
prescheduled announcements (Lee & Mykland 2008). Jumps are "highly prevalent and distinctly
less persistent than the continuous sample path variation" (ABD 2007), so a forecast that
...
**Jump component:**
```
J_t  = max(RV_t − BV_t, 0)            (truncation is a practical fix; negative finite-sample
RJ_t = (RV_t − BV_t) / RV_t            estimates are clamped at zero — Barndorff-Nielsen & Shephard 2004)
```
...
the arithmetic is hand-checkable. Nothing here is a backtest.** Script:
...
toy has no fees, no bid–ask bounce, and no microstructure noise, all of which inflate RV on real
1–5-minute data.
...
| Tier 2: Databento Standard (MBP-1) | L1 quotes+trades for exact bar builds | ~$200/mo + usage | Honest microstructure, your own grid | Overkill if you only need 5-min RV |
| Academic route: LOBSTER / WRDS TAQ | Replay-grade data for research | ~hundreds/yr academic | Publication-grade replication | Access friction, not real-time |
...
quality and empirical regularities, not trading Sharpe. There is no credible after-cost trading
Sharpe for "the bipower signal" as a standalone trigger, and none is claimed.

| Study / source | Market & period | Metric | Before/after cost | Caveat |
|---|---|---|---|---|
| Andersen, Bollerslev & Diebold (2007), *Rev. Econ. & Stat.* — HAR-RV-CJ model separating continuous/jump components | DM/$, S&P 500, 30-yr Treasury yield | Significant out-of-sample volatility-forecast improvements; almost all predictability from the *non-jump* component | Before cost (forecast evaluation, not a P&L backtest) | Forecast-accuracy metric, not tradable P&L; gains are in the continuous component you *keep*, not the jump you removed |
...
| IHSG intraday study (2025–2026), 5-min bars, 93 days — bipower decomposition of Jakarta Composite | Indonesia, Dec 2025–May 2026 | Jump component ≈ 11.77% of daily RV on average; 32.3% of days show significant jumps; share rises to 16.36% on stress days | Before cost (descriptive) | Single emerging market, short sample; not an out-of-sample trading test |
...
clusters where jumps are expected and priced; vol-of-vol regimes where the Gumbel critical
value miscalibrates.
...
1. **Microstructure noise masquerading as jumps** — on 1-min raw returns, bid–ask bounce
   inflates RV−BV. Mitigation: 5-minute (or coarser) sampling; noise-robust pre-averaging variants.
...
7. **Truncation bias** — max(RV−BV, 0) biases small-sample jump estimates upward. Mitigation:
   use formal tests (LM or BNS z-test) for significance, not raw truncation.

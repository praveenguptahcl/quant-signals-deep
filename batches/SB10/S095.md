...
Cash-and-carry P&L: $\Pi_{\mathrm{gross}} \approx \sum_j N_j f_j + \Pi_{\mathrm{basis}}$ with $\Pi_{\mathrm{basis}} = (S_1-S_0) + (P_0-P_1)$ per $1 matched (zero if basis unchanged); net: $\Pi_{\mathrm{net}} = \Pi_{\mathrm{gross}} - N_{\mathrm{spot}}(c_{\mathrm{entry}}+c_{\mathrm{exit}}) - N_{\mathrm{perp}}(c_{\mathrm{entry}}+c_{\mathrm{exit}}) - \mathrm{borrow} - \mathrm{margin} - \mathrm{slippage}$.

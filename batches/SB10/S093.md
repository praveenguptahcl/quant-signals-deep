...
**Step 1 — hand-computed novelty (centroid form).** Four prior stories: $A=[1,0,0,1,0,0]$ (earnings+downgrade), $B=[1,0,0,1,0,0]$ (same, rewritten), $C=[1,0,0,0,1,0]$ (earnings+dividend), $D=[0,1,0,0,0,1]$ (lawsuit+fraud). The trailing centroid $\bar{c} = \tfrac{1}{4}(A+B+C+D) = [0.75,0.25,0,0.50,0.25,0.25]$.

Focal story $E$ (a stale downgrade rewrite): $[1,0,0,1,0,0]$. $\cos(E,\bar{c}) = \frac{0.75+0.50}{\sqrt{2}\cdot 1.0} = \frac{1.25}{1.4142} = 0.8839$ ✓ (hand-check: dot $=1.25$; $\|\bar{c}\| = \sqrt{0.5625+0.0625+0.25+0.0625+0.0625} = \sqrt{1.0} = 1.0$). $\mathrm{Novelty}_E = 1 - 0.8839 = \mathbf{0.12}$ → stale.

Focal story $F$ (first-ever merger rumor): $[0,0,1,0,0,0]$. $\cos(F,\bar{c}) = \frac{0}{1\cdot 1.0} = 0.00$ ✓. $\mathrm{Novelty}_F = 1 - 0.00 = \mathbf{1.00}$ → novel.

...
Let story $i$ have embedding (or TF-IDF vector) $e_i$, and let $P_i$ be the comparison set of prior stories about the same firm (Tetlock 2011: the previous 10 stories; this batch's reconstruction uses a 24-hour or 30-day window — *example*). The **staleness** is the maximum cosine similarity to any prior story; the **novelty** is its complement:

$$\\mathrm{Staleness}_i = \\max_{j \\in P_i} \\cos(e_i, e_j), \\qquad
\\mathrm{Novelty}_i = 1 - \\max_{j \\in P_i} \\cos(e_i, e_j),$$

with $\\cos(u,v) = \\frac{u \\cdot v}{\\|u\\|\\,\\|v\\|}$. In production form:

$$N_i \\propto (1 - \\max_j \\cos(e_i,e_j)) \\cdot \\log(1 + \\text{source diversity}) \\cdot w_{\\text{event-type}}$$
...
**Causal timing:** novelty is computed at story arrival $t$ using only stories with timestamps $\le t$ (trailing set); tradable no earlier than the first quote after $t+1$ that the venue's timestamps honestly support. **Variants:** (1) *centroid distance* — novelty vs the trailing 10-story *centroid* instead of max-similarity (smoother, less outlier-driven); (2) *event-type-conditional* — separate novelty models per event type (earnings vs legal vs analyst); (3) *cross-source novelty* — penalize stories that merely repeat what another outlet reported first (wire-speed dedup).
...
**Step 1 — hand-computed novelty.** Four prior stories: $A=[1,0,0,1,0,0]$ (earnings+downgrade), $B=[1,0,0,1,0,0]$ (same, rewritten), $C=[1,0,0,0,1,0]$ (earnings+dividend), $D=[0,1,0,0,0,1]$ (lawsuit+fraud).

Focal story $E$ (a stale downgrade rewrite): $[1,0,0,1,0,0]$. $\cos(E,A) = \frac{2}{\sqrt{2}\,\sqrt{2}} = 1.00$ ✓ (hand-check: dot $=1\cdot1+1\cdot1=2$, norms $\sqrt{2}$ each). $\mathrm{Novelty}_E = 1 - 1.00 = \mathbf{0.00}$ → stale.

Focal story $F$ (first-ever merger rumor): $[0,0,1,0,0,0]$. $\cos(F,D) = \frac{0}{1\cdot\sqrt{2}} = 0.00$ ✓; all four cosines are 0. $\mathrm{Novelty}_F = 1 - 0.00 = \mathbf{1.00}$ → novel.
...
def novelty(story_text, prior_texts):            # prior_texts: trailing set P_i, ts <= t
    ...
    cos = e[0] @ e[1:].T                         # cosine vs each prior story
    return 1.0 - float(cos.max())                # Novelty_i
...
- Duck.ai Q-SB10-1/Q-SB10-3 novelty formulas: Novelty_i = 1 − max cos(e_i,e_j) over 24h/30d sets; production N_i ∝ (1−max cos)·log(1+source diversity)·w_event-type (verified arithmetic, framing unverified).

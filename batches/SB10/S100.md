...
Hand-check: A: ln(51.40/50.00) = 2.76% − 0.30% = **2.46%**. G: ln(19.72/20.05) = −1.66% − (−0.20%) = **−1.46%**. Quintile mean intraday ARs: Q1 **−1.67%**, Q2 −0.88%, Q3 +0.05%, Q4 +0.92%, Q5 **+1.92%** — monotonic in SUE. PEAD leg = 1.92% − (−1.67%) = **+3.59%** gross.
...
|---|---|---|---|---|
| Earnings announcement timestamps | event-level (minute/second) | 10+ years | IBES/Compustat via WRDS; Benzinga calendar (~$100–200/mo); Polygon (~$30–200/mo) | Binding constraint — minute precision minimum |
| Actual vs expected EPS | event-level | 10+ years | IBES (analyst) / Compustat (reported) | Point-in-time consensus; no restatements |
| Intraday bars | 1-minute | matches events | Databento (~$200/mo + usage); LOBSTER (hundreds/yr) | First-tradable-quote needs sub-minute or auction data |
...
- Timestamp/intraday implementation details for modern PEAD trading (auction-entry mechanics, first-quote identification) — practitioner knowledge; dedicated published sources not captured this batch.
- Vendor pricing for Benzinga earnings calendar (~$100–200/mo), Polygon corporate actions (~$30–200/mo), and Databento intraday (~$200/mo + usage) per the cost model — indicative, verify before budgeting.

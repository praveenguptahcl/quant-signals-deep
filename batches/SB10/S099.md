...
```mermaid
flowchart TB
    source["SOURCE — Google Trends<br/>(weekly SVI, 0-100)"]
    ingest["INGEST — joint downloads,<br/>query-definition freeze (weekly SVI)"]
    clean["CLEAN — common-benchmark rescaling,<br/>re-pull validation (weekly series)"]
    feat["FEATURE — ASVI z-form (winsorized)<br/>+ canonical log-median (weekly panel)"]
    pred["PREDICT — attention filter /<br/>event-confirmation score (weekly scores)"]
    exec["EXECUTE — condition downstream<br/>signals (weekly decisions)"]

    ctx["CONTEXT — event calendar<br/>(weekly)"]
    risk["RISK — tiny-denominator cap,<br/>query-drift monitor (per-signal)"]

    source -->|"weekly SVI"| ingest
    ingest -->|"weekly SVI"| clean
    clean -->|"weekly series"| feat
    feat -->|"weekly panel"| pred
    pred -->|"weekly scores"| exec
    ctx -.->|"weekly events"| pred
    risk -.->|"per-signal gate"| exec

...
- **T025 — Trade-Classification Trend Filter — filter/conditioner**: conditions momentum entries on sign persistence — a breakout accompanied by persistent signs is more likely genuine continuation; one against persistent opposing flow is faded or vetoed.
...
flowchart LR
    ...
    FEAT -->|classified sign series| SIG["Signal S018\nsign autocorrelation C(τ)"]
    SIG -->|C(tau) curve| GATE{"Cost / toxicity\ngate?"}
    GATE -->|pass| OUT["Downstream consumer\n(execution / T025 filter)"]

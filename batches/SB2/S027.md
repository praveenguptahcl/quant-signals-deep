...
**Base signal (Gao–Han–Li–Zhou form).** Let $P_{9:30,t}$, $P_{15:30,t}$, $P_{\text{close},t}$ be prices on day $t$. Define
...
Gao et al. report $\hat\beta = 0.0694$ (their tables scale by 100, showing 6.94), significant at 1%, $R^2 = 1.6\%$ on SPY 1993–2013. The tradable signal is the sign rule:
...
- **T006 — Intraday Trend + Vol-Regime Allocator** — *trend leg*: S027 supplies the into-the-close momentum component, scaled by HMM vol regime (S079) alongside S025.
...
|---|---|---|---|---|
| O/H/L/C | float | 1-min bars, RTH | Tier 0–1 | Only 9:30, 15:30, close anchors strictly needed |
...
```mermaid
flowchart LR
    FEED["Raw feed\n(SIP 1-min bars +\nauction imbalance)"] --> ING["Ingest + normalize\n(anchors 9:30/15:30,\nDST, halts)"]
    ING --> FEAT["Feature compute\n(r_first, r_last\nper 1-min bars)"]
    FEAT --> SIG["Signal S027\nsign(r_first)\n→ hold to close"]
    SIG --> GATE{"Mechanism check\n(info vs mechanical\nflow day)?"}

...
**Jargon first.** An *ETF* (exchange-traded fund) trades like a stock but holds a basket. *iNAV* (intraday NAV, 15-second cadence) is the issuer's live estimate of the basket's worth. The *premium/discount* is (ETF price − iNAV)/iNAV — negative means the ETF is cheap vs its basket. *Authorized Participants (APs)* are institutions contractually allowed to *create* ETF shares (deliver basket → receive shares) or *redeem* them (reverse) in *creation units* (typically 25,000–100,000 shares — *unverified chatbot claim*, Q-TB4-3 §5). A *block trade* is a single large print (≥ 10,000 shares, *example* — S094's default) that usually marks institutional activity. *RVOL* (relative volume) is today's volume in a time-of-day bin divided by its trailing average — 2.6× means the bin traded 2.6× its normal volume. *BlockImb* is signed block imbalance: (buy-block volume − sell-block volume)/total block volume, in [−1, +1], with trade direction signed by the Lee–Ready (1991) quote/tick test. *Basis* is futures-minus-cash. *Information share* (Hasbrouck 1995) is which venue discovers price first. *RTH* = regular trading hours.

**Universe & session.** US-listed equity ETFs with a licensed 15-second iNAV, 20-day ADV ≥ $20M, median quoted spread ≤ 4 bps (*example* filters), and a liquid futures proxy (index ETFs via MES/MXF-class micro futures). Exclude: international ETFs with closed home markets (stale-iNAV mirage — Engle & Sarkar 2006), ex-div and index-rebalance days, first/last 15 minutes. Session 09:45–15:45 ET, flat 15:45 (*example*). This is the *statistical proxy* variant of S056 (non-AP: short the rich leg, buy the cheap leg, exit on convergence) — the true creation/redemption loop is described for completeness but is **not executable** without an AP agreement; see T009 for the lead-lag-confirmed sibling of this trade (T009 uses S056+S059+S014; this chapter replaces lead-lag/spread with flow confirmation S094+S032).

**Entry rule.** At each 1-min snapshot *t*, compute premium_t (S056), RVOL_t over the trailing 15-min window vs the same-window 20-day mean (S032), and BlockImb over the trailing 15 min (S094). **Enter** (signal at *t*, fills at *t+1*) when all four hold:

- |premium_t| > 8 bps — the all-in cost bound c (*example*, S056's default);
- RVOL_t ≥ 2.0 (*example*) — the dislocation is happening on institutional-size flow, not a quote flicker;
- sign(BlockImb_t) agrees with the *fade direction*: discount (buying cheap ETF) requires BlockImb ≥ +0.3 (*example*) — buy blocks; premium (shorting rich ETF) requires BlockImb ≤ −0.3 (*example*) — sell blocks. Blocks leaning *against* the fade = veto (someone informed is pushing the "wrong" way, or the iNAV is stale);
...
**Position sizing.** Risk on the *premium*: stop distance (14 − 8) bps × P_ETF ≈ $0.07–0.08/share on a $120 ETF (*example*). N_ETF = ⌊R / stop-distance⌋ with R = $300 (*example*) → ~1,000 shares at $120. Futures proxy: beta-equivalent notional (e.g. $120k ETF ⇒ 4 micro contracts at $5/pt × 6,000 = $30k each). Portfolio: max 4 concurrent dislocations, gross ≤ 2× equity on a $1M paper book (*example*).

**Risk limits** (*example* unless noted). Daily loss stop −1.5%; per-trade hard stop −$500; iNAV-staleness kill switch (no fresh print in 60 s ⇒ no new entries); futures-halt kill switch (flatten both legs — never hold the proxy naked); borrow-flag gate on the short-ETF mirror.

**Cost model.** 5 bp one-way on notional per equity leg turn (*example*, TB4 shared convention from Q-TB4-1); micro futures $0.75/contract round-trip (*indicative — verify before budgeting*); half-spread embedded in fill prices; no financing (intraday), no AP create/redeem fee (non-AP — you never touch the primary market). Applied explicitly in T4.

**Order/execution sketch.** Simultaneous marketable limits on ETF + futures legs at the *t+1* snapshot; ETF participation ≤ 10% of visible depth (*example*); one leg rejects ⇒ cancel the other within 1 s (*example*) — an unhedged premium bet is not this strategy. Latency honesty: on SIP/L1 this is `simulated only — requires MBO/ITCH`; the 8-bps entry bound, not queue modeling, is what keeps the example honest. Backtest causality: signal at *t* ⇒ fill at *t+1*, purged/embargoed validation (S088) on every threshold.
...
**Trade T1 — discount fade, blocks confirm (winner).** Snapshot *t*: premium −10.2 bps (iNAV ≈ 120.563, ETF mid 120.44; < −8 ✓), 15-min RVOL 2.6 (≥ 2.0 ✓), BlockImb +0.55 (buy blocks agree with buying cheap ✓). Fill at *t+1*: long **1,000 SYNETF @ 120.44**, short **4 MXF @ 6,000.00**. Exit ~35 min later: premium −3.1 bps (< −4 ✓): sell 1,000 @ **120.72** (+280.00), cover 4 MXF @ **6,001.20** (−24.00).
...
**Trade T2 — premium fade, blocks confirm (winner).** Premium +9.1 bps, RVOL 2.2, BlockImb −0.48 (sell blocks agree with shorting rich ✓): short **800 SYNETF @ 121.10**, long **3 MXF @ 6,010.00**. Exit at +2.8 bps: cover @ **120.86** (+192.00), sell futures @ **6,011.50** (+22.50). Gross +214.50; costs (96,880+96,688)×0.0005 = −96.78, futures −2.25; **net +115.47**. Cumulative: +247.89.

**Trade T3 — discount fade, stop (loser).** Premium −9.4 bps, RVOL 2.4, BlockImb +0.41: long **900 @ 119.80**, short **4 MXF @ 5,990.00**. The discount *widens* — APs step back, the basket keeps falling: stop at premium −14.2 bps (≥ 14): sell @ **119.52** (−252.00), cover @ **5,991.60** (−32.00). Gross −284.00; costs −110.69; **net −394.69**. Cumulative session: +132.42 + 115.47 − 394.69 = **−146.81**.

**Where the example is optimistic.** The losing trade is the honest one: T3 shows exactly how this book dies — the dislocation widens past the stop while the futures hedge *also* loses (basis against you). Optimisms that remain: blocks are assumed perfectly signed by Lee–Ready on SIP data (midquote prints mis-sign ~15–25% of the time in practice — the confirmation filter is noisier than shown); the iNAV is assumed assemblable (a real 15-second iNAV is indicative, and the basket may not trade at those prints — S056's S10 warning); fills at the touch with no partial fills; the futures proxy is assumed basis-neutral for an ETF it only approximates; financing/borrow on the short-ETF mirror are stubbed. The session's −$146.81 on ~$330k notional is the point: after-cost, the non-AP proxy needs roughly a 2:1 win ratio just to tread water — the example is arithmetic, not a backtest, and makes no performance claim.

### T5. Data & infra — what must run

**Pipeline inventory.** Pre-open: iNAV license check, 20-day time-of-day volume baselines per ETF, borrow flags for short-mirror names. Intraday (09:45–15:45 ET): 15-second iNAV poller with staleness watchdog (no fresh print in 60 s ⇒ no new entries); ETF L1 stream; micro-futures stream; a 1-min snapshotter stamping premium_t, RVOL_t, BlockImb_t; Lee–Ready signer on the consolidated tape (quote test with 5-s lag per the paper, tick test for midquote prints). Post-close: persist every snapshot + decision; rebuild volume baselines and block-size distributions overnight. Storage: snapshots tiny; L1 for traded names ~2–8 GB/symbol-day (cost-model §4) — record only traded names.

**M5 Max feasibility: feasible (Tier M).** Per cost-model §2 this is snapshot arithmetic: a few thousand 1-min evaluations/day, trailing means over 20 days — trivial; the Lee–Ready signing over 15-min windows is the heaviest step and runs in milliseconds on polars. RAM < 1 GB hot (§3). Engineering: 40–100 h Tier M+ harness (§5) ≈ $6,000–15,000 loaded-cost estimate — the work is the two-leg FSM, the iNAV-clock alignment, and the causality-proof backtest. Bottleneck: licensing the 15-second iNAV and aligning its clock with the futures feed, not compute. Breaks at: multi-ETF basket-assembly simulation (capital, not compute) or full OPRA-grade options-flow confirmation (not needed here).

**Feed-outage safe mode.** iNAV stale > 60 s → no new entries; existing positions run to convergence/stop on last-known premium. ETF quote hole → flatten ETF leg and futures hedge together. Futures halt/limit → flatten everything immediately (an unhedged premium bet is not the strategy). Clock skew > 50 ms vs NTP → halt all. Restart = flat + re-derive from the on-disk ledger.
...
Capacity: on liquid US equity ETFs the non-AP proxy's honest capacity is near zero — the AP loop manufactures shares and removes the mispricing; dislocations that pay (March-2020 bond ETFs, holiday-stranded international funds, thin thematic ETFs) are either AP-only or carry gap risk that isn't "arb." Documented decay: Jain et al. (2021) — algorithmic competition has been shrinking deviations. Failure regimes: AP step-back in stress (deviations widen past the stop exactly when liquidity is thinnest), stale-iNAV mirages on international/overnight baskets, futures-proxy basis blowouts.

**Honest bottom line.** For a ≤$1M paper operation: this is a superb *education* strategy and a poor *income* strategy — the example's −$146.81 session is the honest advertisement. The documented edge is single-digit bps per dislocation, the holding time is minutes, and the other side of your proxy trade is a professional AP desk with the actual creation loop. Paper-trade it to learn premium accounting, two-leg execution, block-flow reading, and cost-bound discipline; do not mistake a synthetic winner for a scalable return. An institutional desk monetizes this with AP status, creation-unit balance sheet, and co-located simultaneity — none of which a paper book has. (Contrast T009, the lead-lag-confirmed sibling: same verdict, different confirmation.)
...
3. **Block mis-signing** — Lee–Ready misclassifies midquote prints ~15–25% of the time on SIP data; the confirmation filter is noisier than the example. Mitigation: require |BlockImb| ≥ 0.3 (*example*) with a 15-min window (aggregation dilutes mis-signing); the RVOL ≥ 2.0 gate as a second opinion.
4. **AP step-back in stress** — dislocations widen beyond the 14-bps stop when liquidity is thinnest (the T3 trade). Mitigation: the hard stop, no averaging down, flatten-everything on futures halt.

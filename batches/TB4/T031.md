...
**Universe & session.** US common stocks, price ≥ $5 (`example`), average daily dollar volume ≥ $5mm (`example`), shortable (locate available). Same-GICS-industry pairing is preferred — two regional banks are economic substitutes; a bank and a biotech are not, no matter what the statistics say. Session: daily bars, RTH (regular trading hours) closes only. Exclusions: both legs must have no scheduled earnings or corporate-action events within ±2 days (`example`); halted names are dropped; names involved in announced mergers/spinoffs are force-exited.

**Formation (nightly/quarterly, `example` cadence).** Once per 6-month trading window (`example`), on a 252-day formation window (`example`) strictly before trading begins:
...
4. Run the Engle–Granger two-step on survivors: OLS (ordinary least squares) $\log P^A_t = \alpha + \beta \log P^B_t + \varepsilon_t$, ADF (Augmented Dickey–Fuller) test on residuals with MacKinnon critical values at 5% (`example`); keep pairs that pass. The **hedge ratio** $\hat{\beta}$ is the number of B-shares held per A-share.
...
**Position sizing.** Per-pair risk budget 0.25–1.0% of NAV at $2\sigma$ of the spread (`example`); e.g. on a $1mm book, a pair entered at $z=2$ risks ~$5,000 of adverse excursion. Portfolio heat: sum over open pairs of $|z|\times$dollar-risk $\le$ 4–8% of NAV (`example`); hard cap 20 simultaneous pairs (`example`); gross book 200–400% of NAV (`example`).
...
**Order/execution sketch.** At the $t+1$ open, send simultaneous marketable limit orders on both legs (cancel the second leg if the first fails — never leave one leg unhedged). Participation ≤10% of visible depth per leg (`example`). Shorts require a confirmed locate first.
...
All numbers are **synthetic**. Two fictional large-cap same-industry names, "Alpha" (A) and "Beta" (B). Formation (not shown): 252 days, 9 zero crossings (passes the ≥6 `example` gate), $\sigma_{\text{form}} = 0.020$, SSD small → pair accepted. Trading window: 20 days, generated with `rng = np.random.default_rng(131)` — a hand-shaped two-episode divergence path plus seeded noise, printed exactly as the script computed it and plotted in `images/T031_example.png`. Notional $100,000 per leg (`example`). Costs: 5 bp one-way per leg; borrow 50 bp annualized on the short leg (`example`). Entries/exits use the `example` thresholds from T2 ($|z|\ge 2$ enter, zero-cross exit).
...
**M5 Max feasibility: feasible (Tier M).** The screen is the classic cheap-pairs workload: per `notes/cost-model.md` §2, numpy vectorized math runs ~50–200M elements/sec and polars simple ops ~10–50M rows/sec. The 2,000-name daily panel is tens of MB; the 2,000² float32 distance matrix is ~16 MB by byte arithmetic (4M entries × 4 B) — computed in industry-chunked blocks rather than one dense 2M-pair blast. The EG/ADF step runs only on the few-thousand-pair shortlist (seconds to minutes); rolling z-scores on the live 20-pair book are trivial. RAM sits far below the 77 GB working budget (§3). A chatbot research lead (Grok, Q-TB4-2 — unverified) puts the full nightly batch at ~20–60 minutes wall clock, comfortably inside the close→open window. Engineering: screen Tier M 20–60 h plus strategy harness M+ 40–100 h → **~100–160 h ≈ $15,000–$24,000** `loaded-cost estimate` at $150/hr (§5).

**Storage growth:** daily bars for 3,000 stocks ≈ 5 MB/day parquet (§4) → ~1.3 GB/year; pair metadata (spreads, z-scores, formation stats) is megabytes. Archive freely.

**Feed-outage safe mode.** This is a daily-bar strategy, so an outage degrades gracefully: (1) if the nightly bar feed is late, freeze all new entries and new formations — manage only existing positions; (2) trade open pairs on the last-known $z$ with the stop tightened to $|z|>3$ (`example`); (3) if the borrow file is stale >1 trading day, veto any new short and shrink existing HTB shorts; (4) if the outage exceeds 3 trading days, flatten the book into the close — a pairs book flying blind on stale spreads is not a book, it is a hope.
...
**Verdict: build the screen and the cost/heat engine on Tier-1 data; rent borrow, locates, and execution from the prime broker.** The algorithm is ~100 lines of numpy; no vendor sells your thresholds. The crossover: if you will short anything beyond easy-to-borrow large-caps, price institutional borrow history *before* you trust any backtest — that single feed can exceed every other line combined.

### T7. Success-ratio evidence

| Study / source | Market & period | Metric | Before/after cost | Caveat |
|---|---|---|---|---|
| Gatev, Goetzmann & Rouwenhorst (2006) | US equities, 1962–2002, top-20 pairs | ~11% annualized excess return, self-financing, 1-day delayed execution | After the authors' conservative cost estimates | Pre-crowding sample; the number everyone quotes and nobody replicates today |
| Do & Faff (2010) | US equities, 1962–2009, top-20 pairs | 0.86%/mo (1962–88) → 0.37%/mo (1989–2002) → 0.24%/mo (2003–09) | Before explicit costs | The decay finding; pairs still spike in prolonged stress (2000–02, 2007–09) |
...
| Do & Faff (2012) | US equities, 1963–2009 | Baseline rule "largely unprofitable after 2002" once commissions + impact + short fees are charged | After costs | Grok-reported (Q-TB4-3) — unverified chatbot claim; paper is real, the specific phrasing is not independently re-verified |

**Capacity.** The tradable book is the liquid mid-cap same-industry set — not mega-cap (spreads too tight) and not micro-cap (borrow + impact). A chatbot lead (Grok, Q-TB4-3 — unverified) sizes it at roughly $10–50mm for a GC-only book.

**Decay.** Documented and steep: roughly a 70% fall in monthly excess from the first era to the last, driven more by worse convergence (non-convergent pairs 26% → 42%) than by simple crowding. Independent replications through 2020 show the post-2009 equity curve mostly flat even before costs.
...
7. **Cost blowup on illiquid legs** — the quality gate ignores economics; a high-$ZC$ pair can still be untradeable after spread. *Mitigation:* apply the cost/borrow gate *after* the quality gate, on net expected profit per trade.
8. **Quant-unwind / correlation breaks** — August-2007-style: every residual-reversion book dumps the same longs together. *Mitigation:* portfolio heat caps, gross limits, a drawdown kill switch, and no leverage on the pair book.
...
Source log: Grok answered Q-TB4-1..3 (2026-09-10); claims adopted into the evidence sections are cited to checkable papers above, and every other Grok claim is quarantined below as an unverified lead.

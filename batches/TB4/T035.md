...
*Batch TB4 · Strategy 35/100 · Signals S058, S066, S063*
...
**Universe & session.** One futures complex at a time (`example`: ES-like index futures, adjacent quarterlies M1 vs M2, $50 multiplier). Session: signals evaluated on daily settlement curves; entries at the next session's open; no new entries inside 10 days of front-month expiry (`example`). Exclusions: roll week, FOMC/macro event days where the curve reprices on rate expectations (your fair-curve inputs move faster than your prints), and any day either leg's bid–ask exceeds 2 ticks.
...
**Position sizing.** Vol-targeted: target daily P&L vol $4,000 (`example`) on a $100mm NAV book. Spread daily vol from the S063 Park estimator = 3.8 index points (`example`) → per-spread daily σ$ = 3.8 × $50 = $190 → N = floor(4000/190) = 21 → 20 spreads (`example` rounding). Cap: max 40 spreads per expiry pair (`example`); max 2 expiry pairs at once.

**Risk limits.** Daily loss stop −1% NAV (`example`); the curve book's gross is bounded by margin: 40 spreads × 2 legs × span margin must stay under 30% of NAV (`example`). Kill-switch: either leg's prints stale > 5 minutes, HAR forecast repricing above 25% (`example`), or a central-bank surprise — flatten; the fair curve is invalid until inputs settle.
...
| S063 range-based RV estimator | **Sizing** (risk) | Park estimator on 5-min bars → spread daily vol → N_spreads = target_σ$ / (σ_spread_pts × $50) |
...
**All numbers synthetic** (random seed 135 = stage number, `batches/TB4/plot_T035.py` — re-runnable). ES-like future, $50 multiplier, S = 5,000, T1 = 30/365, T2 = 60/365, r = 5.0%, q = 1.8%, y = 0. Fair values: `fair_1 = 5000·e^{0.032·0.0822} = 5013.2`, `fair_2 = 5000·e^{0.032·0.1644} = 5026.5` → fair spread 13.3 pts. Observed at entry: F_1 = 5014, F_2 = 5038 → observed spread 24.0 → **signal = +10.5 pts** (day 0) decaying as below. Entry requires signal > +8 **and** HAR < 18%.

| Day | signal (pts) | HAR ann% | Event |
|---|---|---|---|
| 0 | +10.50 | 13.2 | above bound, but entry rule waits for day ≥ 3 stability |
...
| Professional: listed spread execution via FCM algos | spread-order execution, margin netting | bundled in FCM relationship | one-fill spread orders, real margin offsets | you still own signal, sizing, and the vol gate |
...
| Grok Q-TB4-3 §4 (unverified lead) | futures basis timing | "Sharpe ~0.6–0.9 in futures" for basis strategies | before full retail costs | chatbot claim — unverified; calendar spreads are a narrower sleeve than "basis strategies" |
...
1. Hull, J. C. *Options, Futures, and Other Derivatives* (term structure, cost of carry with convenience yield, calendar-spread mechanics — the textbook [D] source for S058's fair-spread formula).
2. Corsi, F. (2009). "A Simple Approximate Long-Memory Model of Realized Volatility." *Journal of Financial Econometrics* 7(2): 174–196 — https://academic.oup.com/jfec (HAR-RV: the verified forecaster behind the S066 timing gate).
...
- Grok Q-TB4-1 (C): futures cost bands (exchange + NFA + 0.25–1 tick slippage) and the 8–20 bp richness buffer — adopted as `example` parameters.

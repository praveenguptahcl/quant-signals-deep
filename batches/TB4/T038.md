...
**Universe & session.** Sleeve A (momentum): 11 US sector ETFs (XLK, XLF, XLE, XLV, XLI, XLP, XLY, XLB, XLRE, XLC, XLU-class), RTH only, minimum 20-day median spread ≤ 4 bps (*example* filter). Sleeve B (fade): ~300 S&P-500-class names, price ≥ $10, 20-day ADV ≥ $10M, borrowable, spread ≤ 10 bps (*example*); exclude earnings-day names and stocks halted or in a corporate action. Session window for both: 09:30–16:00 ET; everything flat by 15:55 ET — no overnight carry, no after-hours gap risk.

**Entry rule.** *Sleeve A — sector momentum (S062, intraday adaptation).* At ranking time t_r = 12:00 ET (*example*), compute each sector ETF's return from the open to t_r; rank; **long the top 3 in equal dollar weight** (*example*), from the first bar after t_r to 15:55 ET. The documented literature form (S062) is monthly: rank 20 industries by 6-month return, long top 3 / short bottom 3; the intraday adaptation is the paper-trade version a ≤$1M book can actually hold — long-only on the ETF legs, no short-borrow needed.
*Causal timing:* the ranking uses returns through t_r only; the earliest fill is the bar after t_r; the literature's overnight-aware variant (rank on prior close-to-open + first hour) is the fallback if the midday ranking ever proves too crowded.

*Sleeve B — idiosyncratic fade (S039 + S080 confirmation).* For each stock i at bar t compute the market-plus-sector residual: r(i,t) = α(i) + β_m(i)·r_mkt(t) + β_s(i)·r_sec(t) + u(i,t), cumulate U(i,t,L) = Σ û over L = 30 min (*example*), z(i,t) = U/(σ̂·√L) (S039, sector-augmented variant 2). Independently compute the PCA eigenportfolio residual and its s-score s(i,t) (S080). **Enter only when both agree**: |z(i,t)| ≥ 1.5 (*example*) AND sign(s(i,t)) = sign(U(i,t)) with |s| ≥ 1.25 (*example*) — long the negative-residual names, short the positive ones. The two-model confirmation exists because factor-model miss is the documented killer of residual books (too few PCs leaves factor in the "residual"; S039's sector term and S080's statistical factors catch different misses).
*Causal timing:* β̂ estimated on data ≤ t−1 (never the formation window — that would regress the shock on itself); signals computed at bar t using bars ≤ t; earliest fill t+1. Corporate-action-adjusted total-return bars only.
...
**Risk limits** (*example* unless noted). Daily loss stop −1.5% of NAV; per-sleeve stop −1.0%; kill switch: halt new fades if 3 consecutive fades stop out (the residual model is probably broken that day), halt momentum sleeve if SPY's morning ranking universe shows intraday reversal regime (a documented momentum-failure regime — see T7); feed outage ⇒ safe mode (T5).

**Cost model.** 5 bp one-way on notional per equity leg turn (*example*, the TB4 shared convention from Q-TB4-1; labeled lead), 5 bp one-way on ETF legs; half-spread embedded in fill prices; no financing (intraday, flat by close), no borrow on longs; short-residual names require a borrow flag — skip any name not flagged borrowable rather than paying a special. Applied explicitly in T4.

**Order/execution sketch.** Sleeve A at 12:00: marketable limits on 3 ETF legs, participation ≤ 10% of visible depth (*example*), all-or-nothing intent (an unbalanced sector book is not the strategy). Sleeve B: limit orders at the touch, one name at a time, participation ≤ 10%; if a leg fails to fill within 2 min (*example*), cancel — never chase a residual that already converged. Queue-position honesty: on SIP/L1 fills this is `simulated only — requires MBO/ITCH`; the cost model's 5 bp is doing the heavy lifting for realism, not the queue model. Everything backtested with signal at t → fill at t+1, one-day purged/embargoed validation (S088) on parameter choices.
...
**Trade T1 — sector-momentum sleeve (winner).** At 12:00 ET the morning ranking: XLK +1.4%, XLE +1.1%, XLU +0.9% (top 3, *example*); book shows the XLK leg: long **800 XLK @ 100.00**, exit 15:55 @ **100.28**.
...
**Trade T2 — idiosyncratic fade (winner).** AAA residual z = −1.6 (≥ 1.5 ✓), s-score −1.5 (sign agrees ✓): long **500 AAA @ 50.00** → exit @ **50.18** as z decays to −0.2 (+90.00). BBB residual z = +1.9: short **300 BBB @ 80.00** → cover @ **79.90** (+30.00). Gross +120.00; costs (25,000+25,090+24,000+23,970)×0.0005 = −49.03; **net +70.97**. Cumulative: +214.86.

**Trade T3 — idiosyncratic fade (stop).** CCC z = −1.7, s = −1.5 (sign agrees ✓): long **400 CCC @ 60.00**; DDD z = +1.8: short **250 DDD @ 90.00**. The residuals *widen* (|z| hits 2.8): stop fires at |z| ≥ 2.5 — sell CCC @ **59.88** (−48.00), cover DDD @ **90.14** (−35.00). Gross −83.00; costs (24,000+23,952+22,500+22,535)×0.0005 = −46.49; **net −129.49**. Cumulative session: +143.89 + 70.97 − 129.49 = **+85.36**.

**Where the example is optimistic.** The z-scores are hand-anchored to converge exactly when the example needs them to; a real S039/S080 estimator has *estimation error on every bar* — β̂ wobbles, σ̂ is noisy, and the two models agree less often than the toy implies, so real trade frequency is far lower. Fills are at the touch with no partial fills and no borrow cost on the short legs (the example requires the borrow flag but prices the short at 0 fee — a live short book pays GC 25–50 bp annual minimum). The 5 bp cost is below what illiquid fade names actually cost (effective spreads of 10–30 bps are common in the exact names where residuals stretch furthest). The sector-momentum leg is long-only XLK on a day XLK kept drifting — half the battle is the day selection, which the example is handed for free. Finally, the session nets +$85.36 on ~$180k notional — treat this as *arithmetic illustration of the cost drag*, not a backtest: no real performance claim is made.

### T5. Data & infra — what must run

**Pipeline inventory.** Nightly (post-close): refresh 252-day return panel, refit PCA eigenvectors (S080), re-estimate market/sector betas per name, rebuild the 30-min residual-vol series and the borrow/earnings exclusion list. Pre-open: verify today's corporate-action adjustments, confirm sector-ETF constituents. Intraday (12:00–15:55 ET): 1-min bar stream for 11 sector ETFs + ~300 names; ranking job at 12:00; residual/z/s-score recompute on each 1-min bar close (numpy vectorized — milliseconds); heat and daily-loss monitors continuous. Post-close: persist bars + every signal snapshot + decisions to parquet; purged/embargoed re-fit of nothing (parameters frozen — refit monthly at most).

**M5 Max feasibility: feasible (Tier M).** Per cost-model §2, 311 names × 1-min bars is trivial: 311 × 390 = ~121k bars/day; PCA refit on 300×252 nightly is seconds (cost-model §2/§3: the 2000-name pair screen runs minutes; 300 names is a rounding error). RAM: hot working set < 2 GB (§3); 60-day 1-min archive ~200 MB (§4: 1-min bars 500 symbols ≈ 50 MB/day — this book is lighter). Engineering: 40–100 h Tier M+ harness (§5) ≈ $6,000–15,000 loaded-cost estimate — the work is the two-sleeve execution FSM and the causality-proof backtest harness, not the math. Bottleneck: corporate-action/borrow data hygiene, not compute. Breaks at: 3,000-name universes (pairwise PCA gets heavy — but this strategy needs only 300) or tick-level residual recomputation (not required).

**Feed-outage safe mode.** Bar feed gap > 2 min (*example*): no new sleeve-B entries (z-scores computed on stale bars are poison); existing fades run to their time stop or flatten. Sector-ETF quote hole: skip the momentum sleeve entirely that day. Borrow feed stale: hard veto on new shorts. All states rebuildable from the on-disk ledger; restart = flat + re-derive, never resume blind.
...
| Tier-1: Polygon Stocks Advanced / Alpaca SIP | Real-time SIP 1-min bars + corporate actions | ~$30–200/mo, *indicative — verify before budgeting* | The honest minimal stack for both sleeves | No borrow data; no PCA-ready long history bundled |
...
**Verdict: buy the Tier-1 bar feed, build the logic.** The strategy's data needs are deliberately boring (1-min bars + splits/divs + a borrow flag); nothing here justifies Tier-2 spend. **Build if** you want the two-model confirmation and your own heat accounting — no vendor sells "S039-vs-S080 agreement" as a product. **Buy more data if** you scale the fade sleeve past ~300 names (then borrow history and point-in-time sector maps become the binding constraint, and the borrow feed becomes the expensive line per Q-TB4-2). Crossover: the day the fade sleeve's short book starts paying specials regularly, rent institutional securities-finance data or shrink the short book to GC names — the example's 0-fee shorts are the first fiction to die in production.
...
**Honest bottom line.** For a ≤$1M paper operation: sleeve A is a clean, cheap, learnable trend sleeve — paper-trade it to learn ranking hygiene and session timing. Sleeve B is the real education and the real risk: the documented edge is a *residual* edge measured in single-digit bps per trade, and the two-model confirmation that makes it safer also makes it trade rarely. An institutional desk runs this at 10–100× the name count with a borrow desk and a risk model (Barra-class, $150k+/yr); the paper book's honest role is to learn *which half of the edge is estimation error* before sizing it. Do not annualize the +$85.36 synthetic session.

### T8. Failure modes & pitfalls

1. **Momentum/reversal sign collision** — the sector-momentum sleeve is long winners while the fade sleeve shorts positive residuals *inside* those sectors. Mitigation: sleeve B is sector-neutralized (S039 variant 2 + S080), so the fade book cannot fight the overlay on the same names; monitor sleeve-level P&L separately and kill a sleeve, never the combination.
2. **Factor-model miss** — too few PCs leaves market factor inside the "residual" (you're fading beta, not noise); too many PCs fits noise as factors (Grok's lead: 75%-variance cutoffs lose money). Mitigation: the two-model agreement gate; monitor the residual's correlation to SPY — above 0.3 (*example*) ⇒ refit.
3. **Intraday momentum reversal regimes** — morning winners collapse (macro shock, rotation day). Mitigation: regime veto (skip sleeve A if the prior day's ranking anti-persisted); hard 15:55 flatten regardless.
4. **Borrow specials on short residuals** — the cheapest-to-fade names are the hardest to borrow. Mitigation: borrow-flag gate pre-entry; model GC 25–50 bp carry in the cost bound; never assume the example's 0-fee shorts.
5. **β̂ staleness through regime breaks** — residuals computed against a 120-day beta during a factor rotation are garbage. Mitigation: 5-day beta refresh (*example*, S039 default); halt fades if cross-sectional residual dispersion explodes (the model is misspecified that day).
6. **2007-style crowded de-lever** — every residual book dumps the same longs. Mitigation: hard heat cap (Σ|z|·$risk ≤ 4% NAV), daily loss stop −1.5%, and accept that the stop *is* the strategy in this regime — no averaging down.
7. **Lookahead in the ranking/residual window** — ranking on bars that include t_r+1, or β̂ fit on the formation window. Mitigation: causality unit tests in the harness (signal-at-t ⇒ fill-at-t+1 asserted on every backtest bar), purged/embargoed validation (S088).
8. **Cost-model optimism on fade names** — the widest residuals live in the widest-spread names. Mitigation: 5 bp is the *floor*; gate entries on trailing effective spread (S014-style) and require the edge to clear 2× the spread (*example* safety margin).

...
*Batch TB4 · Strategy 32/100 · Signals S054, S088, S051*
...
**Universe & session.** Same as T031: liquid US common stocks, price ≥ $5, dollar volume ≥ $5mm, shortable, same-industry preference (`example`). Daily bars, RTH closes. Exclusions: earnings/corporate-action windows ±2 days, halts, announced M&A — plus a dependence screen: drop pairs with $|\hat{\rho}| < 0.5$ (`example`), where the conditional trigger is noise.
...
**Position sizing.** Per-trade risk 0.25–0.75% of NAV (`example`; smaller than T031 — thinner edge, higher turnover). Portfolio heat: open $|M_t|$-weighted dollar risk ≤ 4% NAV (`example`); hard cap 15 pairs (`example`).
...
**Cost model.** Same 5 bp one-way per leg round-trip assumption (`example`) plus borrow on the short leg — but the binding constraint is *turnover*: the trigger fires more often than ±2σ z-scores, so each trade must clear ~20–40 bp round trip (`example` hurdle) to survive. Costs are charged explicitly in the example.
...
| Borrow / HTB file | Daily 16:30–18:30 ET | Veto/resize shorts |
...
|---|---|---|---|
| Tier-0: Stooq / Alpaca IEX daily | Free daily bars | ~$0 | Enough to prototype marginals and PITs |
...
| Open-source copula libraries | Estimators, PIT tooling | ~$0 | Useful scaffolding — but the family-selection and purged-CV logic is yours to write and validate |
...
| Liew & Wu (2013), via 2025 review | US equities | Copula signals "more consistent" and more profitable than distance/cointegration | Reported as after-cost in the review summary | Secondary summary — the primary paper's cost accounting was not independently verified here |
| Rad et al., regime notes | US, post-2009 | Copula trade frequency steadier than distance/coint; hurts less on non-convergent trades and in downturns | n/a (shape, not level) | Grok-reported (Q-TB4-3) — unverified lead; directionally consistent with the tail-dependence story |
...
5. Bailey, D., Borwein, J., López de Prado, M. & Zhu, Q. J. (2017). "Pseudo-mathematics and financial charlatanism: The effects of backtest overfitting on out-of-sample performance." *Notices of the AMS*, 61(5). (Probability of backtest overfitting — why the S088 gate exists.)

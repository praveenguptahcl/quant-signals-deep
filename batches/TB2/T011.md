...
2. **S047 timing (bounce):** wait for the liquidity shock — `example` trigger: mid drops (long) ≥ 8 bps within ≤ 2 s, last print seller-initiated at the bid, bid size after the print ≥ 1.5× pre-print (replenishment). The Roll (1984) process in S047, $\Delta p_t = \frac{s}{2}(Q_t - Q_{t-1}) + u_t$, says a sell-initiated print mechanically depresses the trade price by half the spread; the entry waits for that mechanical print rather than chasing the move.
...
| S047 bid–ask bounce | Entry timer | Fire only on a seller-initiated bid print with mid impulse ≥ 8 bps/2 s and bid-size replenishment ≥ 1.5×. `example` thresholds |
...
for each name in liquid_universe:                      # ADV>=50M, spread<=5bps, price>=10 (example)
    ...
    on L1 quote/trade event e:                         # S047 trigger
        if impulse(e, >=8bps in <=2s) and print_on_bid(e) and bid_replenished(e, >=1.5x):
...
Cost schedule (`example`): commission $0.005/share + $0.50/ticket per side; spread cost embedded in fill prices (passive entries pay $0; aggressive exits pay the half-spread).

| Trade | Setup (synthetic) | Entry | Shares | Exit | Gross | Costs (comm.) | Net | Exit reason |
|---|---|---|---|---|---|---|---|---|
...
| T2 | mid impulse −5 bps, replenished | Buy 1,000 @ **99.84** (bid) | 1,000 | Sell @ **99.87** (bid, aggressive) | +$30.00 | −$11.00 | **+$19.00** | +5 min time stop |
| T3 | mid impulse −7 bps, no replenishment follow-through | Buy 1,000 @ **99.79** (bid) | 1,000 | Sell @ **99.74** (bid, aggressive) | −$50.00 | −$11.00 | **−$61.00** | stop: new extreme −1 tick |
| T4 | mid impulse −5 bps | Buy 500 @ **100.11** (bid) | 500 | Sell @ **100.11** (bid) | $0.00 | −$6.00 | **−$6.00** | time stop, scratch |
| T5 | mid impulse −8 bps, strong replenishment | Buy 1,000 @ **99.66** (bid) | 1,000 | Sell @ **99.73** (ask) | +$70.00 | −$11.00 | **+$59.00** | 80% mid recovery |
| **Total** | | | | | **+$130.00** | **−$50.00** | **+$80.00** | |

Line-by-line (T1): buy 1,000 × 99.91 = −$99,910, commission −$5.50; sell 1,000 × 99.99 = +$99,990, commission −$5.50 → gross +$80.00, net **+$69.00**. (T3): −$99,790 − $5.50 in; +$99,740 − $5.50 out → gross −$50.00, net **−$61.00**. Cumulative net: +$69.00, +$88.00, +$27.00, +$21.00, **+$80.00** — the chart's staircase.
...
|---|---|---|---|---|
| Tier-0: Alpaca paper trading + IEX/SIP-delayed data | Paper execution API, delayed bars | ~$0 | Free live-fire testing of the order-state machine | Delayed data; SIP-only; no honest queue modeling |
| Tier-1: Polygon Stocks Advanced | Real-time SIP L1 + 1-min bars, corporate actions | ~$30–200/mo | Cheap honest mids and trigger events | No MBO; history depth costs extra |
| Tier-2: Databento Standard (L1 MBP-1) | Exchange-grade L1, clean timestamps | ~$200/mo + usage | The minimum feed this strategy's claims require | Usage billing on heavy months |
...
| Execution: broker API (e.g. IBKR/Alpaca) | Order routing, no matching-engine build | ~$0 + commissions | Don't write a matching engine | Smart-routing opacity; queue honesty still yours |
...
6. **Overfit trigger thresholds.** 8 bps / 2 s / 1.5× replenishment are `example` values with no optimality claim. *Mitigation:* sensitivity sweep; prefer thresholds flat across ±50% perturbations; re-estimate quarterly.
...
- Kaul, G. and Nimalendran, M. (1990), "Price Reversals: Bid-Ask Errors or Market Overreaction?", *Journal of Financial Economics* — transaction-price reversals are mostly bounce; bid-to-bid autocorrelation disappears.
...
- Roll, R. (1984), "A Simple Implicit Measure of the Effective Bid-Ask Spread in an Efficient Market," *Journal of Finance* — the bounce data-generating process behind S047.

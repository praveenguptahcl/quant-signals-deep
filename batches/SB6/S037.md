...
Let $p(i) = \log P(i)$ and $r(i) = p(i) - p(i-1)$. Local **bipower variation** over a window of $k$ returns ending at $i$:
$$\hat{V}(i) = \frac{\pi}{2} \sum_{j=i-k+2}^{i} |r(j-1)| \cdot |r(j)|$$
...
4. Bipower variation: $\hat{V} = (\pi/2)\Sigma = \mathbf{0.00013816}$; $\sqrt{\hat{V}} = \mathbf{0.011754}$.
5. $Z(\text{open}) = 0.019803/0.011754 = \mathbf{1.685}$.
6. With $\Delta = 1/390$: $C(\Delta) = \mathbf{3.031}$, $S(\Delta) = \mathbf{0.2894}$; $T(\text{open}) = (1.685 - 3.031)/0.2894 = \mathbf{-4.65}$.
7. Decision: $-4.65 < q(0.95) = \mathbf{2.970}$ → **NOT a Lee–Mykland jump at 5%** → the fade is permitted. (Conclusion unchanged by the typo correction.)
8. Fade P&L (from S036): short at $102.00$, cover at bar 10 at $99.2148$ → $\Pi(10) = \mathbf{+2.731\%}$ before costs.

**What to notice:** the overnight return looked dramatic (+2.00%), but scaled by local volatility and judged against the extreme-value distribution it is utterly ordinary — $T = -4.65$ is *far* below the threshold. That is the filter's value proposition in one number: it stops you from treating every big gap as information. Limits: the 10-bar window is illustrative, not statistically reliable; no costs; one passing test on one tape proves the arithmetic, not the strategy.
...
| Trades/quotes (optional, finer) | event | tick/L1 | Tier 2 | For sub-minute variants; microstructure noise caveat |
...
| Tier 1: Polygon Stocks Advanced | Full-session 1-min bars, corp actions | ~$30–200/mo | Covers S6 at 1-min | SIP timestamps; fine here |
| Tier 2: Databento (TAQ-like) | Exchange-native intraday, imbalance feeds | ~$200/mo + usage | Honest auction/overnight prints | Cost if you scan full history |
| Academic: LOBSTER / WRDS TAQ | MBO/ITCH replay, publication history | hundreds/yr academic | Research-grade | Not for live trading |
...
| Stübinger & Schneider (2019), J. Risk & Fin. Mgmt | S&P 500, 1998–2015, high-frequency | Jump-test-based overnight-gap strategy: 51.47% p.a., Sharpe 2.38 **after** costs (5 bp/half-turn; 20 bp round trip) | After cost (authors' model) | Single-team backtest; no independent replication; an upper bound, not an expectation |
...
![S037 worked example — Lee–Mykland opening-jump test with corrected arithmetic, T(open) = −4.65 vs q(0.95) = 2.970, fade permitted](images/S037_example.png)

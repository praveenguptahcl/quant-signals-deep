...
2. **Lookahead leakage** — using the bar's close before the bar closes, or contemporaneous spread. *Mitigation:* signal at t uses data ≤ t; earliest fill t+1; exchange-time timestamps; tick logic on SIP bars is labeled `simulated only — requires MBO/ITCH`.

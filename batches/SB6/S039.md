...
| **Build-or-buy in one line** | Build: one regression per stock per bar on 1-minute data; buy the sector-ETF bars, build the residualizer. |
...
Causal timing: β̂ estimated on data ≤ t−1 (never the formation window — that would regress the shock on itself); U(i,t,L) uses bars ≤ t; earliest fill t+1. Variants: (1) market-only residual (SPY beta); (2) sector+market residual (adds sector ETF — kills industry momentum contamination, cf. Da et al. 2014); (3) PCA/statistical-factor residual (S080-style eigenportfolios — no ETF needed, but factors can rotate).
...
Fade-B trade walk: long B at 99.500 (bucket-6 close, midpoint-marked — see contamination note), exit three buckets later at 99.800 as it reverts (+0.30% raw, +0.30% residual, market flat). Gross **+30.2bp**; costs: effective half-spread 3bp × 2 legs + 1bp fees = 7bp → **net +23.2bp** on this toy trade.
...
- **T063 — Stretched-Move Z-Score Fade**: S039 as a veto/quality filter — skip stretched moves with no idiosyncratic content.

### S6. Data required — exact spec

| Field | Type | Granularity | Source tier | Notes |
|---|---|---|---|---|
| Stock 1-min OHLCV | float/int | 1-min bars | Tier 1 | formation + hold measurement |
...
| Session/halt calendar | timestamps | daily | Tier 1 | exclude halts, half-days, DST |

Collection: Polygon stocks v3 aggregates (stocks + ETFs, one subscription) or Alpaca SIP. Ingest sketch (≤20 lines, polars):

...
| **When it works** | Liquid pairs with genuine economic linkage (ETF–futures, ADR–ordinary), exchange timestamps, leads that survive ±50–100 ms timestamp-shift stress tests. |
...
Storage: per `notes/cost-model.md` §4, tick trades ≈ L1 quotes+trades at ~2–8 GB/day per liquid symbol in parquet — per-symbol daily files. Data-quality checklist: exchange (not SIP/vendor-receipt) timestamps; clock-sync across venues; no interpolated ticks; splits/dividends adjusted; halts excluded; FX timestamps aligned for the ADR variant; the ±50–100 ms shift stress test run before any monetization claim.
...
flowchart LR
    ...
    ING -->|sorted observation times| FEAT["Feature compute\n(two-pointer overlap sweep, lag grid)"]
    FEAT -->|ρ̂(ℓ) curve| SIG["Signal S060\nHY lead-lag ℓ̂ + cost gate"]
    SIG -->|laggard direction| GATE{"Cost / ts-stress\ngate?"}

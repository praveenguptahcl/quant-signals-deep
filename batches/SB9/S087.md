...
- **Mental model 3:** This is plumbing, not alpha. It never generates a trading signal by itself; it makes the *inputs* to your models well-behaved. Compare every model against plain returns and log prices.
...
6. **Refitting $d^*$ on overlapping windows** (churn and implicit lookahead) → mitigate: refit on a fixed cadence (e.g. monthly) on non-overlapping training blocks.
7. **ADF test fragility** (size/power issues, heteroskedasticity, breaks) → mitigate: complement with a variance-ratio or KPSS check; never let one test statistic be the only gate.

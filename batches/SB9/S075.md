...
Day 1 check: $\sigma_I^2 = 27^2 = 729$; $\sum w^2\sigma_i^2 = (33^2+35^2+37^2)/9 = 412.67$; $\sum_{i\ne j} w_iw_j\sigma_i\sigma_j = 2(33{\cdot}35+33{\cdot}37+35{\cdot}37)/9 = 815.33$; so $\rho_{\text{impl}} = (729-412.67)/815.33 = \mathbf{0.3920}$.
...
4. **Margin expansion** — short-option margin and stress collateral spike exactly when P&L is worst. *Mitigation:* pre-fund stress collateral; size to stressed margin, not current.
5. **Discrete-rebalance error** — variance-swap math assumes continuous hedging; daily hedging leaks P&L. *Mitigation:* model discrete hedging explicitly; widen the required edge by the slippage estimate.
6. **Dividend/borrow distortion** — special dividends and hard-to-borrow names corrupt IV. *Mitigation:* borrow-rate feed; exclude or haircut affected names.
7. **Surface model risk** — implied correlation depends on your IV interpolation. *Mitigation:* compute with two surface methods; trade only when they agree.
8. **Crowding** — every vol desk runs dispersion; unwinds correlate. *Mitigation:* monitor dealer positioning (S074); reduce into crowded regimes.

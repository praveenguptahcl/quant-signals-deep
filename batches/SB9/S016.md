...
**Price discovery** is the incorporation of new information into a security's price. The **efficient price** is the unobservable "true" value common to all trading venues — each venue's quoted price is the efficient price plus venue-specific noise. A **venue** is a trading location: NYSE, Nasdaq, BATS, a dark pool, a foreign listing. When the same stock trades in many places, **cointegration** keeps the venue prices from drifting apart (they share one long-run anchor), but *which* venue moves first when news arrives is an empirical question.
...
Picture three venues quoting the same stock at 10:00:00. Venue A's mid ticks up 2¢, B and C follow 300 ms later. The VECM sees A's innovation *leading* the common move and credits A with most of the permanent innovation — A is where price discovery happened. But note what IS does *not* say: it doesn't say the price goes up next (the innovation already happened), it doesn't say A's lead survives your latency and fees, and it doesn't distinguish "A had the informed traders" from "A just reacts fastest to public news" or "B and C mechanically follow A." A high IS can mean faster reaction to public information, more informed flow, better liquidity, lower latency — or merely that the other venues copy it. **The information-share statistic alone is descriptive, not predictive.**
...
- **Mental model 3:** This is a *routing and monitoring* tool that becomes a trading input only inside a conditional lead–lag model tested out-of-sample after latency and costs (T082, T089).
...
The moving-average long-run impact matrix is $C(1) = \beta_\perp (\alpha_\perp' \Gamma \beta_\perp)^{-1} \alpha_\perp'$ (with $\Gamma = I - \sum_i \Gamma_i$); its rows are identical up to scale, giving the common **long-run impact row vector** $\psi$ ($1 \times n$). With the Cholesky factor $F$ ($\Omega = FF'$, lower triangular, for a given venue ordering), the **information share** of venue $j$ is:
...
| Sampling | $\Delta t$ | ≤1 s bars (example) | Too fine: asynchrony/staleness distorts $\Omega$; too coarse: the lead is already arbitraged | 1-s synchronized mids *— example* |
| Estimation window | $T$ | minutes–days | Too short: $\psi$, $\Omega$ noisy; too long: structural breaks (venue leadership changes) | 1 session *— example* |
| VECM lags | $k$ | 1–5 | Too few: residual autocorrelation biases $\Omega$; too many: overfit | 2 *— example* |

**Normalization:** log mid-prices, synchronized to a common clock (venue timestamps, millisecond-aligned). **Causal timing:** IS is estimated on a *history* window and used as a slow-moving venue attribute (routing/monitoring) — not a per-bar trigger; any lead–lag trade built on it executes no earlier than $t+1$ after the leading venue's move. **Variants:** (1) *trade-vs-quote decomposition* (Pascual et al. 2006) splitting each venue's IS into trade-related and trade-unrelated shocks; (2) *Hasbrouck lower/upper bounds* (report min/max over orderings — the mandated default); (3) *time-varying IS* on rolling windows to catch leadership changes.
...
**Step 1 — estimate.** The VECM yields long-run impact row $\psi = [0.2963,\ 0.2768,\ 0.2825]$ (nearly equal — the common efficient price loads on all venues, as theory says) and innovation covariance $\Omega = [[0.000613,\ 0.000172,\ 0.000121],[0.000172,\ 0.000524,\ 0.000101],[0.000121,\ 0.000101,\ 0.000499]]$ (strong off-diagonals — the venues share the efficient-price shock contemporaneously), with $\psi\Omega\psi^\top = 0.00019818$.

**Step 2 — one ordering, by hand.** For ordering (A, B, C), the Cholesky factor is $F = [[0.024763,\ 0,\ 0],[0.006953,\ 0.021815,\ 0],[0.004889,\ 0.003083,\ 0.021571]]$. Then $[\psi F]_A = 0.2963(0.024763) + 0.2768(0.006953) + 0.2825(0.004889) = 0.0106431$, and $IS_A = 0.0106431^2 / 0.00019818 = 0.5716 \approx \mathbf{0.572}$; the same arithmetic gives $IS_B = 0.241$, $IS_C = 0.187$ for this ordering.
...
**What to notice:** A was *designed* as the leader and averages highest — yet its bounds (24–57%) overlap B's and C's completely. A point estimate like "A = 39%" is spurious precision; the bounds are the result. **Limits:** synthetic cointegration is clean; real venues have halts and tick-size differences, and this example has no latency.
...
import polars as pl
frames = []
for venue in ["A", "B", "C"]:                       # one panel per venue
    q = pl.read_parquet(f"quotes_{venue}.parquet")  # venue-ts, bid, ask
...
panel = frames[0]
for f in frames[1:]:
    panel = panel.join(f, on="ts", how="inner")      # common clock; inner = all venues live
panel = panel.with_columns(pl.all().exclude("ts").log().name.suffix("_log"))
# VECM on the log-mid panel; drop halted/illiquid bars before estimation
```
...
**Verdict: build the pipeline, buy venue-identified data.** The econometrics is a day's work and must be yours (your orderings, your windows, your staleness handling); SIP data cannot answer "which venue" by construction. Crossover: if you only need a *single* venue's lead–lag against the SIP, Tier-1 plus a lead–lag regression (S059/S060) is cheaper than full multi-venue IS.
...
Regimes where it fails: leadership changes (the "dominant" venue of 1995 is not the dominant venue of 2026); high correlation regimes where bounds go uninformative (as in §S4); sampling asynchrony that manufactures spurious leads; news regimes where all venues react to the same public signal (the lead isn't exploitable). **Honest bottom line:** as a standalone directional signal this has no documented edge — the evidence base is *attribution*, and every number above is before-cost and descriptive. As a venue-routing and monitoring input (T082) and a quote-staleness detector (T089), it earns its keep: knowing *where* discovery happens tells you which feed to buy, which quotes to trust, and which venue's moves deserve a response.

### S10. Failure modes & pitfalls

1. **Cholesky-ordering shopping** (reporting the ordering most favorable to your venue) → mitigate: always report min/max bounds over all $n!$ orderings; the midpoint is illustrative only.
2. **Sampling asynchrony** (stale prints on one venue look like a "lag") → mitigate: venue timestamps, common-clock bar alignment, drop bars where any venue is stale.
...
4. **Structural breaks in leadership** (IS estimated over a month when leadership flipped mid-month) → mitigate: rolling windows; change-point monitoring on the bounds.
5. **Endogeneity** (all venues react to the same news — the "lead" is reaction speed, not information) → mitigate: news-time controls; don't trade the lead without a conditional model tested out-of-sample.
6. **Latency erasing the lead** (measured lead 300 ms; your round trip 800 ms) → mitigate: compare the lead horizon against measured execution latency + fees before any trading claim.
7. **Over-interpreting a high IS** ("venue A discovers prices, so fade venue B") → mitigate: remember the four readings of high IS — the statistic doesn't choose among them.
...
3. Pascual, Roberto, Pascual-Fuster, Bartolomé & Climent, Francisco (2006). "Cross-listing, price discovery and the informativeness of the trading process." *Journal of Financial Markets* 9: 144–161. (Bibliographic record from this batch's verified research notes; stable URL not captured this batch — do not substitute an unverified working-paper link.)
4. Gonzalo, Jesús & Granger, Clive W. J. (1995). "Estimation of common long-memory components in cointegrated systems." *Journal of Business & Economic Statistics* 13: 27–35. (Component-share complement, cited via the Su & Chong reference.)
5. Federal Reserve Bank of New York. Staff Report on FX market price discovery and the shifting locus of trading (this batch's research notes captured the working-paper PDF; the Liberty Street Economics summary is a lead, URL not verified this batch). https://www.newyorkfed.org/medialibrary/media/research/staff_reports/sr624.pdf
...
- NY Fed FX price-discovery staff study findings as summarized by the bot (relative contributions change across periods; higher-IS market not determined by size) — URL not captured; treat as a lead.

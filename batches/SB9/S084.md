...
Picture 9:47:03 on a liquid name: bid $231.40 × 800 / ask $231.41 × 300. A 500-share market buy lifts the ask. The mid moves from $231.405 to, say, $231.41 — a +$0.005 quote revision. Was that half-cent *information* (someone knows something) or *mechanics* (the book was thin on the ask side)? Hasbrouck's idea: don't look at one trade — model the whole joint dance of trades and quote revisions as a VAR, then ask the model: "if a completely unexpected buy arrives right now, how much does the quote *permanently* move over the next minutes?" The answer is the **impulse response function (IRF)** — the cumulative quote revision after a trade shock — and its limit is the **long-run impact (LRI)**, the trade's permanent price impact. That permanent component is the trade's *information content*: transitory wiggles wash out, but information stays in the price.

Why should this work economically? Two forces. First, **adverse selection**: when an informed trader buys, the market maker (or the book) learns something bad may be coming and reprices upward — the trade *causes* a permanent revision. Second, **inventory and liquidity mechanics**: even uninformed flow moves quotes temporarily as liquidity providers manage inventory, and those temporary moves decay. The VAR separates the two: permanent = information, transitory = plumbing. The key discipline is that only the *unexpected* trade moves the needle — expected, autocorrelated order flow (the same algorithm slicing an order) is already in the price. As this batch's verified research notes put it: "The informative component = UNEXPECTED trade innovation, not total volume."
...
A trade of size $q$ (shares) has permanent impact $\approx \mathrm{LRI} \times q$. Equivalently, the trade equation can be written in single-equation form $q_t = \alpha_q + \sum_\ell (\phi_\ell \Delta m_{t-\ell} + \psi_\ell q_{t-\ell}) + u_{q,t}$; the quote equation's response to $u_{q,t}$ is what carries the information.
...
First, the raw input shape — 12 events of the synthetic tape (bid/ask in dollars, $q$ = trade sign):
...
| 6 | 99.9904 | 100.0004 | 99.9954 | −0.0436 | −1 |
| 7 | 99.9638 | 99.9738 | 99.9688 | −0.0266 | −1 |
| 8 | 99.9371 | 99.9471 | 99.9421 | −0.0267 | −1 |
| 9 | 99.9287 | 99.9387 | 99.9337 | −0.0084 | +1 |
| 10 | 99.9430 | 99.9530 | 99.9480 | +0.0143 | +1 |
| 11 | 99.9363 | 99.9463 | 99.9413 | −0.0068 | +1 |

Estimating the VAR(1) by OLS on all 5,000 events gives $\hat A_1 = [[0.1046,\ 0.0120],[0.1479,\ 0.5958]]$ (close to the true DGP, as it should be). Hand-check the IRF arithmetic from $\hat A_1$: with $\Psi_0 = I$, the first-period response is $\mathrm{IRF}(1) = e_m^\top(I + \hat A_1)e_q = \hat A_1[0,1] = 0.0120$ dollars = **1.20 cents** — a one-step quote revision of 1.20¢ per unexpected buy. Cumulating: IRF(2) = 2.04¢, IRF(3) = 2.55¢, IRF(5) = 3.05¢, IRF(10) = 3.31¢, converging to the closed-form LRI $= e_m^\top(I-\hat A_1)^{-1}e_q =$ **3.33 cents** — the permanent impact of one unexpected buy innovation. The chart shows this convergence: a steep first few events, then flattening at the 3.33¢ asymptote by about event 20.

**What to notice:** the IRF starts at 0 (no contemporaneous effect by construction) and *builds* — Hasbrouck's finding that "a trade's full price impact arrives only with a protracted lag" is visible right here. **Limits:** this toy tape has no fees, no spread cost, and the DGP is a clean VAR(1) — real order flow has long memory that a 1-lag VAR cannot capture, and the LRI is a measurement of information content, not a backtested trading profit.
...
**Verdict: build the estimator, buy the tape.** The VAR/IRF math is 50 lines and needs your own lags, horizons, and size transforms (you can't buy your own thresholds), but synthesizing exchange-timestamped trades+quotes yourself is re-licensing TAQ — buy Databento or TAQ access. Crossover: if SIP 1-s bars suffice for your horizon (minutes, not seconds), the Tier-1 route wins on cost.
...
Regimes where it fails: volatility spikes (parameters unstable — refit or stand down); news jumps (quote revisions are jump-driven, not trade-driven); thin names (signing error dominates); crowded periods where everyone trades the same innovation proxy. **Honest bottom line:** as a standalone trigger this is a *measurement* edge, not a documented net-of-cost alpha — the literature documents the *existence and shape* of trade informativeness, not a tradable Sharpe. As a filter/validator (T052) and cost-model input it is genuinely useful: it tells you which flow to fade and which to respect.

### S10. Failure modes & pitfalls

1. **Trade-signing error** (Lee–Ready/quote-rule mislabels mid-spread prints) → mitigate: use exchange timestamps, tick-test fallback, report signing-agreement rates.
2. **Lookahead via quote synchronization** (using the quote *after* the trade to sign it, then predicting that same quote) → mitigate: strict backward asof joins only; trade at $t$ sees quotes $\le t$.
3. **Lag misspecification** (too few lags on long-memory flow) → mitigate: AIC/BIC lag selection per symbol, refit on rolling windows.
4. **Regime breaks** (FOMC, earnings, vol spikes change $A_\ell$ intraday) → mitigate: regime-gated refits; widen or suspend thresholds in jumps.
...
7. **Cost blindness** (3.33¢ permanent impact means nothing if the spread is 5¢) → mitigate: always net the IRF against effective spread + fees before any trading claim.

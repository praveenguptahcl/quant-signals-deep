...
**Causal timing:** $r$ and $\delta^*$ are recomputed on every quote update from the *current* mid, inventory, and vol estimate — every input must be $\le t$ (stale vol or mid = stale quotes = picked off). **Variants:** (1) *infinite-horizon approximation* (drop the $T-t$ terms for a stationary quoting policy); (2) *asymmetric $\delta_a \ne \delta_b$* (the full HJB solution skews each side separately); (3) *volatility-regime A-S* (widen $\gamma$ or $\sigma$ inputs in stress rather than trusting the closed form).

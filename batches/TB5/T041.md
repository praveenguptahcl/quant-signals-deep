...
1. *Deseasonalized-vol throttle.* Each 1-minute bar, deseasonalize returns (r̃ = r / s_b, S067) and compute trailing-30-minute realized σ. If it exceeds 1.5× the HAR-implied intraday rate (*example*), multiply *m* by 0.75 (*example*).
2. *Expansion throttle.* If S076 fires (range ratio > 1.75, *example*, on a squeeze-armed tape), multiply *m* by 0.50 (*example*) for the next 6 bars (*example*).
...
| S076 — Vol breakout / squeeze | **Throttle / veto regime input** | Expansion bar (range ratio > 1.75, *example*) → *m* × 0.50 for 6 bars (*example*); squeeze-armed required |
...
for bar in session:                                # 1-min bars
    ...
    if sig30 > 1.5 * har_implied_intraday:         # example throttle band
        m = m * 0.75
    if range_ratio(bar) > 1.75 and armed:          # S076 expansion, example k
        m, throttle_until = m * 0.50, bar + 6

...
# Synthetic dealer-GEX wall map (SPY-like, $mm per 1% spot move) — synthetic strikes
...
print("Synthetic dealer-GEX map ($mm/1%):")
for k, g in zip(strikes, gex):
    print(f"  strike {k}: {g:+.0f}  ({roles[k]})")
print("Net dealer GEX +1.8bn (positive: fade regime) | flip 568 | spot 575.20 (open)")
...
print(f"  short {shares} @ {entry_px:.2f} (11:40, spot tags 580 call wall) "
      f"-> cover @ {exit_px:.2f}")
...
ax1.set_ylabel("Dealer GEX ($mm / 1%)")
ax1.set_xlabel("Strike (synthetic)")
ax1.set_title("Dealer-GEX wall map")
...
ax2.text(-1.6, entry_px + 0.5, f"short 200 @ {entry_px:.2f}", color=PALETTE["loss"], fontsize=9, va="center")

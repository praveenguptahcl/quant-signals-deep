...
P0 = 250.00
phi = np.zeros(N)
phi[0:80] = 0.12             # persistent block (trend regime)
phi[80:160] = -0.12          # anti-persistent block (reversion regime)
...
print("seed=154 | trades (side,entry_bar,exit_bar,entry_px,exit_px,gross,cost,net):")
for tr in trades:
    s, te, tx, pe, px, g, c, n = tr
    print(f"  {'LONG ' if s>0 else 'SHORT'} entry={te} exit={tx} entry_px={pe:.3f} "
          f"exit_px={px:.3f} gross={g:+.2f} cost={c:.2f} net={n:+.2f}")

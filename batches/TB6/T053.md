...
|---|---|---|---|---|
| Build on `hmmlearn` + `statsmodels` (own stack) | Full HMM library, causal filter, allocator | $0 software + ~$50–500/mo bars data | The causality is yours to verify; no black box | 100–200 h engineering (Grok's Q-TB6-2 band; indicative) |
| Retail platform (QuantConnect) | Research/backtest harness | ~$0–$96/mo | Fast sleeve backtests | You still write the HMM and the allocator |
| Vendor "regime labels" feed | Third-party bull/bear labels | Rarely listed; typically $$$$ institutional | Saves modeling time | Rarely transparent, rarely causal, never tuned to *your* sleeves — the label methodology is the product and you can't see it |
| Institutional (full quant stack) | Production regime models | $$$$ | Battle-tested | Absurd for a ≤$1M paper op |

*All prices indicative — verify before budgeting.*

**Verdict: build; buy only clean bars.** There is no serious off-the-shelf "momentum/reversal HMM allocator" worth a license — the library is small, causal inference is the whole product, and vendor labels can't be audited for lookahead. **Buy** Tier-1 adjusted bars (~$50–500/mo retail-ish, indicative) and spend the budget on the weekly refit/monitoring job, not on labels.
...
**Capacity & decay.** The allocator adds no capacity constraint beyond the sleeves'. Documented decay channel: as volatility-targeting and regime-timing crowded post-2015, the marginal value of the HMM over a plain vol target shrank — the honest claim (per Grok's Q-TB6-3 synthesis, downgraded to lead status) is *crash-avoidance vs static momentum*, smaller vs vol-targeting, and easy to fake with smoothed states. **Intraday allocator evidence is thin** — most HMM allocation work is daily; intraday, persistence is lower and turnover explodes without hysteresis.
...
2. **Smoothed-label leakage in research** — backtests that tag history with a full-sample HMM fit disagree with the live forward filter (~1 day in 11 in one ES/SPY walk, per Grok's Q-TB6-3 — *unverified chatbot claim*, treat as a warning not a statistic). *Mitigation:* forward filter only, in research and live; purged walk-forward (S088).

...
**Storage:** headlines are tiny (thousands of stories/day ≈ MBs); a multi-year archive with full text runs to low GBs — trivial. **Data-quality checklist:** first-available vs publication timestamp (a *labeled unverified chatbot lead*, Q-SB3-2 — see Unverified leads — stresses this belongs in every implementation); timezone normalization; duplicate/corrected headlines; corporate actions in the tagging; halts (news during halts is untradable); DST/half-days.
...
1. **Publication timestamp ≠ first available timestamp.** Backtests on publication times are fantasies → *use the vendor's availability timestamp; log your own receipt time (a *labeled unverified chatbot lead*, Q-SB3-2 — see Unverified leads).*

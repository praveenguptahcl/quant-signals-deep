...
Step-by-step: (1) bin 4: RVOL = 206/60 = **3.43** > 2.0 → trigger; (2) BlockImb = (25+18−0)/43 = **+1.0** → long; (3) enter 2,000 shares at \$40.00 in bin 5; (4) bin 9: RVOL = 157/55 = **2.85**, BlockImb = **−1.0** — imbalance flipped → exit at \$40.35; (5) gross = 2,000 × \$0.35 = **\$700**; (6) costs \$50 → **net +\$650**.
...
| Quotes (bid/ask) | float | L1 | Tier 1–2 | lagged midquote for the quote test |
...
t = trades.join_asof(q, on="ts", strategy="backward", tolerance="5s")  # lagged mid
t = t.with_columns(
...
8. **Options-variant confusion.** Option volume spikes can be hedging, not direction → *the 2×/500-contract rule is a flag, not a signal; confirm with equity prints.*
...
```mermaid
flowchart LR
    FEED["Raw feed<br/>(trades + L1 quotes)"] -->|"tick trades"| ING["Ingest + normalize<br/>(exchange ts, bad ticks)"]
    ING -->|"signed trades"| FEAT["Feature compute<br/>(Lee-Ready, 5-min bins)"]
    FEAT -->|"RVOL+imbalance"| SIG["Signal S094<br/>RVOL + block pressure"]
    SIG -->|"volume signal"| GATE{"Cost / toxicity<br/>gate?"}
...
**Unverified leads:** none required for this chapter — all quantitative claims above trace to the cited papers or the labeled synthetic example. RVOL trigger levels and block-size thresholds are marked *example* throughout.

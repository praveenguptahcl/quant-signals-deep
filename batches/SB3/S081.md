...
| **When it dies** | Two-sided bursts with no directional resolution, stale kernels after regime shifts, and any horizon where spread + fees exceed the few-bp edge. |
...
and symmetrically for λˢ_t. The **branching ratio** n = α/β (< 1 required for stability — **subcritical**; n → 1 means the market is near-critical) is the expected number of child events each event spawns. The signal:
...
- **T056 — predictor input (filter/feature)**: the Hasbrouck trade–quote VAR predictor takes the imbalance as an exogenous state variable — bursts modulate its quote-revision forecast and veto entries when adverse-selection intensity is extreme.
...
Collection: Databento MBO (full-depth, aggressor side known) or LOBSTER (academic MBO/ITCH replay); retail fallback: Polygon trades + SIP quotes with Lee–Ready signing (noisier). ≤20-line ingest sketch:
...
Storage per cost-model.md §4: L1 quotes+trades for a liquid name run ~2–8 GB/symbol-day parquet; trades-only is a small fraction — plan on hundreds of MB/symbol-day (planning estimate derived from §4; trades ≈ 10–200/sec busy per §2). DQ checklist: exchange-vs-SIP timestamp reconciliation; trade-signing accuracy audit; corp actions; halts/auctions excluded; DST/half-days; venue symbology mapping.
...
| Sjogren & DeLise (2021), arXiv:2110.07075 — general compound Hawkes for mid-price prediction | Futures and stock LOB data | Predicts mid-price direction and volatility | Before-cost (prediction study, not net trading) | Accuracy ≠ tradable edge after spread/fees |
...
4. **Trade-signing error:** Lee–Ready misclassifies ~10–15% of trades in fast markets, injecting noise exactly when bursts matter. Mitigation: MBO aggressor side where available; audit signing accuracy.
...
7. **Cost blowup:** a seconds-horizon signal pays the spread every round trip. Mitigation: model spread + fees per trade explicitly; most calibrations fail this gate — that is the signal telling you the truth.

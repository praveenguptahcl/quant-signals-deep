...
| Class weight / sampling | — | — | ignored: predicts the majority class | balance or PR-AUC tuning — *example* |
...
Synthetic tape, seed **86086**: 12 primary-signal trades with side $s_t$, features (rvol_z, sprd_z), triple-barrier outcome $y_t$, meta probability $\hat{p}_t$, and decision at $\tau = 0.55$ (*example*). Economics: win +\$18, loss −\$14, \$2 round-trip cost → taken win nets +\$16, taken loss nets −\$16. All numbers synthetic.
...
- **T092 — veto**: the PIN/toxicity stand-down rule is itself meta-labeled — the meta-model learns when the stand-down is trustworthy.
...
**Stack options:** Python + polars + XGBoost/scikit-learn (AFML's reference stack); Rust buys nothing here; DuckDB if the event store outgrows RAM. **Engineering time:** Tier **L**, 4–12 h → **\$600–1,800** loaded cost at \$150/hr (per `notes/cost-model.md` §5). *One-line verdict: feasible on the M5 Max (Tier L, ~8 h ≈ \$1.2k eng + the primary's data cost); bottleneck is feature quality, not compute; nothing breaks at 500 symbols.*
...
| Tier-2 professional feed | Honest L1/L2 for microstructure features (Databento class) | ~\$200/mo + usage, *indicative — verify before budgeting* | Microstructure honesty | Usage metering |
| Academic route | AFML (2018) + the author's SSRN lecture notes | Book cost (~tens of \$) | The canonical specification | Still a build, not a product |
...
| Practitioner replication: lopez-de-prado-work-review (GitHub, 2026) | US ETFs, EMA(20/60) crossover primary | After a 2 bp round-trip cost, XLF/XLV produced <50 net-profitable bets and were dropped by the minimum-class floor | **After cost** (2 bp) | Not peer-reviewed; the harness acted as a *filter*, rejecting weak primaries rather than manufacturing profit |
| Chatbot lead Q-SB3-3 (Duck.ai, 2026-09-10) | General | "PRIMARY SUPPLIES THE SIDE — meta-labeling is NOT a replacement for real economic signal; Sharpe 0.2 → 2.0 is implausible" | n/a | *Unverified chatbot claim*; consistent with AFML's framing |
...
5. **Class imbalance.** 80% of bets lose → the model predicts "skip everything" → *class weights; judge by PR-AUC and net P&L, never raw accuracy.*
...
flowchart LR
    ...
    ING -->|"clean bars"| FEAT["Feature compute<br/>(meta features at event t)"]
    FEAT -->|"event features"| SIG["Signal S086<br/>meta P(win) overlay"]
    SIG -->|"bet prob"| GATE{"Cost / toxicity<br/>gate?"}
...
- darufinance (2026). "lopez-de-prado-work-review — Project 09" — EMA(20/60) primary, triple-barrier labels, bagging meta-labelers, 2 bp cost; XLF/XLV dropped below the 50-net-bet floor. https://github.com/darufinance/lopez-de-prado-work-review/blob/HEAD/projects/09_ensembles_importance/writeup/README.md
...
- Duck.ai (same), Q-SB3-3: "PRIMARY SUPPLIES THE SIDE"; Sharpe 0.2 → 2.0 via meta-labeling is implausible; triple-barrier/meta-labeling only after the primary shows positive purged OOS.

...
Provenance **[D]** (documented: López de Prado, *AFML*, 2018, ch. 7). Family **F — Statistical/ML infrastructure**. This is a *validation procedure*, not a signal: it generates no trades, only trustworthy performance estimates.
...
**Embargo rule.** Additionally drop a buffer of $L_E$ observations at the fold boundary, with $L_E$ at least the maximum label look-forward horizon: $L_E \ge \max_i (t_i(1) - t_i(0))$. Practitioner guidance from the batch's chatbot source (Duck.ai, GPT-5.6 Luna, 2026-09-10 — *labeled lead*): 0–1% of the sample for short non-overlapping labels; 2–5% for daily 5–10-day horizons; 5–10% for long horizons; 5–20% for highly overlapping intraday labels; always take the larger of the percentage rule and the horizon rule.

**⚠ Convention flag (operator-verified).** The chatbot's worked example (Q-SB3-1) implements the embargo by removing the *first 3 test events*; de Prado's standard (AFML ch. 7) removes the *last train observations before the test fold*. The arithmetic is identical (3 removed either way; 35 train / 17 test / 8 removed = 60), but the boundary side differs — do not present the chatbot's choice as de Prado's standard. The example and chart below follow the chatbot's verified arithmetic and label it; either convention breaks the overlap, but document whichever you use.
...
Step-by-step: (1) **Purge** — training candidates are $E_1$–$E_{40}$; drop any with $t_0 \le 65$ and $t_1 \ge 41$: $E_{36}$ ([36,41]) through $E_{40}$ ([40,45]) — 5 events; (2) **Embargo** (chatbot convention, flagged above) — drop the first 3 test events $E_{41}$–$E_{43}$; (3) remainder: train $E_1$–$E_{35}$ (35), test $E_{44}$–$E_{60}$ (17), removed 8. Check: 35 + 17 + 8 = 60 ✓ (arithmetic verified against the chatbot's Q-SB3-1 example).
...
folds = (ev.with_row_index("k").with_columns((pl.col("k") % K).alias("fold")))
def purged_train(test_fold: int):
    ta, tb = folds.filter(pl.col("fold") == test_fold).select("t0","t1").row(0)
    test = folds.filter(pl.col("fold") == test_fold)
    train = folds.filter(
        (pl.col("fold") != test_fold)
        & ~((pl.col("t0") <= tb) & (pl.col("t1") >= ta))   # purge overlap
    ).sort("t0").slice(EMB, None)                          # embargo (train-side; see S3)
    return train, test
```
...
Purged CV has no "returns" — its documented value is measured in *phantom performance destroyed*: studies where naive validation showed edge and honest validation showed none.| Study / source | Market & period | Metric | Before/after cost | Caveat |
|---|---|---|---|---|
...
| Bailey, Borwein, López de Prado & Zhu (2017), "The Probability of Backtest Overfitting" | Simulated: random-walk prices with a fake seasonal effect (~4 years daily) | IS Sharpe 1.0–2.2 across optimized configs, yet PBO ≈ 55% — the IS-best config lands in the *bottom half* out-of-sample about as often as a coin flip | **Before cost** (diagnostic demonstration) | Demonstrates selection overfitting; the companion example with a *real* injected monthly effect shows PBO near 0 — the test discriminates correctly |
| Practitioner replication: PBO complexity ladder (GitHub, rchhabra17, 2026) | SPY, 2010–2026, MA-crossover configs, CSCV S=16, costs held fixed | PBO 0.37–0.57 across complexity rungs; PBO did *not* rise monotonically with parameter count | **Before cost** (costs fixed across rungs) | Single market/single strategy family; shows the diagnostic working, not a trading edge |
| Chatbot lead Q-SB3-1 (Duck.ai, GPT-5.6 Luna, 2026-09-10) | Synthetic 60-event schedule | 35 train / 17 test / 8 removed = 60 — arithmetic **verified** by the operator | n/a — schedule, not performance | *Labeled chatbot source*; boundary-side convention differs from de Prado's standard (S3 flag) |
...
6. **Tuning the embargo on the test set.** Trying embargo lengths until OOS looks good is itself overfitting → *fix the embargo rule before seeing test results.*
...
```mermaid
flowchart LR
    FEED["Raw feed<br/>(labeled events)"] -->|"event table"| ING["Ingest + normalize<br/>(t0, t1 per event)"]
    ING -->|"timed events"| FEAT["Feature compute<br/>(purge + embargo masks)"]
    FEAT -->|"fold masks"| SIG["Signal S088<br/>leak-free CV folds"]
    SIG -->|"CV score"| GATE{"Cost / toxicity<br/>gate?"}

...
**Quoted spread** (dollars; half-spread in bps):
...
$$S^r_t = 2 \, D_t \, (M_{t+\Delta} - M_t)$$

**Price impact** (adverse selection; the residual):
$$\mathrm{PI}_t = S^e_t - S^r_t$$
...
| Direction rule | $D_t$ | quote rule / tick rule / LR | mis-signing flips $S^e$ sign | — | quote rule on NBBO *example* |
...
| 2 | 99.992 | 100.022 | 100.007 | +1 | 100.019 | +2.40 | +0.42 | +1.99 |
| 3 | 100.008 | 100.018 | 100.013 | +1 | 100.018 | +1.01 | −1.34 | +2.35 |
| 4 | 99.996 | 100.026 | 100.011 | −1 | 99.997 | +2.64 | +0.26 | +2.39 |
| 5 | 99.999 | 100.019 | 100.009 | −1 | 99.999 | +2.09 | +0.01 | +2.08 |
| 6 | 99.996 | 100.016 | 100.006 | +1 | 100.015 | +1.70 | +1.26 | +0.44 |
| 7 | 99.994 | 100.024 | 100.009 | −1 | 99.995 | +2.85 | +1.16 | +1.69 |
| 8 | 99.994 | 100.024 | 100.009 | +1 | 100.025 | +3.29 | +0.47 | +2.82 |
| 9 | 99.998 | 100.028 | 100.013 | −1 | 99.997 | +3.12 | +0.38 | +2.74 |
| 10 | 99.993 | 100.013 | 100.003 | −1 | 99.995 | +1.78 | −2.15 | +3.93 |
| 11 | 99.996 | 100.026 | 100.011 | −1 | 99.998 | +2.58 | −0.45 | +3.03 |
| 12 | 99.996 | 100.026 | 100.011 | +1 | 100.025 | +2.92 | +0.16 | +2.75 |
...
$S^r = 2(100.009-100.007) = +0.0042$ = **+0.42¢** — the provider kept less than a
fifth of what the taker paid.

**Step 3 — impact.** $\mathrm{PI} = 2.40 - 0.42 = \mathbf{+1.99¢}$ — the rest was
adverse selection / drift.

**Step 4 — averages.** Mean effective **+2.33¢**, mean realized **+0.19¢**, mean
impact **+2.14¢**: in this toy tape, liquidity providers collected 2.33¢ and kept
0.19¢ — over 90% went to price impact.
...
impact (red). Two honest artifacts of toy data: trade 1's impact is *negative*
(−0.55¢, the mid moved in the provider's favor — pure noise here), and several
realized spreads are negative, which in real data flags toxic flow but here is just
the random walk. Real tapes also show price improvement (effective < quoted) far more
often than this one. And again: no fees, no latency, twelve trades — this is
arithmetic practice, not a cost study.
...
out = t.with_columns(
    ...
    rlzd=2 * pl.col("D") * (pl.col("mid_fwd") - pl.col("mid")),
).with_columns(impact=pl.col("eff") - pl.col("rlzd"))
```

...
- **Mental model:** there is no universal "gaps continue X% of the time" — continuation probability is a function of gap size (in ATR), news, and premarket volume.
...
**What to notice.** Scenario B is the mandatory lesson: a +1.5% gap on no news fades *through* the prior close — the naive "buy every gap" rule loses on both legs here. Scenario C shows why stops exist: a 2.5% gap that initially pushes higher still ends −0.5% on the day. **Limits:** hand-picked anchors; no spreads (the opening spread is the day's widest — often 5–20 bps on mid-caps), no chase slippage, no fees. Mechanics illustration, not a backtest.
...
- **T012 — Jump-Filtered Overnight-Gap Fade** — *filter/veto*: if S023's go conditions confirm (news flag + RVOL + first-minutes high break), veto the fade — the gap is informed and fading it is picking up pennies in front of a steamroller.
...
| 1-min OHLCV | float/int | 1-min, first 60–90 min | Tier 1 | SIP bars suffice |
...
| Tier 2: Databento Standard | L1 + imbalance + OHLCV | ~$200/mo + usage | honest auction prints, venue detail | overkill for a daily-gap signal |
...
| Duck.ai Q-SB5-3 (labeled chatbot lead) | Practitioner synthesis | No stable universal continuation % — stratify by gap size in ATR, news/no-news, premarket volume, opening spread; report E[return], not P(continue) | n/a | *Unverified chatbot claim* — but the stratification advice matches the papers above |
...
8. **Cost blowup:** the go's positive skew comes with poor hit rate; two full-stop losses erase a week of small wins. Mitigate: size by the E[return] equation with costs; hard daily loss stop.

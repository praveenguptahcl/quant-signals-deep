...
- **T003 — VPIN-Gated Breakout Trader** — breakout-timing candidate: Donchian breaks are taken only when order-flow toxicity (VPIN) is low; toxicity spikes veto the trade.
...
| Rust | Only if you drop to L1 tick events across hundreds of symbols (Python loop tops out ~100–500k events/sec per cost-model §2) |
...
| Academic: LOBSTER / TAQ | MBO/ITCH replay, full tick history | ~hundreds/yr academic; institutional $$$$ | True microstructure research | Cost and complexity far beyond the signal's needs |
...
| Rayome & Jain (2008), "Do Turtles Have Fat Tails?", *J. of the Academy of Finance* | Soybeans; Donchian 20-day & 55-day systems | Academic examination of Turtle-style Donchian channels | Before-cost | Single-commodity study; tests the construction, not an intraday strategy |
| Evidence synthesis, duck.ai Q-SB5-3 (2026-09-10; labeled chatbot) | Intraday equities, practitioner literature | No documented universal intraday after-cost alpha; results highly regime-dependent, commonly erased by whipsaws, spread, slippage, session effects | After-cost orientation | Chatbot synthesis, not a paper — see Unverified leads |
...
flowchart LR
    ...
    GATE -->|"pass: order intent"| OUT["Downstream consumer<br/>(T019 / T003)"]
...
2. Rayome, A. & Jain, R. (2008) — "Do Turtles Have Fat Tails? Donchian Channels and Turtle Trading: The Case of Soybeans," *Journal of the Academy of Finance*, Summer 2008. http://carrels.distantreader.org/journal-journalOfFinanceIssues-ojsoai/cache/jfi-2421.pdf

...
$$\%D_t = \mathrm{SMA}(\%K_t, 3) = \frac{\%K_t + \%K_{t-1} + \%K_{t-2}}{3}$$
...
2. **Stochastic RSI / %R with different lookbacks** — practitioners often run %R(14) against Stoch(14,3) for confirmation; as shown above they're near-duplicates, so the honest variant is *different* lookbacks, not different names.
...
|---|---|---|---|---|---|---|---|---|
| 9 | 101.55 | 101.95 | 101.62 | 101.78 | — | — | — | |
| 10 | 101.78 | 102.16 | 101.82 | 102.00 | — | — | — | |
| 11 | 102.00 | 102.81 | 102.45 | 102.62 | — | — | — | |
| 12 | 102.62 | 103.27 | 102.91 | 103.10 | — | — | — | |
| 13 | 103.10 | 103.58 | 103.23 | 103.42 | — | — | — | |
| 14 | 103.42 | 103.78 | 103.45 | 103.62 | 95.9 | — | −4.1 | |
| 15 | 103.62 | 103.91 | 103.55 | 103.71 | 94.9 | — | −5.1 | |
| 16 | 103.71 | 103.88 | 103.53 | 103.70 | 95.4 | 95.0 | −5.7 | |
| 17 | 103.70 | 103.59 | 103.24 | 103.41 | 94.3 | 94.8 | −14.6 | **SHORT cross** |
| 18 | 103.41 | 103.21 | 102.87 | 103.06 | 75.9 | 88.4 | −24.1 | |
| 19 | 103.06 | 102.88 | 102.51 | 102.68 | 60.7 | 77.2 | −39.3 | |
| 20 | 102.68 | 102.47 | 102.12 | 102.31 | 43.3 | 58.1 | −57.7 | |
| 21 | 102.31 | 102.14 | 101.75 | 101.95 | 22.5 | 41.1 | −77.5 | |
| 22 | 101.95 | 101.81 | 101.45 | 101.61 | 6.6 | 23.8 | −93.4 | |

Step-by-step at the trigger bar: at bar 16's close, %K = 95.4 ≥ %D = 95.0 (the 3-bar SMA of the unrounded %K values). At bar 17's close, %K = 94.3 < %D = 94.8 — a cross down, with %K at 94.3 > 80 (*example* overbought line) → **short fade**. Williams %R confirms the overbought read: −14.6, inside the −20 line. Earliest fill: bar 18's open at 103.41. Cover example: bar 20's close at 102.31 → gross **+1.10 = +106 bps** before costs — the fade catches the first leg of the rollover.

**What to notice:** the cross at bar 17 is *marginal* — %K 94.3 vs %D 94.8, a 0.5-point cross. That's deliberate pedagogy: textbook crosses are often this thin, and a production rule needs a minimum cross magnitude or %R-confirmation to avoid trading dust. Also note %R collapses to −93.4 by bar 22 — deep "oversold" — while price is still falling: the classic pinned-at-the-extreme behavior that punishes mechanical fading without a regime check. This toy tape has no spread, no fees, and a perfectly clean rollover; nothing here is a performance claim.

### S5. Strategies that use this signal

- **T013 — RSI-2 / IBS Extreme Fade** — *primary oscillator-fade consumer (genuine fit)*: Connors-style oversold/overbought fading combining RSI-2 and internal bar strength; the %K/%D crossover and %R extremes are the natural timing overlay for its fade entries.
...
|---|---|---|---|---|
| Open, high, low, close | float | 5–15 min bars (intraday) | Tier 0–1 | Any OHLCV vendor; SIP consolidated is fine |
...
What breaks first at 500 symbols or full OPRA: nothing — this signal never touches options or depth; the full US universe on 5-min bars remains a trivially small frame.
...
1. **Trend-pinning** — the #1 killer: fading every new high in a trend. *Mitigation:* regime-gate — only take fades when a trend filter (e.g. ADX, or price within a defined range for N bars) says "range."
...
5. **Cost blowup** — oscillators on 1-min bars can fire several times a day; each fade pays the full spread at the worst moment (you're providing liquidity to momentum). *Mitigation:* model full spread-crossing cost; require expected rotation > 3× cost; prefer 5–15 min grids.
...
flowchart LR
    ...
    FEAT -->|"oscillator values (bar-level)"| SIG["Signal S044<br/>stochastic / %R fade"]
    SIG -->|"signal (bar-level)"| GATE{"Cost / regime<br/>gate?"}

...
|---|---|---|---|---|
| Open, high, low, close | float | 1–15 min bars | Tier 0–1 | Any OHLCV vendor; consolidated SIP is fine |
...
bars = (bars.sort(["symbol","ts"])
            ...
            .with_columns(streak = pl.when(pl.col("sign")==pl.col("sign").shift(1).over("symbol"))
                                    .then(pl.col("streak").shift(1).over("symbol")+1)
                                    .otherwise(1)))
```
...
Stack: **Python+polars** — pick it; nothing here justifies Rust (no L2/MBO event rates). DuckDB only if you want SQL access to the bar archive.
...
What breaks first at 500 symbols or full OPRA: nothing — this signal doesn't touch options or depth; scaling to the full US universe is still a bars problem, trivial.
...
| Academic route (CRSP/TAQ via WRDS) | Publication-grade history | institutional — *indicative, verify before budgeting* | Reproducible research archive | Cost and access friction for zero analytic gain |
...
| Haendler, Heston, Korajczyk & Sadka (2025), "The intra-day stock return periodicity puzzle" (out-of-sample follow-up) | US stocks, extended sample | Open/midday periodicity fully explained by institutional trading proxies (VWAP-like, MOC); up to 30% of close periodicity | Before-cost (explanatory, not a trading P&L) | Confirms the institutional-flow mechanism behind intraday continuation — supports the continuation story, not a parameter set |
...
4. **Cost blowup** — a 5-bar streak on a 1-min grid can mean 20+ round trips a day; spread alone eats a 2–3 bps edge. *Mitigation:* model half-spread + fees per trade in the trigger test; require expected move > 3× cost.
...
flowchart LR
    ...
    FEAT -->|"streak S_t, direction (bar-level)"| SIG["Signal S031<br/>continuation / exhaustion"]
    SIG -->|"signal (bar-level)"| GATE{"Cost / toxicity<br/>gate?"}

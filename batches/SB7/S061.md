...
| | |
|---|---|
| **What it is** | FX-adjusted price gap between an ADR (a US-listed certificate representing foreign shares) and its home-market shares; trade it toward zero. |
...
overlap, use the last synchronous print (or the Hayashi–Yoshida lead-lag alignment of S060,
`example` choice) — never mix a live ADR quote with a stale home-market close.
...
| Entry threshold | `z_entry` | 1.5–3.0 | trades inside the cost band | never trades | 2.0 (`example`) |
...
Variants: (1) **log-premium OU fit** — AR(1) half-life of `ℓ_t`, size by expected decay (S051);
...
**The after-cost reality** (operator-verified arithmetic, chatbot-sourced cost stack labeled in S12):
...
| Tier 1: Polygon Stocks Advanced + FX | 1-min/real-time SIP bars, FX ticks, corp actions | ~$30–200/mo | Clean API, one vendor | Borrow data absent; ADR ratio history thin |
| Tier 2: Databento + borrow-data vendor | L1/L2 + borrow/availability history | ~$200/mo + usage; borrow history ~$10–50k/yr | Honest microstructure + the borrow leg that decides tradability | Cost; borrow history is the expensive part |
| Academic/institutional: TAQ + OptionMetrics-style refs | Publication-grade history | $$$$ institutional | Point-in-time everything | Overkill for a monitor |
...
|---|---|---|---|---|
| Gagnon & Karolyi (2010), "Arbitrage opportunities in the depositary receipts market: Myth or reality?" | 506 US cross-listed stocks, 35 countries, intraday quotes | Mean deviation from parity **4.9 bps**; deviations positively related to holding-cost proxies after controlling for transaction costs and foreign investment restrictions | Before-cost (descriptive) | 4.9 bps is the *average* — small vs any realistic round-trip stack |
| Duck.ai chatbot literature summary (Q-SB7-3), arithmetic operator-verified 2026-09-10 | Illustrative ADR trade | Gross 1.50% → net **−0.10%** after spread/fees/borrow/FX/slippage | After-cost (illustrative) | Chatbot-sourced cost stack — `unverified chatbot claim` as literature, arithmetic verified |
...
```mermaid
flowchart LR
    FEED["Raw feeds<br/>(ADR quotes + local quotes + FX ticks)"] --> ING["Ingest + normalize<br/>(dual calendars, DST, halts, ratio changes)"]
    ...
    SIG --> GATE{"Cost / borrow / hours<br/>gate?"}
...
1. Gagnon, L. & Karolyi, G.A. — "Arbitrage opportunities in the depositary receipts market:
   Myth or reality?" (citing literature dates the Gagnon–Karolyi ADR-arbitrage work to 2010):
   ...
   https://www.researchgate.net/publication/4959640_Arbitrage_opportunities_in_the_depositary_receipts_market_Myth_or_reality

...
Setup. Formation window of n returns per leg: r_{A,t}, r_{B,t}. Fit marginals F̂_A, F̂_B (empirical CDF or parametric), then
...
|---|---|---|---|---|
| Copula family | C(·;θ) | Gaussian, t, Clayton, Gumbel, Frank | One family only → misspecification risk; kitchen-sink → overfitting | Student-t, selected by rolling OOS likelihood (example) |
...
**What to notice (and its limits):** the toy tape has no fees, no borrow cost, no bid–ask spread, and only 10 days — it demonstrates the trigger mechanics, not profitability. Two entries in 10 days is illustration, not a trade-frequency claim. In real data the marginals must be estimated (PIT error), ρ must be estimated with uncertainty, and the 0.05/0.95 thresholds are example choices, not institutional standards.
...
- **T100 — Grand Ensemble** — component feature: the S054 conditional probabilities and mispricing index feed the capstone all-signal blend as tail-dependence features alongside linear pairs signals.
...
| Borrow/availability (for short leg) | snapshot | daily | Tier 1–2 | Short-sale feasibility; HTB names kill the trade |

Collection: any OHLCV vendor (Stooq/Alpaca free daily; Polygon Stocks Advanced ~$30–200/mo indicative — verify before budgeting). Ingest sketch (≤20 lines, Python/polars):
...
| Academic route (WRDS / papers' code) | Replication datasets, published pair lists | institutional | Ground-truth methodology checks | Not a live feed |
...
| Chatbot research lead (Q-SB7-3): adaptive copula study on crypto perpetual futures, Gaussian/t/Clayton/Gumbel/Frank + AIC + PIT diagnostics | Crypto perps (recent) | Negative net returns at 0.08% baseline round-trip cost despite contained drawdowns | After-cost | Unverified chatbot claim — literature lead, not a citable result |
...
5. **Multiple testing across pairs:** 124,750 pair tests will find "tail dependence" by luck. Mitigation: economic prefilter (sector/geography), multiple-testing control (FDR), OOS validation of the selected set.
...
8. **Stale/illiquid legs:** zero-volume days produce fake PIT ranks. Mitigation: liquidity filter (ADV, spread cap) before the pair ever enters the screen.
...
flowchart LR
    ...
    FEAT -->|uniforms u_t, v_t| SIG["Signal S054<br/>copula conditional prob p(A|B)"]

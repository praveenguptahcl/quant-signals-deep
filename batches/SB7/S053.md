...
| **When it works** | Large candidate universes where most "cointegrated" pairs are statistical mirages; the filter cheaply discards slow, excursion-prone spreads before they cost money. |
...
**Why crossings measure reversion speed.** For an AR(1) spread with coefficient $\phi$, the expected crossing rate is approximately $\arccos(\phi)/\pi$ per bar (Rice's-formula intuition): $\phi = 0.7$ → ~0.25 crossings/bar (~63/year on daily bars); $\phi = 0.995$ → ~0.03/bar (~8/year). More crossings ⇔ faster, tighter mean reversion ⇔ the OU half-life (S051) is short. The two signals are cousins: S051 estimates the clock parametrically, S053 reads the rhythm non-parametrically.
...
|---|---|---|---|---|
| Gatev, Goetzmann & Rouwenhorst (2006) | CRSP US, 1962–2002 | ~11% p.a. excess return; entries at 2σ, exits at the next zero crossing | **before cost** | the exit mechanic this filter is built around; decay after 1989 |
| Do & Faff (2010) | US, 1962–2009 | profitability concentrated in 1970s–80s; marked decline after ~1989 | **before cost** | via CIR WP PT2.5 summary; motivates quality gating |
| Do & Faff (2012) | US | transaction costs materially erode pairs profitability | **after cost** | via MDPI/CIR summaries; the filter must earn its keep net of costs |

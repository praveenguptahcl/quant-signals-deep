...
*Batch SB7 · Signal 90/200 · Provenance [D] · Family F — Statistical/ML Infrastructure*
...
| **When it works** | As a slow regime label over rolling windows with confidence intervals — it conditions *which* strategy family gets risk, not when to enter. |
...
Provenance **[D]**: Lo–MacKinlay (1988) variance ratio and Hurst (1951) rescaled-range analysis are
documented statistical methods; the regime-toggle *trading* use is practitioner reconstruction.
...
Why should regimes exist at all? Order-flow regimes (institutional VWAP execution vs. retail
...
Four-period block (verified unchanged): VR(4) = **0.5667**, Ĥ(4) = (1/2)[1 + ln(0.5667)/ln 4]
≈ **0.295** → anti-persistence / mean reversion at k = 4.

What to notice — the chart's punchline: **the same tape says momentum at k=2 (Ĥ=0.826) and
reversal at k=4 (Ĥ=0.295)**. A single short sample contradicts itself across horizons, which is
...
| Duck.ai chatbot literature summary (Q-SB7-3) — `unverified chatbot claim` | Practitioner VR-toggle literature | VR-toggle strategies "can look attractive in-sample but lose after turnover"; standalone weak due to finite-sample noise, Hurst bias from short memory + vol clustering, structural breaks mimicking long memory, microstructure noise → apparent anti-persistence, horizon selection, discovered-after-testing bias | After-cost (claimed) | Chatbot summary of literature, no checkable citation — treat as a research lead |
| This chapter's worked example | Synthetic 9-return tape | k=2 → momentum label; k=4 → reversal label — the label contradicts itself | N/A — toy | Demonstrates why single-sample VR is a diagnostic, not a rule |
...
validated against no-toggle, MA-trend-filter, and vol-only-gating baselines under the same
information set.
...
```mermaid
flowchart LR
    FEED["Raw feed<br/>(1-min bars, any vendor)"] --> ING["Ingest + normalize<br/>(drop overnight gaps, adjust corp actions)"]
    ING -->|"1-min log returns, RTH only"| FEAT["Rolling VR/Hurst<br/>(k=2,4,8, W=250, robust SEs)"]
    FEAT -->|"VR(k), H^(k) per bar"| SIG["Signal S090<br/>VR/Hurst regime label"]
    SIG --> GATE{"Stable label +<br/>outside CI? toggling cheap?"}

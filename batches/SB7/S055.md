...
ΔX_t = Π·X_{t−1} + Σ_{i=1}^{k} Γ_i·ΔX_{t−i} + ε_t, ε_t ~ i.i.d.(0, Σ).
...
**Normalization choices:** prices vs log prices (logs keep β interpretable as elasticities); β̂ normalized (e.g. first coefficient = 1) since vectors are identified only up to scale; spreads centered on formation mean and scaled by formation σ. Lag order by AIC/BIC on the underlying VAR.
...
|---|---|---|---|---|---|
| r = 0 | 56.61 | 35.19 | 48.98 | 22.30 | reject — at least 1 relation |
| r ≤ 1 | 7.63 | 20.26 | 4.87 | 15.89 | **accept — rank = 1** |
| r ≤ 2 | 2.77 | 9.16 | 2.77 | 9.16 | accept |
...
Signal events over the full 120 days (marked on the chart): day 14 (+1.73), day 30 (+2.33), day 87 (+2.19) breach +2σ → short-basket entries; day 38 (−1.90) breaches −2σ → long-basket entry. A short basket on day 87 means: sell 1.0 unit of X1, buy 0.7113 of X2, buy 0.3631 of X3; exit when e_t re-crosses toward zero.

**What to notice (and its limits):** the toy has no costs, and the truth (rank 1, β = [1, −0.7, −0.4]) is known — real applications never get that comfort. Note what the example teaches anyway: (a) the test correctly found rank 1 here, but with N = 60 on the same seed it over-rejected toward rank 2 — small samples inflate rank, which is why the 120-observation minimum matters; (b) β̂ ≠ β exactly — hedge ratios carry estimation error, and sizing must respect it; (c) the band breach on day 38 (−1.90 vs −1.712) is marginal — real desks demand confirmation (persistence, cost check) rather than trading every touch.
...
- **T007 — Cointegration Z-Score Pairs** — filter/veto: run the Johansen rank test on the Engle–Granger pair shortlist first; only pairs surviving the multivariate rank check earn capital, which screens out pairwise artifacts that contradict the joint structure.
...
|---|---|---|---|---|
| De Bruijn et al. (2019), Erasmus thesis: Johansen-assisted vs EG-only pair selection | Crypto FX pairs, 16 weeks | 6.81%/week (Johansen) vs 5.97%/week (EG2SLS), incl. transaction costs | After-cost | Tiny sample (16 weeks); crypto; student thesis, not peer-reviewed |
...
6. **Cost blindness:** 3–5 leg baskets pay 3–5 spreads plus borrow on each short leg. Mitigation: full multi-leg cost model per trade; legs with HTB borrow veto the basket.
...
flowchart LR
    ...
    ING -->|aligned N-leg panel| FEAT["Feature compute<br/>(Johansen rank, beta-hat, EC spread)"]
